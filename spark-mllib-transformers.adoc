...
A *transformer* is a function that maps (aka _transforms_) a `DataFrame` into another `DataFrame`.

```
transformer: DataFrame => DataFrame
```

It simply prepares a dataset for an machine learning algorithm to work with.
