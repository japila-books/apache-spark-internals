...
`TaskScheduler` schedules link:spark-taskscheduler-tasks.adoc[tasks] for a single link:spark-anatomy-spark-application.adoc[Spark application] according to link:spark-taskscheduler-schedulingmode.adoc[scheduling mode].
...
`TaskScheduler` is given sets of tasks (as link:spark-taskscheduler-tasksets.adoc[TaskSets]) submitted to it from the link:spark-dagscheduler.adoc[DAGScheduler] for each stage, and is responsible for scheduling the tasks to the cluster, running them, retrying if there are failures, and mitigating stragglers.
