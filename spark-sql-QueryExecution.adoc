...
NOTE: `toStringWithStats` is used when `ExplainCommand` link:spark-sql-ExplainCommand.adoc#run[is executed] (when `cost` attribute is enabled, and it is not by default).
