...
| `LogicalPlan` (of a structured query) being the result of executing the session-owned link:spark-sql-SessionState.adoc#optimizer[Catalyst Query Optimizer] to <<withCachedData, withCachedData>>.
...
=== [[toStringWithStats]] Building Text Representation with Stats -- `toStringWithStats` Method
...
NOTE: `toStringWithStats` is used when `ExplainCommand` link:spark-sql-LogicalPlan-ExplainCommand.adoc#run[is executed] (when `cost` attribute is enabled, and it is not by default).
