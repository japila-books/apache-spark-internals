...
NOTE: `executedPlan` is the phase when link:spark-sql-whole-stage-codegen.adoc#CollapseCodegenStages[CollapseCodegenStages] physical optimization rule is executed to collapse physical operators that support code generation together as a link:spark-sql-SparkPlan-WholeStageCodegenExec.adoc[WholeStageCodegenExec] operator.
...
=== [[prepareForExecution]] Optimizing Physical Plan (by Executing preparations Physical Optimization Rules) -- `prepareForExecution` Method
