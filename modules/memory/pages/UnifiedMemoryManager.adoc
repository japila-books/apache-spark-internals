...
*UnifiedMemoryManager* is the default xref:MemoryManager.adoc[MemoryManager] (based on xref:ROOT:spark-configuration-properties.adoc#spark.memory.useLegacyMode[spark.memory.useLegacyMode] configuration property).
...
`apply` computes the <<getMaxMemory, maximum heap memory>> (using the input xref:ROOT:spark-SparkConf.adoc[SparkConf]).

`apply` computes the size of the on-heap storage region which is a fraction of the maximum heap memory based on xref:ROOT:spark-configuration-properties.adoc#spark.memory.storageFraction[spark.memory.storageFraction] configuration property (default: `0.5`).

In the end, `apply` creates a <<creating-instance, UnifiedMemoryManager>> (with the given and computed values).

`apply` is used when `SparkEnv` utility is used to xref:ROOT:spark-SparkEnv.adoc#create[create a SparkEnv] (for the driver and executors).
