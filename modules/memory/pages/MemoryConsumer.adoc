...
NOTE: If not specified, `pageSize` defaults to link:TaskMemoryManager.adoc#pageSizeBytes[TaskMemoryManager.pageSizeBytes] and `ON_HEAP` memory mode.
...
`acquireMemory` link:TaskMemoryManager.adoc#acquireExecutionMemory[acquires execution memory of `size` size]. The memory is recorded in <<used, used>> registry.
