...
UnsafeShuffleWriter <<open, opens a ShuffleExternalSorter and the buffers>> immediately while being <<creating-instance, created>>.
...
insertRecordIntoSorter requests the <<serOutputStream, SerializationStream>> to write out the record (write the xref:ROOT:spark-SerializationStream.adoc#writeKey[key] and the xref:ROOT:spark-SerializationStream.adoc#writeValue[value]) and to xref:ROOT:spark-SerializationStream.adoc#flush[flush].
