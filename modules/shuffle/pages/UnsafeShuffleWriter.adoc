...
insertRecordIntoSorter requests the <<serOutputStream, SerializationStream>> to write out the record (write the xref:ROOT:SerializationStream.adoc#writeKey[key] and the xref:ROOT:SerializationStream.adoc#writeValue[value]) and to xref:ROOT:SerializationStream.adoc#flush[flush].
...
insertRecordIntoSorter requests the <<partitioner, Partitioner>> for the xref:rdd:spark-rdd-Partitioner.adoc#getPartition[partition] for the given record (by the key).
...
closeAndWriteOutput removes the internal `ByteArrayOutputStream` and xref:ROOT:SerializationStream.adoc[SerializationStream].
...
open requests the <<serializer, SerializerInstance>> for a xref:ROOT:spark-SerializerInstance.adoc#serializeStream[SerializationStream] to the <<serBuffer, serBuffer>> (available internally as the <<serOutputStream, serOutputStream>> reference).
...
xref:ROOT:SerializationStream.adoc[SerializationStream] (that is created when the <<serializer, SerializerInstance>> is requested to xref:ROOT:spark-SerializerInstance.adoc#serializeStream[serializeStream] with the <<serBuffer, ByteArrayOutputStream>>)
