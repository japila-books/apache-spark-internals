...
`FetchFailedException` exception xref:ROOT:spark-Executor-TaskRunner.adoc#run-FetchFailedException[may be thrown when a task runs] (and xref:ROOT:spark-ShuffleBlockFetcherIterator.adoc#throwFetchFailedException[`ShuffleBlockFetcherIterator` did not manage to fetch shuffle blocks]).
