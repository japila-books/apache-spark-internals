...
`FetchFailedException` exception xref:core:TaskRunner.adoc#run-FetchFailedException[may be thrown when a task runs] (and xref:storage:ShuffleBlockFetcherIterator.adoc#throwFetchFailedException[`ShuffleBlockFetcherIterator` did not manage to fetch shuffle blocks]).
...
When `FetchFailedException` is reported, xref:core:TaskRunner.adoc#run-FetchFailedException[`TaskRunner` catches it and notifies `ExecutorBackend`] (with `TaskState.FAILED` task state).

The root `cause` of the `FetchFailedException` is usually because the xref:core:Executor.adoc[] (with the xref:storage:BlockManager.adoc[BlockManager] for the shuffle blocks) is lost (i.e. no longer available) due to:
