...
`getDataFile` requests the <<blockManager, BlockManager>> for the xref:ROOT:BlockManager.adoc#diskBlockManager[DiskBlockManager] that is in turn requested for the <<spark-DiskBlockManager.adoc#getFile, shuffle block data file>> (for a <<spark-BlockDataManager.adoc#ShuffleDataBlockId, ShuffleDataBlockId>>)
