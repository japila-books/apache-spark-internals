...
`IndexShuffleBlockResolver` is <<creating-instance, created>> and managed exclusively by xref:SortShuffleManager.adoc#shuffleBlockResolver[SortShuffleManager] (so it link:spark-shuffle-ShuffleManager.adoc#shuffleBlockResolver[can access shuffle block data]).
