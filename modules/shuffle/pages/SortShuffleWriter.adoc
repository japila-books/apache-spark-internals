...
write requests ExternalSorter to xref:ROOT:spark-ExternalSorter.adoc#writePartitionedFile[write all the records (previously inserted in) into the temporary partitioned file in the disk store]. ExternalSorter returns the length of every partition.
...
In the end, stop requests the ExternalSorter to xref:ROOT:spark-ExternalSorter.adoc#stop[stop] and increments the shuffle write time task metrics.
