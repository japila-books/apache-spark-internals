== [[BypassMergeSortShuffleWriter]] BypassMergeSortShuffleWriter -- ShuffleWriter for BypassMergeSortShuffleHandle

`BypassMergeSortShuffleWriter` is a concrete link:ShuffleWriter.adoc[ShuffleWriter] that xref:scheduler:ShuffleMapTask.adoc[ShuffleMapTask] uses to <<write, write records into one single shuffle block data file>> when the xref:scheduler:ShuffleMapTask.adoc#runTask[task runs for a `ShuffleDependency`].
...
| link:IndexShuffleBlockResolver.adoc[IndexShuffleBlockResolver].
...
| [[mapStatus]] `mapStatus`
| xref:scheduler:MapStatus.adoc[MapStatus] that <<stop, `BypassMergeSortShuffleWriter` returns when stopped>>
...
1. xref:storage:BlockManager.adoc[BlockManager]
2. link:IndexShuffleBlockResolver.adoc[IndexShuffleBlockResolver]
...
NOTE: `write` is part of link:ShuffleWriter.adoc#contract[`ShuffleWriter` contract] to write a sequence of records to a shuffle system.
...
`write` then link:IndexShuffleBlockResolver.adoc#writeIndexFileAndCommit[requests the internal `IndexShuffleBlockResolver` to write shuffle index and data files] (with `dataTmp` as `null`) and sets the internal `mapStatus` (with the address of xref:storage:BlockManager.adoc[BlockManager] in use and <<partitionLengths, partitionLengths>>).

However, when there are records to write, `write` creates a new link:spark-Serializer.adoc[Serializer].

NOTE: link:spark-Serializer.adoc[Serializer] was specified when <<creating-instance, `BypassMergeSortShuffleWriter` was created>> and is exactly the `Serializer` of the xref:rdd:ShuffleDependency.adoc.adoc#serializer[ShuffleDependency].
...
For <<numPartitions, every partition>>, `write` xref:storage:DiskBlockManager.adoc#createTempShuffleBlock[requests `DiskBlockManager` for a temporary shuffle block and its file].

NOTE: `write` uses xref:storage:BlockManager.adoc#diskBlockManager[`BlockManager` to access `DiskBlockManager`]. `BlockManager` was specified when <<creating-instance, `BypassMergeSortShuffleWriter` was created>>.

`write` xref:storage:BlockManager.adoc#getDiskWriter[requests `BlockManager` for a `DiskBlockObjectWriter`] (for the temporary `blockId` and `file`, link:spark-SerializerInstance.adoc[SerializerInstance], `fileBufferSize` and `writeMetrics`).
...
NOTE: `write` uses the xref:rdd:ShuffleDependency.adoc.adoc#partitioner[`Partitioner` from the `ShuffleDependency`] for which <<creating-instance, `BypassMergeSortShuffleWriter` was created>>.
...
`write` link:IndexShuffleBlockResolver.adoc#getDataFile[requests `IndexShuffleBlockResolver` for the shuffle block data file] for `shuffleId` and `mapId`.
...
In the end, `write` link:IndexShuffleBlockResolver.adoc#writeIndexFileAndCommit[requests `IndexShuffleBlockResolver` to write shuffle index and data files] for the `shuffleId` and `mapId` (with `partitionLengths` and the temporary file) and creates a new <<mapStatus, mapStatus>> (with the xref:storage:BlockManager.adoc#shuffleServerId[location of the `BlockManager`] and <<partitionLengths, partitionLengths>>).
