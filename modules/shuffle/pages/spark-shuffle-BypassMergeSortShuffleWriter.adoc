...
`BypassMergeSortShuffleWriter` is a concrete link:ShuffleWriter.adoc[ShuffleWriter] that xref:scheduler:spark-scheduler-ShuffleMapTask.adoc[ShuffleMapTask] uses to <<write, write records into one single shuffle block data file>> when the xref:scheduler:spark-scheduler-ShuffleMapTask.adoc#runTask[task runs for a `ShuffleDependency`].
