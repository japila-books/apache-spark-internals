...
ExternalSorter <<maybeSpillCollection, tries to spill the in-memory collection to disk>> and, xref:shuffle:Spillable.adoc#maybeSpill[if spilled], creates a new collection.
...
ExternalSorter tracks the peak size (in bytes) of the <<in-memory-collection>> whenever requested to <<maybeSpillCollection, maybeSpillCollection>>.
