...
init creates a xref:core:NettyBlockRpcServer.adoc[] (for the xref:ROOT:SparkConf.adoc#getAppId[application id], a JavaSerializer and the given xref:storage:BlockDataManager.adoc[BlockDataManager]) that is used to create a <<transportContext, TransportContext>>.
