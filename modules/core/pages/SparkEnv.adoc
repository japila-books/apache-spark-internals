...
| [[conf]] conf
| xref:ROOT:spark-SparkConf.adoc[SparkConf]
...
| `bindAddress`
| Used to create xref:rpc:index.adoc[RpcEnv] and link:spark-NettyBlockTransferService.adoc#creating-instance[NettyBlockTransferService].
...
environmentDetails is used when SparkContext is requested to xref:ROOT:spark-SparkContext.adoc#postEnvironmentUpdate[post a SparkListenerEnvironmentUpdate event].
