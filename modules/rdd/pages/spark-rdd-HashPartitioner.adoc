...
It is possible to re-shuffle data despite all the records for the key `k` being already on a single Spark executor (i.e. xref:ROOT:BlockManager.adoc[BlockManager] to be precise). When ``HashPartitioner``'s result for `k1` is `3` the key `k1` will go to the third executor.
