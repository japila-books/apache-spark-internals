...
[[storageLevel]]
`RDD` has a xref:storage:StorageLevel.adoc[storage level] that...FIXME
...
The storage level of an RDD is xref:storage:StorageLevel.adoc#NONE[StorageLevel.NONE] by default which is...FIXME
...
. There are no xref:rdd:ShuffleDependency.adoc[ShuffleDependencies] among the xref:index.adoc#dependencies[dependencies] of the RDD
...
When an RDD is link:spark-rdd-caching.adoc[cached], for specified xref:storage:StorageLevel.adoc[storage levels] (i.e. all but `NONE`) link:spark-cachemanager.adoc[`CacheManager` is requested to get or compute partitions].
