...
`ShuffleDependency` is a link:spark-rdd-Dependency.adoc[RDD Dependency] on the output of a xref:scheduler:ShuffleMapStage.adoc[ShuffleMapStage] for a <<rdd, key-value pair RDD>>.
...
1. xref:scheduler:MapOutputTrackerMaster.adoc#getPreferredLocationsForShuffle[`MapOutputTrackerMaster` finds preferred `BlockManagers` with most map outputs] for a `ShuffleDependency`,
...
1. xref:scheduler:MapOutputTracker.adoc#getStatistics[`MapOutputTracker` computes the statistics for a `ShuffleDependency`] (and is the size of the array with the total sizes of shuffle blocks),

2. xref:scheduler:MapOutputTrackerMaster.adoc#getPreferredLocationsForShuffle[`MapOutputTrackerMaster` finds preferred `BlockManagers` with most map outputs] for a `ShuffleDependency`,
