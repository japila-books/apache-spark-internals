...
`compute` requests `ShuffleManager` for a xref:shuffle:ShuffleManager.adoc#getReader[ShuffleReader] to xref:ROOT:spark-shuffle-ShuffleReader.adoc#read[read] the key-value pairs from the xref:spark-rdd-ShuffleDependency.adoc#shuffleHandle[ShuffleHandle] for the input `split` partition.
