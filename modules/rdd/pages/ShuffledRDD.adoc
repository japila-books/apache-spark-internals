...
`ShuffledRDD` is an xref:rdd:index.adoc[RDD] of key-value pairs that represents the *shuffle step* in a link:spark-rdd-lineage.adoc[RDD lineage]. It uses custom <<ShuffledRDDPartition, ShuffledRDDPartition>> partitions.
...
Internally, `compute` makes sure that the input `split` is a link:spark-rdd-ShuffleDependency.adoc[ShuffleDependency]. It then link:spark-shuffle-ShuffleManager.adoc#contract[requests `ShuffleManager` for a `ShuffleReader`] to read key-value pairs (as `Iterator[(K, C)]`) for the `split`.
...
NOTE: `getPreferredLocations` is part of xref:rdd:index.adoc#contract[RDD contract] to specify placement preferences (aka _preferred task locations_), i.e. where tasks should be executed to be as close to the data as possible.
