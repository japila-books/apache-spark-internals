...
If the RDD is marked as persistent the first time, the RDD is xref:core:ContextCleaner.adoc#registerRDDForCleanup[registered to `ContextCleaner`] (if available) and link:spark-SparkContext.adoc#persistRDD[`SparkContext`].
