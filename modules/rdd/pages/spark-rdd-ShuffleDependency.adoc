...
2. xref:ROOT:DAGScheduler.adoc#getOrCreateShuffleMapStage[`DAGScheduler` finds or creates new `ShuffleMapStage` stages] for a `ShuffleDependency`,

3. xref:ROOT:DAGScheduler.adoc#createShuffleMapStage[`DAGScheduler` creates a `ShuffleMapStage`] for a `ShuffleDependency` and a `ActiveJob`,

4. xref:ROOT:DAGScheduler.adoc#getMissingAncestorShuffleDependencies[`DAGScheduler` finds missing ShuffleDependencies for a RDD],

5. xref:ROOT:DAGScheduler.adoc#submitMapStage[`DAGScheduler` submits a `ShuffleDependency` for execution].
====
