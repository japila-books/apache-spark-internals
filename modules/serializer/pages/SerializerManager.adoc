...
When link:spark-SparkEnv.adoc#create[`SparkEnv` is created] (either for the driver or executors), it instantiates `SerializerManager` that is then used to create a xref:storage:BlockManager.adoc[BlockManager].
...
`getSerializer` selects the "best" xref:serializer:Serializer.adoc[Serializer] given the input types for keys and values (in a RDD).
