...
The purpose of the DAGSchedulerEventProcessLoop is to have a separate thread to process events asynchronously alongside xref:ROOT:spark-scheduler-DAGScheduler.adoc[DAGScheduler].
...
JobSubmitted is a DAGSchedulerEvent that is posted when DAGScheduler is requested to xref:ROOT:spark-scheduler-DAGScheduler.adoc#submitJob[submit a job] or xref:ROOT:spark-scheduler-DAGScheduler.adoc#runApproximateJob[run an approximate job].
