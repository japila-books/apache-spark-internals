...
The purpose of the `DAGSchedulerEventProcessLoop` is to have a separate thread to process events asynchronously and serially, i.e. one by one, and let link:spark-scheduler-DAGScheduler.adoc[DAGScheduler] do its work on the main thread.
...
* [[CompletionEvent-task]] <<spark-scheduler-Task.adoc#, Task>>
...
2. A xref:rdd:index.adoc[RDD] (as `finalRDD`)
