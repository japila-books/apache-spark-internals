...
NOTE: `newBroadcast` is executed for link:spark-SparkContext.adoc#broadcast[`SparkContext.broadcast` method] and when xref:scheduler:MapOutputTracker.adoc#serializeMapStatuses[`MapOutputTracker` serializes `MapStatuses`].
...
Refer to xref:io:CompressionCodec.adoc[CompressionCodec].

Used when link:spark-TorrentBroadcast.adoc#creating-instance[`TorrentBroadcast` is created] and later when link:spark-TorrentBroadcast.adoc#writeBlocks[it stores broadcast blocks to `BlockManager`]. Also in xref:serializer:SerializerManager.adoc#settings[SerializerManager].
|===
