...
`getStatuses` finds xref:scheduler:MapStatus.adoc[MapStatuses] for the input `shuffleId` in the <<mapStatuses, mapStatuses>> internal cache and, when not available, fetches them from a remote xref:ROOT:MapOutputTrackerMaster.adoc[MapOutputTrackerMaster] (using RPC).
