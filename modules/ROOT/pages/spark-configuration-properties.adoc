...
*(internal)* Minimum number of partitions (threshold) when `MapStatus` object creates a xref:scheduler:spark-scheduler-MapStatus.adoc#HighlyCompressedMapStatus[HighlyCompressedMapStatus] (over xref:scheduler:spark-scheduler-MapStatus.adoc#CompressedMapStatus[CompressedMapStatus]) when requested for xref:scheduler:spark-scheduler-MapStatus.adoc#apply[one] (for xref:shuffle:ShuffleWriter.adoc[ShuffleWriters]).
