...
NOTE: `run` uses `SparkEnv` xref:core:SparkEnv.adoc#serializer[to access the current `Serializer`]. `SparkEnv` was specified when xref:spark-Executor.adoc#creating-instance[the owning `Executor` was created].
