...
In the end, `register` xref:core:ContextCleaner.adoc#registerAccumulatorForCleanup[registers the accumulator for cleanup] (only when link:spark-SparkContext.adoc#cleaner[`ContextCleaner` is defined in the `SparkContext`]).
