...
NOTE: `containsShuffle` is used exclusively when xref:scheduler:DAGScheduler.adoc#createShuffleMapStage[`DAGScheduler` creates a `ShuffleMapStage`] (for link:spark-rdd-ShuffleDependency.adoc[ShuffleDependency] and link:spark-scheduler-ActiveJob.adoc[ActiveJob]).
...
NOTE: `registerShuffle` is used exclusively when xref:scheduler:DAGScheduler.adoc#createShuffleMapStage[`DAGScheduler` creates a `ShuffleMapStage`] (for link:spark-rdd-ShuffleDependency.adoc[ShuffleDependency] and link:spark-scheduler-ActiveJob.adoc[ActiveJob]).
