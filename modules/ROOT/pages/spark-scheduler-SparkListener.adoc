...
| `SparkContext` link:spark-SparkContext.adoc#unpersistRDD[unpersists an RDD], i.e. removes RDD blocks from `BlockManagerMaster` (that can be triggered link:spark-SparkContext.adoc#unpersist[explicitly] or xref:core:ContextCleaner.adoc#doCleanupRDD[implicitly]).
