...
| `BlockManagerMasterEndpoint` receives a link:spark-blockmanager-BlockManagerMasterEndpoint.adoc#UpdateBlockInfo[UpdateBlockInfo] event (which is when `BlockManager` xref:ROOT:BlockManager.adoc#tryToReportBlockStatus[reports a block status update to driver]).
