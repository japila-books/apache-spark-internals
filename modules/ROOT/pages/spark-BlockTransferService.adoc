...
NOTE: `uploadBlockSync` is used exclusively when `BlockManager` is requested to xref:ROOT:BlockManager.adoc#replicate[replicate] (when a xref:storage:StorageLevel.adoc[replication level is greater than 1]).
