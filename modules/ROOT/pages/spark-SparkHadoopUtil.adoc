...
Internally, `runAsSparkUser` reads the current username (as xref:ROOT:SparkContext.adoc#SPARK_USER[SPARK_USER] environment variable or the short user name from Hadoop's `UserGroupInformation`).
