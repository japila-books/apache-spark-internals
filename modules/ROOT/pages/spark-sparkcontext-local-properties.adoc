...
NOTE: Propagating local properties to workers starts when `SparkContext` is requested to link:spark-SparkContext.adoc#runJob[run] or link:spark-SparkContext.adoc#submitJob[submit] a Spark job that in turn xref:ROOT:DAGScheduler.adoc#runJob[passes them along to `DAGScheduler`].
