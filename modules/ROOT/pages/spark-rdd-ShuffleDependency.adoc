...
`ShuffleDependency` uses the <<rdd, RDD>> to know the number of (map-side/pre-shuffle) partitions and the <<partitioner, `Partitioner`>> for the number of (reduce-side/post-shuffle) partitions.
...
4. link:spark-shuffle-SortShuffleManager.adoc#canUseSerializedShuffle[`SortShuffleManager` checks if `SerializedShuffleHandle` can be used] (for a `ShuffleHandle`).
