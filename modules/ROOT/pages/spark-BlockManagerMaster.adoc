...
image::spark-BlockManagerMaster-RegisterBlockManager.png[align="center"]
...
NOTE: `updateBlockInfo` is used exclusively when `BlockManager` is requested to link:spark-BlockManager.adoc#tryToReportBlockStatus[report a block status update to the driver].
