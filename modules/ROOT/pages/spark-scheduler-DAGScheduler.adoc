...
NOTE: `DAGScheduler` uses link:spark-TaskLocation.adoc[TaskLocations] (with host and executor) while link:spark-BlockManagerMaster.adoc[BlockManagerMaster] uses xref:ROOT:BlockManager.adoc#BlockManagerId[BlockManagerId] (to track similar information, i.e. block locations).
