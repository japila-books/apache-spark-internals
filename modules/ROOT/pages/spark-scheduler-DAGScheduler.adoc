...
DAGScheduler works solely on the driver and is created as part of link:spark-SparkContext.adoc#creating-instance[SparkContext's initialization] (right after xref:scheduler:TaskScheduler.adoc[TaskScheduler] and link:spark-SchedulerBackend.adoc[SchedulerBackend] are ready).
...
NOTE: `handleJobSubmitted` is used when DAGSchedulerEventProcessLoop is requested to xref:ROOT:spark-scheduler-DAGSchedulerEventProcessLoop.adoc#doOnReceive[handle a JobSubmitted event].
