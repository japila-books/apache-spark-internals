...
`TransportConf` is a class for the transport-related network configuration for modules, e.g. xref:deploy:ExternalShuffleService.adoc[ExternalShuffleService] or link:yarn/spark-yarn-YarnShuffleService.adoc[YarnShuffleService].
...
`fromSparkConf` creates a <<TransportConf, TransportConf>> for `module` from the given xref:ROOT:SparkConf.adoc[SparkConf].
