...
The root `cause` of the `FetchFailedException` is usually because the link:spark-Executor.adoc[executor] (with the xref:ROOT:BlockManager.adoc[BlockManager] for the shuffle blocks) is lost (i.e. no longer available) due to:
...
You should review the logs of the Spark application using link:spark-webui.adoc[web UI], xref:spark-history-server:index.adoc[Spark History Server] or cluster-specific tools like https://hadoop.apache.org/docs/stable/hadoop-yarn/hadoop-yarn-site/YarnCommands.html#logs[yarn logs -applicationId] for Hadoop YARN.
