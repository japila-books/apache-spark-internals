...
[[ENDPOINT_NAME]]
`HeartbeatReceiver` is a xref:rpc:index.adoc#ThreadSafeRpcEndpoint[ThreadSafeRpcEndpoint] registered on the driver under the name *HeartbeatReceiver*.
...
| <<Heartbeat, Heartbeat>>
| Posted when `Executor` xref:core:Executor.adoc#reportHeartBeat[informs that it is alive and reports task metrics].
...
NOTE: `Heartbeats` messages are the mechanism of xref:core:Executor.adoc#heartbeats-and-active-task-metrics[executors to inform the Spark application that they are alive and update about the state of active tasks].
