...
It hosts a local xref:ROOT:BlockManager.adoc[Block Manager] that serves blocks to other workers in a Spark cluster. Workers communicate among themselves using their Block Manager instances.
...
Once stages are defined, Spark will generate xref:scheduler:Task.adoc[tasks] from link:spark-scheduler-Stage.adoc[stages]. The first stage will create xref:scheduler:spark-scheduler-ShuffleMapTask.adoc[ShuffleMapTask]s with the last stage creating xref:scheduler:spark-scheduler-ResultTask.adoc[ResultTask]s because in the last stage, one action operation is included to produce results.
