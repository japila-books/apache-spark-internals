...
The fundamental concepts of DAGScheduler are *jobs* and *stages* (refer to xref:scheduler:spark-scheduler-ActiveJob.adoc[Jobs] and xref:ROOT:spark-scheduler-Stage.adoc[Stages] respectively) that it tracks through <<internal-registries, internal registries and counters>>.
...
DAGScheduler uses link:spark-SparkContext.adoc[SparkContext], xref:scheduler:TaskScheduler.adoc[TaskScheduler], link:spark-scheduler-LiveListenerBus.adoc[LiveListenerBus], link:spark-service-mapoutputtracker.adoc[MapOutputTracker] and xref:ROOT:BlockManager.adoc[BlockManager] for its services. However, at the very minimum, DAGScheduler takes a `SparkContext` only (and requests `SparkContext` for the other services).
...
DAGScheduler is given a xref:scheduler:TaskScheduler.adoc[TaskScheduler] to be <<creating-instance, created>>.
...
NOTE: `cleanupStateForJobAndIndependentStages` is used in xref:ROOT:DAGSchedulerEventProcessLoop.adoc#handleTaskCompletion-Success-ResultTask[`handleTaskCompletion` when a `ResultTask` has completed successfully], <<failJobAndIndependentStages, failJobAndIndependentStages>> and <<markMapStageJobAsFinished, markMapStageJobAsFinished>>.
...
NOTE: `markMapStageJobAsFinished` is used in xref:ROOT:DAGSchedulerEventProcessLoop.adoc#handleMapStageSubmitted[handleMapStageSubmitted] and xref:ROOT:DAGSchedulerEventProcessLoop.adoc##handleTaskCompletion[handleTaskCompletion].
...
`submitJob` creates a link:spark-scheduler-JobWaiter.adoc[JobWaiter] and posts a xref:ROOT:DAGSchedulerEventProcessLoop.adoc#JobSubmitted[`JobSubmitted` event].
...
`submitMapStage` creates a link:spark-scheduler-JobWaiter.adoc[JobWaiter] (that it eventually returns) and posts a xref:ROOT:DAGSchedulerEventProcessLoop.adoc#MapStageSubmitted[MapStageSubmitted] event to <<eventProcessLoop, DAGScheduler Event Bus>>).
...
`submitMapStage` announces the map stage submission application-wide (by posting a xref:ROOT:DAGSchedulerEventProcessLoop.adoc#MapStageSubmitted[MapStageSubmitted] to link:spark-scheduler-LiveListenerBus.adoc[LiveListenerBus]).
...
`cancelJobGroup` merely posts a xref:ROOT:DAGSchedulerEventProcessLoop.adoc#StageCancelled[StageCancelled] event to the <<eventProcessLoop, DAGScheduler Event Bus>>.
...
`cancelJobGroup` prints the following INFO message to the logs followed by posting a xref:ROOT:DAGSchedulerEventProcessLoop.adoc#JobGroupCancelled[JobGroupCancelled] event to the <<eventProcessLoop, DAGScheduler Event Bus>>.
...
`cancelAllJobs` merely posts a xref:ROOT:DAGSchedulerEventProcessLoop.adoc#AllJobsCancelled[AllJobsCancelled] event to the <<eventProcessLoop, DAGScheduler Event Bus>>.
...
`taskStarted` merely posts a xref:ROOT:DAGSchedulerEventProcessLoop.adoc#BeginEvent[BeginEvent] event to the <<eventProcessLoop, DAGScheduler Event Bus>>.
...
`taskGettingResult` merely posts a xref:ROOT:DAGSchedulerEventProcessLoop.adoc#GettingResultEvent[GettingResultEvent] event to the <<eventProcessLoop, DAGScheduler Event Bus>>.
...
`taskEnded` simply posts a xref:ROOT:DAGSchedulerEventProcessLoop.adoc#CompletionEvent[CompletionEvent] event to the <<eventProcessLoop, DAGScheduler Event Bus>>.
...
`taskSetFailed` simply posts a xref:ROOT:DAGSchedulerEventProcessLoop.adoc#TaskSetFailed[TaskSetFailed] to <<eventProcessLoop, DAGScheduler Event Bus>>.

NOTE: The input arguments of `taskSetFailed` are exactly the arguments of xref:ROOT:DAGSchedulerEventProcessLoop.adoc#TaskSetFailed[TaskSetFailed].
...
`executorLost` simply posts a xref:ROOT:DAGSchedulerEventProcessLoop.adoc#ExecutorLost[ExecutorLost] event to <<eventProcessLoop, DAGScheduler Event Bus>>.
...
`executorAdded` simply posts a xref:ROOT:DAGSchedulerEventProcessLoop.adoc#ExecutorAdded[ExecutorAdded] event to <<eventProcessLoop, DAGScheduler Event Bus>>.
...
`cancelJob` prints the following INFO message and posts a xref:ROOT:DAGSchedulerEventProcessLoop.adoc#JobCancelled[JobCancelled] to <<eventProcessLoop, DAGScheduler Event Bus>>.
...
NOTE: DAGScheduler clears the cache while xref:ROOT:DAGSchedulerEventProcessLoop.adoc#resubmitFailedStages[resubmitting failed stages], and as a result of xref:ROOT:DAGSchedulerEventProcessLoop.adoc#JobSubmitted[JobSubmitted], xref:ROOT:DAGSchedulerEventProcessLoop.adoc#MapStageSubmitted[MapStageSubmitted], xref:ROOT:DAGSchedulerEventProcessLoop.adoc#CompletionEvent[CompletionEvent], xref:ROOT:DAGSchedulerEventProcessLoop.adoc#ExecutorLost[ExecutorLost] events.
...
NOTE: `failJobAndIndependentStages` is called from xref:ROOT:DAGSchedulerEventProcessLoop.adoc#handleJobCancellation[handleJobCancellation] and `abortStage`.
...
NOTE: `abortStage` is used to xref:ROOT:DAGSchedulerEventProcessLoop.adoc#handleTaskSetFailed[handle `TaskSetFailed` event], when <<submitStage, submitting a stage with no active job>>
...
`eventProcessLoop` is xref:ROOT:DAGSchedulerEventProcessLoop.adoc[DAGScheduler's event bus] to which Spark (by <<submitJob, submitJob>>) posts jobs to schedule their execution. Later on, xref:scheduler:TaskSetManager.adoc[TaskSetManager] talks back to DAGScheduler to inform about the status of the tasks using the same "communication channel".
...
NOTE: `submitWaitingChildStages` is executed when DAGScheduler <<submitMissingTasks, submits missing tasks for stage>> and xref:ROOT:DAGSchedulerEventProcessLoop.adoc#handleTaskCompletion-Success-ShuffleMapTask[handles successful `ShuffleMapTask` completion].
...
NOTE: submitStage is also used to xref:ROOT:DAGSchedulerEventProcessLoop.adoc#resubmitFailedStages[resubmit failed stages].
...
NOTE: `handleJobSubmitted` is used when DAGSchedulerEventProcessLoop is requested to xref:ROOT:DAGSchedulerEventProcessLoop.adoc#doOnReceive[handle a JobSubmitted event].
...
NOTE: `handleTaskCompletion` is used exclusively when `DAGSchedulerEventProcessLoop` is requested to xref:ROOT:DAGSchedulerEventProcessLoop.adoc#CompletionEvent[handle a CompletionEvent].

== [[stage-attempts]] Fault recovery - stage attempts

A single stage can be re-executed in multiple *attempts* due to fault recovery. The number of attempts is configured (FIXME).

If `TaskScheduler` reports that a task failed because a map output file from a previous stage was lost, the DAGScheduler resubmits the lost stage. This is detected through a xref:ROOT:DAGSchedulerEventProcessLoop.adoc#handleTaskCompletion-FetchFailed[`CompletionEvent` with `FetchFailed`], or an <<ExecutorLost, ExecutorLost>> event. DAGScheduler will wait a small amount of time to see whether other nodes or tasks fail, then resubmit `TaskSets` for any lost stage(s) that compute the missing tasks.
...
* `dag-scheduler-message` - a daemon thread pool using `j.u.c.ScheduledThreadPoolExecutor` with core pool size `1`. It is used to post a xref:ROOT:DAGSchedulerEventProcessLoop.adoc#ResubmitFailedStages[ResubmitFailedStages] event when xref:ROOT:DAGSchedulerEventProcessLoop.adoc#handleTaskCompletion-FetchFailed[`FetchFailed` is reported].
...
NOTE: `getMissingParentStages` is used when DAGScheduler <<submitStage, submits missing parent ``ShuffleMapStage``s (of a stage)>> and handles xref:ROOT:DAGSchedulerEventProcessLoop.adoc#handleJobSubmitted[JobSubmitted] and xref:ROOT:DAGSchedulerEventProcessLoop.adoc#handleMapStageSubmitted[MapStageSubmitted] events.
...
| `false`
| When enabled (i.e. `true`), xref:ROOT:DAGSchedulerEventProcessLoop.adoc#handleTaskCompletion-FetchFailed[task failures with `FetchFailed` exceptions] will not cause stage retries, in order to surface the problem. Used for testing.
|===
...
Enable `ALL` logging level for `org.apache.spark.scheduler.DAGSchedule` logger to see what happens inside.
...
| Stages that failed due to fetch failures (when a xref:ROOT:DAGSchedulerEventProcessLoop.adoc#handleTaskCompletion-FetchFailed[task fails with `FetchFailed` exception]).
...
Used when DAGScheduler <<createShuffleMapStage, creates a shuffle map stage>>, <<createResultStage, creates a result stage>>, <<cleanupStateForJobAndIndependentStages, cleans up job state and independent stages>>, is informed that xref:ROOT:DAGSchedulerEventProcessLoop.adoc#handleBeginEvent[a task is started], xref:ROOT:DAGSchedulerEventProcessLoop.adoc#handleTaskSetFailed[a taskset has failed], xref:ROOT:DAGSchedulerEventProcessLoop.adoc#handleJobSubmitted[a job is submitted (to compute a `ResultStage`)], xref:ROOT:DAGSchedulerEventProcessLoop.adoc#handleMapStageSubmitted[a map stage was submitted], xref:ROOT:DAGSchedulerEventProcessLoop.adoc#handleTaskCompletion[a task has completed] or xref:ROOT:DAGSchedulerEventProcessLoop.adoc#handleStageCancellation[a stage was cancelled], <<updateAccumulators, updates accumulators>>, <<abortStage, aborts a stage>> and <<failJobAndIndependentStages, fails a job and independent stages>>.
