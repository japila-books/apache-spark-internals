...
* `Executor` is xref:core:Executor.adoc#maxResultSize[created] (and later for a xref:core:TaskRunner.adoc[])
...
a| [[spark.executor.cores]] Number of cores of an xref:core:Executor.adoc[]

| spark.executor.extraJavaOptions
a| [[spark.executor.extraJavaOptions]] Extra Java options of an xref:core:Executor.adoc[]
...
a| [[spark.executor.heartbeatInterval]] Interval after which an xref:core:Executor.adoc[] reports heartbeat and metrics for active tasks to the driver

Default: `10s`

Refer to xref:core:Executor.adoc#heartbeats-and-active-task-metrics[Sending heartbeats and partial metrics for active tasks]

| spark.executor.heartbeat.maxFailures
a| [[spark.executor.heartbeat.maxFailures]] Number of times an xref:core:Executor.adoc[] will try to send heartbeats to the driver before it gives up and exits (with exit code `56`).
...
a| [[spark.executor.instances]] Number of xref:core:Executor.adoc[] in use
...
a| [[spark.executor.memory]] Amount of memory to use for an xref:core:Executor.adoc[]

Default: `1g`

Equivalent to xref:ROOT:SparkContext.adoc#environment-variables[SPARK_EXECUTOR_MEMORY] environment variable.

Refer to xref:core:Executor.adoc#memory[Executor Memory -- spark.executor.memory or SPARK_EXECUTOR_MEMORY settings]
