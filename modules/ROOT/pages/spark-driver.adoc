...
Driver requires the additional services (beside the common ones like xref:shuffle:ShuffleManager.adoc[ShuffleManager], xref:memory:MemoryManager.adoc[MemoryManager], link:spark-BlockTransferService.adoc[BlockTransferService], xref:core:BroadcastManager.adoc[BroadcastManager], link:spark-cachemanager.adoc[CacheManager]):
