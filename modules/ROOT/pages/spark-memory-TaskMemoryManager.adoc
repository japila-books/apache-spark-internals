...
`TaskMemoryManager` manages the memory allocated to an link:spark-scheduler-Task.adoc[individual task].
...
image::spark-TaskMemoryManager.png[align="center"]
...
| `consumers`
| [[consumers]] link:spark-memory-MemoryConsumer.adoc[MemoryConsumers]
...
NOTE: link:spark-memory-MemoryConsumer.adoc[MemoryConsumer] knows its mode -- on- or off-heap.
...
When the memory obtained is less than requested (by `required`), `acquireExecutionMemory` requests all <<consumers, consumers>> to link:spark-memory-MemoryConsumer.adoc#spill[release memory (by spilling it to disk)].
...
If the earlier `spill` on the consumers did not work out and there is still memory to be acquired, `acquireExecutionMemory` link:spark-memory-MemoryConsumer.adoc#spill[requests the input `consumer` to spill memory to disk] (that in fact requested more memory!)
...
NOTE: `acquireExecutionMemory` is called when a link:spark-memory-MemoryConsumer.adoc#acquireMemory[`MemoryConsumer` tries to acquires a memory] and <<allocatePage, allocatePage>>.
...
NOTE: It only handles *Tungsten Consumers*, i.e. link:spark-memory-MemoryConsumer.adoc[MemoryConsumers] in  `tungstenMemoryMode` mode.
...
* `MemoryConsumer` is requested to link:spark-memory-MemoryConsumer.adoc#freeMemory[freeMemory]
...
NOTE: `showMemoryUsage` is used exclusively when `MemoryConsumer` is requested to link:spark-memory-MemoryConsumer.adoc#throwOom[throwOom].
...
NOTE: `pageSizeBytes` is used when `MemoryConsumer` is requested to link:spark-memory-MemoryConsumer.adoc#freePage[freePage] and link:spark-memory-MemoryConsumer.adoc#throwOom[throwOom].
