...
`YarnScheduler` is the link:../spark-scheduler-TaskScheduler.adoc[TaskScheduler] for link:README.adoc[Spark on YARN] in link:spark-submit.adoc#deploy-mode[client deploy mode].

It is a custom link:spark-scheduler-TaskSchedulerImpl.adoc[TaskSchedulerImpl] with ability to compute racks per hosts, i.e. it comes with a specialized <<getRackForHost, getRackForHost>>.
...
NOTE: `getRackForHost` overrides the link:spark-scheduler-TaskSchedulerImpl.adoc#getRackForHost[parent TaskSchedulerImpl's getRackForHost]
