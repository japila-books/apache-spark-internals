...
* `Executor` link:spark-Executor.adoc#creating-instance[is created] (in non-local mode and xref:storage:BlockManager.adoc#initialize[requests `BlockManager` to initialize]).
====
...
| xref:scheduler:TaskScheduler.adoc#applicationId[TaskScheduler.applicationId()]
