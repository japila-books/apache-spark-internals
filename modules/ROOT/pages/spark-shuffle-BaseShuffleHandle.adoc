...
`BaseShuffleHandle` is a `ShuffleHandle` that is created solely to capture the parameters when link:spark-shuffle-SortShuffleManager.adoc#registerShuffle[`SortShuffleManager` is requested for a `ShuffleHandle`] (for a `ShuffleDependency`):

1. [[shuffleId]] `shuffleId`
2. [[numMaps]] `numMaps`
3. [[dependency]] link:spark-rdd-ShuffleDependency.adoc[ShuffleDependency]

NOTE: `BaseShuffleHandle` is the last possible choice when link:spark-shuffle-SortShuffleManager.adoc#registerShuffle[`SortShuffleManager` is requested for a `ShuffleHandle`] (after link:spark-shuffle-BypassMergeSortShuffleHandle.adoc[BypassMergeSortShuffleHandle] and link:spark-shuffle-SerializedShuffleHandle.adoc[SerializedShuffleHandle] have already been considered and failed the check).
