...
*Dynamic Allocation (of Executors)* (aka _Elastic Scaling_) is a Spark feature that allows for adding or removing xref:core:Executor.adoc[Spark executors] dynamically to match the workload.
...
`getDynamicAllocationInitialExecutors` makes sure that xref:core:Executor.adoc#spark.executor.instances[spark.executor.instances] is greater than <<spark.dynamicAllocation.minExecutors, spark.dynamicAllocation.minExecutors>>.

NOTE: Both xref:core:Executor.adoc#spark.executor.instances[spark.executor.instances] and <<spark.dynamicAllocation.minExecutors, spark.dynamicAllocation.minExecutors>> fall back to `0` when no defined explicitly.
...
* xref:core:Executor.adoc#spark.executor.instances[spark.executor.instances]
* `0`
...
NOTE: xref:core:Executor.adoc#spark.executor.instances[spark.executor.instances] setting can be set using link:spark-submit.adoc#command-line-options[`--num-executors` command-line option] of link:spark-submit.adoc[spark-submit].
