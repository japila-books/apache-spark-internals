...
MapOutputTrackerMaster defines 1000 tasks as the threshold of the number of map and reduce tasks to <<getPreferredLocationsForShuffle, compute preferred locations>> with <<shuffleLocalityEnabled, spark.shuffle.reduceLocality.enabled>> as computing the top locations is expensive.
