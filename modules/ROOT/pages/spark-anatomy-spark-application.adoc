...
Every Spark application starts from creating xref:ROOT:SparkContext.adoc[SparkContext].

NOTE: Without xref:ROOT:SparkContext.adoc[SparkContext] no computation (as a Spark job) can be started.
...
A Spark application is uniquely identified by a pair of the xref:ROOT:SparkContext.adoc#applicationId[application] and xref:ROOT:SparkContext.adoc#applicationAttemptId[application attempt] ids.

For it to work, you have to xref:ROOT:SparkConf.adoc[create a Spark configuration using SparkConf] or use a xref:ROOT:SparkContext.adoc#creating-instance[custom SparkContext constructor].
...
After all the data processing is completed, the Spark application finishes by xref:ROOT:SparkContext.adoc#stopping[stopping the Spark context].
