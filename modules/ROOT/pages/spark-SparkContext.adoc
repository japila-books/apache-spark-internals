...
* Accessing services, e.g. <<statusStore, AppStatusStore>>, <<taskScheduler, TaskScheduler>>, link:spark-scheduler-LiveListenerBus.adoc[LiveListenerBus], link:spark-BlockManager.adoc[BlockManager], link:spark-SchedulerBackend.adoc[SchedulerBackends], link:spark-shuffle-ShuffleManager.adoc[ShuffleManager] and the <<cleaner, optional ContextCleaner>>.
...
[[nextShuffleId]]
* shuffle ids using `nextShuffleId` internal counter for link:spark-scheduler-ShuffleMapStage.adoc[registering shuffle dependencies] to link:spark-shuffle-ShuffleManager.adoc[Shuffle Service].
