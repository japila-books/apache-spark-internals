...
`SparkContext` (aka *Spark context*) is the entry point to all services of Apache Spark (execution engine) and so the heart of a Spark application. In fact, you can consider an application a Spark application only when it uses a `SparkContext` (directly or indirectly).
...
* Accessing services, e.g. <<statusStore, AppStatusStore>>, <<taskScheduler, TaskScheduler>>, link:spark-scheduler-LiveListenerBus.adoc[LiveListenerBus], xref:ROOT:BlockManager.adoc[BlockManager], xref:scheduler:SchedulerBackend.adoc[SchedulerBackends], xref:shuffle:ShuffleManager.adoc[ShuffleManager] and the <<cleaner, optional ContextCleaner>>.
...
NOTE: `unpersistRDD` uses `SparkEnv` link:spark-SparkEnv.adoc#blockManager[to access the current `BlockManager`] that is in turn used to xref:ROOT:BlockManager.adoc#master[access the current `BlockManagerMaster`].
...
`cancelJob` requests `DAGScheduler` xref:ROOT:DAGScheduler.adoc#cancelJob[to cancel a Spark job].
...
`cancelStage` simply requests `DAGScheduler` xref:ROOT:DAGScheduler.adoc#cancelJob[to cancel a Spark stage] (with an optional `reason`).
...
`getExecutorStorageStatus` xref:storage:BlockManagerMaster.adoc#getStorageStatus[requests `BlockManagerMaster` for storage status] (of all xref:ROOT:BlockManager.adoc[BlockManagers]).
...
`submitJob` submits a job in an asynchronous, non-blocking way to xref:ROOT:DAGScheduler.adoc#submitJob[DAGScheduler].
...
`unpersist` removes an RDD from the master's xref:ROOT:BlockManager.adoc[Block Manager] (calls `removeRdd(rddId: Int, blocking: Boolean)`) and the internal <<persistentRdds, persistentRdds>> mapping.
...
[[nextShuffleId]]
* shuffle ids using `nextShuffleId` internal counter for link:spark-scheduler-ShuffleMapStage.adoc[registering shuffle dependencies] to xref:shuffle:ShuffleManager.adoc[Shuffle Service].
...
`runJob` requests  xref:ROOT:DAGScheduler.adoc#runJob[`DAGScheduler` to run a job].

TIP: `runJob` just prepares input parameters for xref:ROOT:DAGScheduler.adoc#runJob[`DAGScheduler` to run a job].
...
9. Requests xref:ROOT:DAGScheduler.adoc#stop[`DAGScheduler` to stop]
...
`submitMapStage` xref:ROOT:DAGScheduler.adoc#submitMapStage[submits the input `ShuffleDependency` to `DAGScheduler` for execution] and returns a `SimpleFutureAction`.
...
NOTE: `submitMapStage` _seems_ related to xref:ROOT:DAGScheduler.adoc#adaptive-query-planning[Adaptive Query Planning / Adaptive Scheduling].
...
`cancelJobGroup` requests `DAGScheduler` xref:ROOT:DAGScheduler.adoc#cancelJobGroup[to cancel a group of active Spark jobs].
...
`getPreferredLocs` simply xref:ROOT:DAGScheduler.adoc#getPreferredLocs[requests `DAGScheduler` for the preferred locations for `partition`].
...
NOTE: `maxNumConcurrentTasks` is used exclusively when `DAGScheduler` is requested to xref:ROOT:DAGScheduler.adoc#checkBarrierStageWithNumSlots[checkBarrierStageWithNumSlots].
