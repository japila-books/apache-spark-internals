...
Internally, broadcast requests BroadcastManager for a xref:ROOT:spark-service-broadcastmanager.adoc#newBroadcast[new broadcast variable].

NOTE: The current `BroadcastManager` is available using xref:core:SparkEnv.adoc#broadcastManager[`SparkEnv.broadcastManager`] attribute and is always link:spark-service-broadcastmanager.adoc[BroadcastManager] (with few internal configuration changes to reflect where it runs, i.e. inside the driver or executors).
