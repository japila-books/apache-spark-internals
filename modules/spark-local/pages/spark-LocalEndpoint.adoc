...
| <<KillTask, KillTask>>
| Requests the <<executor, executor>> to xref:core:Executor.adoc#killTask[kill a given task]
...
| Requests the <<executor, executor>> to xref:core:Executor.adoc#stop[stop]
...
[[executor]]
`LocalEndpoint` creates a single xref:core:Executor.adoc[] with the following properties:

* [[localExecutorId]] *driver* ID for the xref:core:Executor.adoc#executorId[executor ID]

* [[localExecutorHostname]] *localhost* for the xref:core:Executor.adoc#executorHostname[hostname]

* <<userClassPath, User-defined CLASSPATH>> for the xref:core:Executor.adoc#userClassPath[user-defined CLASSPATH]

* xref:core:Executor.adoc#isLocal[isLocal] flag enabled
...
When <<receive, received>>, `LocalEndpoint` requests the single <<executor, Executor>> to xref:core:Executor.adoc#killTask[kill a task] (given the `taskId`, the `interruptThread` flag and the reason).
...
When <<receiveAndReply, received>>, `LocalEndpoint` requests the single <<executor, Executor>> to xref:core:Executor.adoc#stop[stop] and requests the given `RpcCallContext` to `reply` with `true` (as the response).
