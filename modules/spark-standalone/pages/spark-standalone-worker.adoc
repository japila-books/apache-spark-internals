...
You can have one or many standalone workers in a standalone cluster. They can be started and stopped using link:spark-standalone-worker-scripts.md[management scripts].
...
NOTE: `receive` is part of xref:rpc:RpcEndpoint.md#receive[RpcEndpoint Contract] to process messages.
...
NOTE: `onStart` is part of xref:rpc:RpcEndpoint.md#onStart[RpcEndpoint Contract] to activate an endpoint and start accepting messages.
