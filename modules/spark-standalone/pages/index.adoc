...
`SparkDeploySchedulerBackend` is the xref:scheduler:SchedulerBackend.adoc[Scheduler Backend] for Spark Standalone, i.e. it is used when you link:spark-SparkContext.adoc#creating-instance[create a SparkContext] using `spark://` link:spark-deployment-environments.adoc#master-urls[master URL].
