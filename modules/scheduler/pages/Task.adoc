...
| [[ShuffleMapTask]] Computes a xref:scheduler:spark-scheduler-ShuffleMapStage.adoc[ShuffleMapStage]
...
Set when `TaskSetManager` is xref:scheduler:TaskSetManager.adoc[created] (to be the <<spark-service-MapOutputTrackerMaster.adoc#getEpoch, epoch>> of the `MapOutputTrackerMaster`)
