...
| [[epoch]] `epoch`
| Current xref:ROOT:MapOutputTracker.adoc#getEpoch[map output tracker epoch].
...
`TaskSetManager` xref:ROOT:MapOutputTracker.adoc#getEpoch[requests the current epoch from `MapOutputTracker`] and sets it on all tasks in the taskset.
