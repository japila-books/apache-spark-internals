...
<<runTask, runTask>> expects that the <<taskBinary, serialized task binary>> should be a tuple of a xref:rdd:RDD.adoc[RDD] and a xref:rdd:ShuffleDependency.adoc[ShuffleDependency]
====
...
Internally, `runTask` requests the <<spark-SparkEnv.adoc#, SparkEnv>> for the new instance of <<spark-SparkEnv.adoc#closureSerializer, closure serializer>> and requests it to xref:serializer:Serializer.adoc#deserialize[deserialize] the <<taskBinary, taskBinary>> (into a tuple of a xref:rdd:RDD.adoc[RDD] and a xref:rdd:ShuffleDependency.adoc[ShuffleDependency]).
