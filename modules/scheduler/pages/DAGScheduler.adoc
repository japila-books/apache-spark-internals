...
submitMissingTasks requests the given xref:scheduler:spark-scheduler-Stage.adoc[Stage] for the xref:scheduler:spark-scheduler-Stage.adoc#findMissingPartitions[missing partitions] (partitions that need to be computed).

submitMissingTasks adds the stage to the <<runningStages, runningStages>> internal registry.

submitMissingTasks notifies the <<outputCommitCoordinator, OutputCommitCoordinator>> that xref:ROOT:spark-service-outputcommitcoordinator.adoc#stageStart[stage execution started].
...
submitMissingTasks requests the stage for a xref:scheduler:spark-scheduler-Stage.adoc#makeNewStageAttempt[new stage attempt].
