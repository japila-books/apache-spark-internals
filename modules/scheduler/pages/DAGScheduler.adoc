...
== [[submitMapStage]] Submitting ShuffleDependency for Execution
...
submitMapStage is used when SparkContext is requested to xref:ROOT:spark-SparkContext.adoc#submitMapStage[submit a MapStage for execution].

== [[cancelStage]] Relaying Stage Cancellation From SparkContext
...
cancelStage merely posts a xref:scheduler:DAGSchedulerEventProcessLoop.adoc#StageCancelled[StageCancelled] event to the <<eventProcessLoop, DAGScheduler Event Bus>>.

cancelStage is used when SparkContext is requested to xref:ROOT:spark-SparkContext.adoc#cancelStage[cancel a stage].

== [[cancelJobGroup]] Relaying Job Group Cancellation From SparkContext
...
```
Asked to cancel job group [groupId]
```

cancelJobGroup is used when SparkContext is requested to xref:ROOT:spark-SparkContext.adoc#cancelJobGroup[cancel a job group].

== [[cancelAllJobs]] Relaying All Jobs Cancellation From SparkContext
...
cancelAllJobs is used when SparkContext is requested to xref:ROOT:spark-SparkContext.adoc#cancelAllJobs[cancel all running or scheduled Spark jobs].

== [[taskStarted]] Relaying Task Started From TaskSetManager
...
taskStarted is used when TaskSetManager is requested to xref:scheduler:TaskSetManager.adoc#resourceOffer[start a task].

== [[taskGettingResult]] Relaying Task Fetching/Getting Result From TaskSetManager
...
taskGettingResult is used when TaskSetManager is requested to xref:scheduler:TaskSetManager.adoc#handleTaskGettingResult[handle a task fetching result].

== [[taskEnded]] Relaying Task End From TaskSetManager
...
taskEnded is used when TaskSetManager is requested to xref:scheduler:TaskSetManager.adoc#handleSuccessfulTask[handleSuccessfulTask], xref:scheduler:TaskSetManager.adoc#handleFailedTask[handleFailedTask], and xref:scheduler:TaskSetManager.adoc#executorLost[executorLost].

== [[taskSetFailed]] Relaying TaskSet Failed From TaskSetManager
...
taskSetFailed is used when TaskSetManager is requested to xref:scheduler:TaskSetManager.adoc#abort[abort].

== [[executorLost]] Relaying Executor Lost From TaskSchedulerImpl
...
== [[executorAdded]] Relaying Executor Added From TaskSchedulerImpl
...
executorAdded is used when TaskSchedulerImpl is requested to xref:scheduler:TaskSchedulerImpl.adoc#resourceOffers[handle resource offers] (and a new executor is found in the resource offers).

== [[cancelJob]] Relaying Job Cancellation From SparkContext or JobWaiter
...
cancelJob is used when xref:ROOT:spark-SparkContext.adoc#cancelJob[SparkContext] or xref:scheduler:spark-scheduler-JobWaiter.adoc[JobWaiter] are requested to cancel a Spark job.
...
handleJobSubmitted is used when DAGSchedulerEventProcessLoop is requested to xref:scheduler:DAGSchedulerEventProcessLoop.adoc#JobSubmitted[handle a JobSubmitted event].
...
resubmitFailedStages is used when DAGSchedulerEventProcessLoop is requested to xref:scheduler:DAGSchedulerEventProcessLoop.adoc#ResubmitFailedStages[handle a ResubmitFailedStages event].

== [[handleMapStageSubmitted]] Handling MapStageSubmitted Event
...
handleMapStageSubmitted is used when DAGSchedulerEventProcessLoop is requested to xref:scheduler:DAGSchedulerEventProcessLoop.adoc#MapStageSubmitted[handle a MapStageSubmitted event].
...
handleTaskCompletion is used when DAGSchedulerEventProcessLoop is requested to xref:scheduler:DAGSchedulerEventProcessLoop.adoc#CompletionEvent[handle a CompletionEvent event].
...
== [[workerRemoved]] Handling Worker Removed Event
...
workerRemoved simply requests the <<eventProcessLoop, DAGSchedulerEventProcessLoop>> to post a xref:scheduler:DAGSchedulerEventProcessLoop.adoc#WorkerRemoved[WorkerRemoved] message.

workerRemoved is used when TaskSchedulerImpl is requested to xref:scheduler:TaskSchedulerImpl.adoc#workerRemoved[handle a removed worker event].
...
runApproximateJob is used when SparkContext is requested to xref:ROOT:spark-SparkContext.adoc#runApproximateJob[run an approximate job].
...
== [[handleWorkerRemoved]] Handling WorkerRemoved Event
...
handleWorkerRemoved is used when DAGSchedulerEventProcessLoop is requested to xref:scheduler:DAGSchedulerEventProcessLoop.adoc#WorkerRemoved[WorkerRemoved] event.
