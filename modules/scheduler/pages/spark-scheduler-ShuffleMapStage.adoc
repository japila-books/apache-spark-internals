...
`ShuffleMapStage` (aka *shuffle map stage* or simply *map stage*) is an xref:scheduler:spark-scheduler-Stage.adoc[intermediate stage] in the *physical execution DAG* that corresponds to a xref:rdd:spark-rdd-ShuffleDependency.adoc[ShuffleDependency].
...
4. `parents` -- the collection of parent xref:scheduler:spark-scheduler-Stage.adoc[Stages]
...
NOTE: `findMissingPartitions` is part of xref:scheduler:spark-scheduler-Stage.adoc#contract[`Stage` contract] that returns the partitions that are missing, i.e. are yet to be computed.
...
In both cases, `outputLocInMapOutputTrackerFormat` is used to xref:ROOT:spark-service-MapOutputTrackerMaster.adoc#registerMapOutputs[register the shuffle map outputs (of the `ShuffleDependency`) with `MapOutputTrackerMaster`].
====
...
The size of `outputLocs` is exactly the number of partitions of the xref:scheduler:spark-scheduler-Stage.adoc#rdd[RDD the stage runs on].
