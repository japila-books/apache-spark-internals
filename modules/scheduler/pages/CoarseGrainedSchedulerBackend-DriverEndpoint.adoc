...
`DriverEndpoint` is a xref:rpc:index.adoc#ThreadSafeRpcEndpoint[ThreadSafeRpcEndpoint] that acts as a <<messages, message handler>> for xref:scheduler:CoarseGrainedSchedulerBackend.adoc[CoarseGrainedSchedulerBackend] to communicate with link:spark-CoarseGrainedExecutorBackend.adoc[CoarseGrainedExecutorBackend].
