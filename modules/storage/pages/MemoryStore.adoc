...
* `Task` is requested to xref:ROOT:spark-scheduler-Task.adoc#run[run] (and cleans up after itself)
...
`putBytes` requests xref:ROOT:spark-MemoryManager.adoc#acquireStorageMemory[storage memory  for `blockId` from `MemoryManager`] and registers the block in <<entries, entries>> internal registry.
...
`putBytes` then requests xref:ROOT:spark-MemoryManager.adoc#acquireStorageMemory[`size` memory for the `blockId` block in a given `memoryMode` from the current `MemoryManager`].
...
NOTE: `evictBlocksToFreeSpace` is used when `StorageMemoryPool` is requested to xref:ROOT:spark-StorageMemoryPool.adoc#acquireMemory[acquireMemory] and xref:ROOT:spark-StorageMemoryPool.adoc#freeSpaceToShrinkPool[freeSpaceToShrinkPool].
...
`reserveUnrollMemoryForThisTask` acquires a lock on <<memoryManager, MemoryManager>> and requests it to xref:ROOT:spark-MemoryManager.adoc#acquireUnrollMemory[acquireUnrollMemory].
...
`maxMemory` requests the <<memoryManager, MemoryManager>> for the current xref:ROOT:spark-MemoryManager.adoc#maxOnHeapStorageMemory[maxOnHeapStorageMemory] and xref:ROOT:spark-MemoryManager.adoc#maxOffHeapStorageMemory[maxOffHeapStorageMemory], and simply returns their sum.
