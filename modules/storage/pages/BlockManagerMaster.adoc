...
NOTE: `BlockManagerMaster` is created in link:spark-SparkEnv.adoc#BlockManagerMaster[`SparkEnv` (for the driver and executors)], and immediately used to create their xref:storage:BlockManager.adoc[BlockManagers].
...
`registerBlockManager` then notifies the driver that the `blockManagerId` xref:storage:BlockManager.adoc[BlockManager] tries to register. `registerBlockManager` posts a link:spark-blockmanager-BlockManagerMasterEndpoint.adoc#RegisterBlockManager[blocking `RegisterBlockManager` message to BlockManagerMaster RPC endpoint].
