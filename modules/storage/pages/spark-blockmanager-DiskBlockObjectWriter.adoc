...
NOTE: `write` is used when xref:shuffle:BypassMergeSortShuffleWriter.adoc#write[`BypassMergeSortShuffleWriter` writes records] and in `ExternalAppendOnlyMap`, `ExternalSorter` and `WritablePartitionedPairCollection`.
