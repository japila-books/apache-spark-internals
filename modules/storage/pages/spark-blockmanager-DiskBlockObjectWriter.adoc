...
`DiskBlockObjectWriter` is a https://docs.oracle.com/javase/8/docs/api/java/io/OutputStream.html[java.io.OutputStream] that xref:storage:BlockManager.adoc#getDiskWriter[`BlockManager` offers for writing blocks to disk].
...
NOTE: `write` is used when xref:shuffle:BypassMergeSortShuffleWriter.adoc#write[`BypassMergeSortShuffleWriter` writes records] and in xref:shuffle:ExternalAppendOnlyMap.adoc[ExternalAppendOnlyMap], xref:shuffle:ExternalSorter.adoc[ExternalSorter] and WritablePartitionedPairCollection.
