...
If xref:ROOT:MapOutputTracker.adoc[MapOutputTracker] was given (when the RPC endpoint was created), it calls xref:ROOT:MapOutputTracker.adoc#unregisterShuffle[MapOutputTracker to unregister the `shuffleId` shuffle].
...
NOTE: `RemoveShuffle` is posted when xref:BlockManagerMaster.adoc#removeShuffle[BlockManagerMaster] and link:spark-blockmanager-BlockManagerMasterEndpoint.adoc#removeShuffle[BlockManagerMasterEndpoint] remove all blocks for a shuffle.
...
When received a `GetMatchingBlockIds`, `BlockManagerSlaveEndpoint` requests <<blockManager, BlockManager>> to xref:ROOT:BlockManager.adoc#getMatchingBlockIds[find IDs of existing blocks for a given filter] and sends them back.
