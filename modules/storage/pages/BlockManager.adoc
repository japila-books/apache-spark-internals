...
BlockManager creates a xref:spark-MemoryStore.adoc[MemoryStore] and xref:spark-DiskStore.adoc[DiskStore].
...
Inside the function, if the xref:rdd:spark-rdd-StorageLevel.adoc[storage `level`]'s replication is greater than 1, it immediately starts <<replicate, replication>> of the `blockId` block on a separate thread (from `futureExecutionContext` thread pool). The replication uses the input `bytes` and `level` storage level.
