...
BlockManager runs on every node in a Spark application, i.e. the xref:ROOT:spark-driver.adoc[driver] and xref:core:Executor.adoc[executors].
...
* `TaskRunner` is requested to xref:core:TaskRunner.adoc#run-result-sent-via-blockmanager[run] (and the result size is above xref:core:Executor.adoc#maxDirectResultSize[maxDirectResultSize])
...
initialize initializes a BlockManager on the driver and executors (see xref:ROOT:SparkContext.adoc#creating-instance[Creating SparkContext Instance] and xref:core:Executor.adoc#creating-instance[Creating Executor Instance], respectively).
...
NOTE: initialize is called when the link:spark-SparkContext-creating-instance-internals.adoc#BlockManager-initialization[driver is launched (and `SparkContext` is created)] and when an xref:core:Executor.adoc#creating-instance[`Executor` is created] (for link:spark-CoarseGrainedExecutorBackend.adoc#RegisteredExecutor[CoarseGrainedExecutorBackend] and link:spark-executor-backends-MesosExecutorBackend.adoc[MesosExecutorBackend]).
...
NOTE: `reregister` is called when a xref:core:Executor.adoc#heartbeats-and-active-task-metrics[`Executor` was informed to re-register while sending heartbeats].
...
NOTE: `shuffleMetricsSource` is used exclusively when `Executor` is xref:core:Executor.adoc#creating-instance[created] (for non-local / cluster modes).
