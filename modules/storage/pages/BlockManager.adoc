...
When <<creating-instance, created>>, BlockManager creates a xref:spark-MemoryStore.adoc[MemoryStore] (with the <<blockInfoManager, BlockInfoManager>>, the <<serializerManager, SerializerManager>>, the <<memoryManager, MemoryManager>> and itself as a xref:spark-BlockEvictionHandler.adoc[BlockEvictionHandler]).
