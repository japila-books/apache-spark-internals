...
* [[shuffleClient]] xref:core:ShuffleClient.adoc[ShuffleClient]
...
NOTE: The input `FetchRequest` contains the remote xref:storage:BlockManager.adoc#BlockManagerId[BlockManagerId] address and the shuffle blocks to fetch (as a sequence of xref:storage:BlockId.adoc[] and their sizes).

`sendRequest` xref:core:ShuffleClient.adoc#fetchBlocks[requests `ShuffleClient` to fetch shuffle blocks] (from the host, the port, and the executor as defined in the input `FetchRequest`).
