...
DiskBlockObjectWriter manages a xref:ROOT:SerializationStream.adoc[SerializationStream] for <<write, writing a key-value record>>:
...
write requests the <<objOut, SerializationStream>> to xref:ROOT:SerializationStream.adoc#writeKey[write the key] and then the xref:ROOT:SerializationStream.adoc#writeValue[value].
