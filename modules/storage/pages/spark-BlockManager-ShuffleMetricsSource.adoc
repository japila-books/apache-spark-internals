...
`ShuffleMetricsSource` lives on a Spark executor and is xref:core:Executor.adoc#creating-instance-BlockManager-shuffleMetricsSource[registered only when a Spark application runs in a non-local / cluster mode].
...
When <<creating-instance, created>>, `ShuffleMetricsSource` gets a <<metricSet, MetricSet>> that `BlockManager` requests from the xref:core:ShuffleClient.adoc#shuffleMetrics[ShuffleClient] (only xref:core:Executor.adoc#creating-instance-BlockManager-shuffleMetricsSource[when in a non-local / cluster mode]).
