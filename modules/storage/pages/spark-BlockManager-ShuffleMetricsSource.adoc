...
When <<creating-instance, created>>, `ShuffleMetricsSource` gets a <<metricSet, MetricSet>> that `BlockManager` requests from the xref:core:ShuffleClient.adoc#shuffleMetrics[ShuffleClient] (only xref:executor:Executor.adoc#creating-instance-BlockManager-shuffleMetricsSource[when in a non-local / cluster mode]).
