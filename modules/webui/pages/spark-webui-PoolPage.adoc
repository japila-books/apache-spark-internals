...
The Fair Scheduler Pool Details page shows information about a link:spark-scheduler-Pool.adoc[`Schedulable` pool] and is only available when a Spark application uses the link:spark-scheduler-SchedulingMode.adoc#FAIR[FAIR scheduling mode] (which is controlled by <<spark-configuration-properties.adoc#spark.scheduler.mode, spark.scheduler.mode>> configuration property).
