...
* Live link:spark-SparkContext-creating-instance-internals.adoc#_statusStore[AppStatusStore] (with a link:spark-core-ElementTrackingStore.adoc[ElementTrackingStore] using an link:spark-core-InMemoryStore.adoc[InMemoryStore] and a xref:core:AppStatusListener.adoc[] for a live Spark application)
