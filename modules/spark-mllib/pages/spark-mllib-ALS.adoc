...
`ALS` is an link:spark-mllib-estimators.md[Estimator] that <<fit, generates a ALSModel>>.
...
| xref:storage:StorageLevel.md#MEMORY_AND_DISK[MEMORY_AND_DISK]
| xref:storage:StorageLevel.md[StorageLevel] for ALS model factors
...
| xref:storage:StorageLevel.md#MEMORY_AND_DISK[MEMORY_AND_DISK]
| xref:storage:StorageLevel.md[StorageLevel] for intermediate datasets. Must not be `NONE`.

| [[itemCol]] `itemCol`
| `item`
| Column name for item ids

Must be all integers or link:../spark-sql-DataType.md[numerics] within the integer value range
...
NOTE: `fit` converts the `dataset` to `RDD` using link:../spark-sql-dataset-operators.md#rdd[rdd] operator.
...
`fit` link:spark-mllib-ALSModel.md#creating-instance[creates] a `ALSModel`.
...
`train` <<partitionRatings, partition the ratings RDD>> (using two xref:rdd:HashPartitioner.md[HashPartitioners] with <<numUserBlocks, numUserBlocks>> and <<numItemBlocks, numItemBlocks>> partitions) and immediately link:../spark-rdd-caching.md#persist[persists] the RDD per `intermediateRDDStorageLevel` storage level.
