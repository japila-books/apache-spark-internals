...
It is an `Estimator` that produces a link:spark-mllib-models.md#KMeansModel[KMeansModel].
...
`KMeans` assumes that `featuresCol` is of type link:spark-mllib-vector.md[VectorUDT] and appends `predictionCol` of type `IntegerType`.
