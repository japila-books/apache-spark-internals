...
`Estimator` is a link:spark-mllib-PipelineStage.md[PipelineStage] and so it can be a part of a link:spark-mllib-Pipeline.md[Pipeline].

NOTE: `Pipeline` considers `Estimator` special and executes `fit` method before `transform` (as for other `Transformer` objects in a pipeline). Consult link:spark-mllib-Pipeline.md[Pipeline] document.
