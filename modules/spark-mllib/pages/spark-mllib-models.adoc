...
`Model` abstract class is a link:spark-mllib-transformers.md[Transformer] with the optional link:spark-mllib-estimators.md[Estimator] that has produced it (as a transient `parent` field).

```
model: DataFrame =[predict]=> DataFrame (with predictions)
```

NOTE: An `Estimator` is optional and is available only after `fit` (of an link:spark-mllib-estimators.md[Estimator]) has been executed whose result a model is.
...
`PipelineModel` is a `Model` of link:spark-mllib-Pipeline.md[Pipeline] estimator.

Once fit, you can use the result model as any other models to transform datasets (as `DataFrame`).

A very interesting use case of `PipelineModel` is when a `Pipeline` is made up of link:spark-mllib-transformers.md[Transformer] instances.
...
`PredictionModel` is basically a link:spark-mllib-transformers.md[Transformer] with `predict` method to calculate predictions (that end up in `prediction` column).
...
`ClassificationModel` is a <<PredictionModel, PredictionModel>> that transforms a `DataFrame` with mandatory `features`, `label`, and `rawPrediction` (of type link:spark-mllib-vector.md[Vector]) columns to a DataFrame with `prediction` column added.

NOTE: A `Model` with `ClassifierParams` parameters, e.g. `ClassificationModel`, requires that a DataFrame have the mandatory `features`, `label` (of type `Double`), and `rawPrediction` (of type link:spark-mllib-vector.md[Vector]) columns.

`ClassificationModel` comes with its own `transform` (as link:spark-mllib-transformers.md[Transformer]) and `predict` (as <<PredictionModel, PredictionModel>>).
...
`LinearRegressionModel` represents a model produced by a link:spark-mllib-estimators.md#LinearRegression[LinearRegression] estimator. It transforms the required `features` column of type link:spark-mllib-vector.md[org.apache.spark.mllib.linalg.Vector].
...
`RandomForestRegressionModel` is a <<PredictionModel, PredictionModel>> with `features` column of type link:spark-mllib-vector.md[Vector].

Interestingly, `DataFrame` transformation (as part of link:spark-mllib-transformers.md[Transformer] contract) uses xref:ROOT:SparkContext.md#broadcast[SparkContext.broadcast] to send itself to the nodes in a Spark cluster and calls calculates predictions (as `prediction` column) on `features`.

=== [[KMeansModel]] KMeansModel

`KMeansModel` is a `Model` of link:spark-mllib-estimators.md#KMeans[KMeans] algorithm.

It belongs to `org.apache.spark.ml.clustering` package.

[source,scala]
----
// See spark-mllib-estimators.md#KMeans
val kmeans: KMeans = ???
