...
Interestingly, `DataFrame` transformation (as part of link:spark-mllib-transformers.adoc[Transformer] contract) uses xref:ROOT:SparkContext.adoc#broadcast[SparkContext.broadcast] to send itself to the nodes in a Spark cluster and calls calculates predictions (as `prediction` column) on `features`.
