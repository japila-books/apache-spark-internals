...
A `Transformer` is a link:spark-mllib-PipelineStage.adoc[PipelineStage] and thus can be a part of a link:spark-mllib-Pipeline.adoc[Pipeline].
...
Each `UnaryTransformer` calls `validateInputType` while executing `transformSchema(schema: StructType)` (that is part of link:spark-mllib-PipelineStage.adoc[PipelineStage] contract).

NOTE: A `UnaryTransformer` is a link:spark-mllib-PipelineStage.adoc[PipelineStage].
