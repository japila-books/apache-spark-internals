...
NOTE: `report` is used when xref:ROOT:SparkContext.adoc#stop[SparkContext], link:spark-Executor.adoc#stop[Executor], Spark Standalone's `Master` and `Worker`, Spark on Mesos' `MesosClusterScheduler` are requested to stop
