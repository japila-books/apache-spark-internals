...
* `Executor` xref:core:Executor.adoc#creating-instance[is created] (and registers a xref:core:ExecutorSource.adoc[])
...
NOTE: `buildRegistryName` uses link:spark-metrics-properties.adoc#spark.metrics.namespace[spark.metrics.namespace] and xref:core:Executor.adoc#spark.executor.id[spark.executor.id] Spark properties to differentiate between a Spark application's driver and executors, and the other Spark framework's components.

(only when <<instance, instance>> is `driver` or `executor`) `buildRegistryName` builds metrics source name that is made up of link:spark-metrics-properties.adoc#spark.metrics.namespace[spark.metrics.namespace], xref:core:Executor.adoc#spark.executor.id[spark.executor.id] and the name of the `source`.
...
NOTE: `report` is used when xref:ROOT:SparkContext.adoc#stop[SparkContext], xref:core:Executor.adoc#stop[Executor], Spark Standalone's `Master` and `Worker`, Spark on Mesos' `MesosClusterScheduler` are requested to stop
