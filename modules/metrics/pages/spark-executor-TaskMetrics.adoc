...
* `Stage` is requested to xref:scheduler:spark-scheduler-Stage.adoc#makeNewStageAttempt[create a new stage attempt] (when `DAGScheduler` is requested to xref:scheduler:DAGScheduler.adoc#submitMissingTasks[submit the missing tasks of a stage])
...
NOTE: `register` is used exclusively when xref:scheduler:spark-scheduler-Stage.adoc#makeNewStageAttempt[`Stage` is requested for its new attempt].
