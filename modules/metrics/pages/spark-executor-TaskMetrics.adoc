...
`TaskMetrics` is a <<metrics, collection of metrics>> (as <<spark-accumulators.adoc#, AccumulatorV2s>>) tracked during execution of a <<spark-scheduler-Task.adoc#, Task>>.
...
TIP: Use <<spark-SparkListener-EventLoggingListener.adoc#, EventLoggingListener>> for post-execution (history) statistics.
...
NOTE: The local values of the accumulators for a link:spark-scheduler-Task.adoc[task] (as accumulated while the link:spark-scheduler-Task.adoc#run[task runs]) are sent from the executor to the driver when the task completes (and <<fromAccumulators, `DAGScheduler` re-creates `TaskMetrics`>>).
...
4. link:spark-shuffle-ShuffleExternalSorter.adoc#spill[`ShuffleExternalSorter` frees execution memory by spilling to disk]
...
NOTE: `registered` is used exclusively when `Task` is <<spark-scheduler-Task.adoc#serializedTaskMetrics, created>>.
...
NOTE: `fromAccumulatorInfos` is used exclusively when `AppStatusListener` is requested to <<spark-SparkListener-AppStatusListener.adoc#onExecutorMetricsUpdate, onExecutorMetricsUpdate>> (for <<spark-history-server.adoc#, Spark History Server>> only).
