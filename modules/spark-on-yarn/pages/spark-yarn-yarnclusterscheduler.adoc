...
`YarnClusterScheduler` is the xref:scheduler:TaskScheduler.adoc[TaskScheduler] for link:README.adoc[Spark on YARN] in link:spark-submit.adoc#deploy-mode[cluster deploy mode].
...
`postStartHook` calls link:spark-yarn-applicationmaster.adoc#sparkContextInitialized[ApplicationMaster.sparkContextInitialized] before the xref:scheduler:TaskSchedulerImpl.adoc#postStartHook[parent's `postStartHook`].
...
`stop` calls the xref:scheduler:TaskSchedulerImpl.adoc#stop[parent's `stop`] followed by link:spark-yarn-applicationmaster.adoc#sparkContextStopped[ApplicationMaster.sparkContextStopped].
