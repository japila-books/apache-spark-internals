...
* `0.8` (when xref:scheduler:CoarseGrainedSchedulerBackend.adoc#spark.scheduler.minRegisteredResourcesRatio[spark.scheduler.minRegisteredResourcesRatio] property is undefined)

* xref:scheduler:CoarseGrainedSchedulerBackend.adoc#minRegisteredRatio#minRegisteredRatio[minRegisteredRatio] from the parent `CoarseGrainedSchedulerBackend`
...
| xref:rpc:RpcEndpointRef.adoc[RPC endpoint reference] to <<yarnSchedulerEndpoint, `YarnScheduler` RPC endpoint>>
| link:../index.adoc#setupEndpoint[Created] when <<creating-instance, `YarnSchedulerBackend` is created>>.
...
Used for <<applicationAttemptId, applicationAttemptId>> which is part of xref:scheduler:SchedulerBackend.adoc#contract[SchedulerBackend Contract].
...
NOTE: `reset` is part of xref:scheduler:CoarseGrainedSchedulerBackend.adoc#contract[CoarseGrainedSchedulerBackend Contract].

`reset` xref:scheduler:CoarseGrainedSchedulerBackend.adoc#reset[resets] the parent `CoarseGrainedSchedulerBackend` scheduler backend and link:../spark-ExecutorAllocationManager.adoc[ExecutorAllocationManager] (accessible by `SparkContext.executorAllocationManager`).
...
NOTE: `doRequestTotalExecutors` is part of the xref:scheduler:CoarseGrainedSchedulerBackend.adoc#doRequestTotalExecutors[CoarseGrainedSchedulerBackend Contract].
...
Ultimately, it calls the parent's xref:executor:CoarseGrainedExecutorBackend.adoc#start[CoarseGrainedSchedulerBackend.start].
...
`stop` calls the parent's xref:executor:CoarseGrainedExecutorBackend.adoc#requestTotalExecutors[CoarseGrainedSchedulerBackend.requestTotalExecutors] (using `(0, 0, Map.empty)` parameters).

CAUTION: FIXME Explain what `0, 0, Map.empty` means after the method's described for the parent.

It calls the parent's xref:executor:CoarseGrainedExecutorBackend.adoc#stop[CoarseGrainedSchedulerBackend.stop].
...
NOTE: `applicationAttemptId` is part of xref:scheduler:SchedulerBackend.adoc#contract[SchedulerBackend Contract].
...
NOTE: `sufficientResourcesRegistered` is part of the xref:scheduler:CoarseGrainedSchedulerBackend.adoc#contract[CoarseGrainedSchedulerBackend contract] that makes sure that sufficient resources are available.

`sufficientResourcesRegistered` is positive, i.e. `true`, when xref:scheduler:CoarseGrainedSchedulerBackend.adoc#totalRegisteredExecutors[totalRegisteredExecutors] is exactly or above <<minRegisteredRatio, minRegisteredRatio>> of <<totalExpectedExecutors, totalExpectedExecutors>>.
