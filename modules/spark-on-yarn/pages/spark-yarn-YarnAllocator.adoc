...
. `sparkConf` -- link:../SparkConf.adoc[SparkConf]
...
It sets the internal `executorMemory` to xref:executor:Executor.adoc#spark.executor.memory[spark.executor.memory].
...
It sets the internal `executorCores` to xref:executor:Executor.adoc#spark.executor.cores[spark.executor.cores].
