...
Without specifying the link:spark-deploy-mode.md[deploy mode], it is assumed `client`.
...
Spark on YARN supports <<multiple-application-attempts, multiple application attempts>> and supports link:spark-data-locality.md[data locality for data in HDFS]. You can also take advantage of Hadoop's security and run Spark in a link:spark-yarn-kerberos.md[secure Hadoop environment using Kerberos authentication] (aka _Kerberized clusters_).
...
The memory for `ApplicationMaster` is controlled by custom settings per link:spark-deploy-mode.md[deploy mode].

For link:spark-deploy-mode.md#client[client deploy mode] it is a sum of link:spark-yarn-settings.md#spark.yarn.am.memory[spark.yarn.am.memory] (default: `512m`) with an optional overhead as link:spark-yarn-settings.md#spark.yarn.am.memoryOverhead[spark.yarn.am.memoryOverhead].
