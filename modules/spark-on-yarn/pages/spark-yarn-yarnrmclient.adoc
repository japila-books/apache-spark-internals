...
`YarnRMClient` is responsible for <<register, registering>> and <<unregister, unregistering>> a Spark application (in the form of link:spark-yarn-applicationmaster.md[ApplicationMaster]) with link:spark-yarn-introduction.md#ResourceManager[YARN ResourceManager] (and hence _RM_ in the name).
...
NOTE: The input `uiAddress` is the web UI of the Spark application and is specified using the link:spark-yarn-applicationmaster.md#runDriver[SparkContext] (when the application runs in `cluster` deploy mode) or using link:../spark-driver.md#spark.driver.appUIAddress[spark.driver.appUIAddress] property.

<<registered, registered>> flag is enabled.

In the end, `register` link:spark-yarn-YarnAllocator.md#creating-instance[creates a new `YarnAllocator`] (using the input parameters of `register` and the internal <<amClient, AMRMClient>>).

NOTE: `register` is used exclusively when link:spark-yarn-applicationmaster.md#registerAM[`ApplicationMaster` registers itself] with the YARN ResourceManager.
...
`unregister` is called when link:spark-yarn-applicationmaster.md#unregister[`ApplicationMaster` wants to unregister].
...
`getMaxRegAttempts` uses link:../SparkConf.md[SparkConf] and YARN's https://hadoop.apache.org/docs/current/api/org/apache/hadoop/yarn/conf/YarnConfiguration.html[YarnConfiguration] to read configuration settings and return the maximum number of application attempts before link:spark-yarn-applicationmaster.md[ApplicationMaster] registration with YARN is considered unsuccessful (and so the Spark application).
...
Internally, it uses YARN-specific methods like link:spark-yarn-YarnSparkHadoopUtil.md#getContainerId[ConverterUtils.toContainerId] and https://hadoop.apache.org/docs/current/api/index.html?org/apache/hadoop/yarn/client/api/YarnClient.html[ContainerId.getApplicationAttemptId].
