...
| link:spark-yarn-settings.adoc#spark.yarn.executor.memoryOverhead[spark.yarn.executor.memoryOverhead] and falls back to link:spark-yarn-YarnSparkHadoopUtil.adoc#MEMORY_OVERHEAD_FACTOR[10%] of the xref:core:Executor.adoc#spark.executor.memory[spark.executor.memory] or link:spark-yarn-YarnSparkHadoopUtil.adoc#MEMORY_OVERHEAD_MIN[384] whatever is larger.
| FIXME
...
* Sets the internal `executorMemory` to xref:core:Executor.adoc#spark.executor.memory[spark.executor.memory].
...
* Creates an internal instance of <<../spark-LauncherBackend.adoc#, LauncherBackend>> (as <<launcherBackend, launcherBackend>>).
...
Internally, `submitApplication` requests the `LauncherBackend` to <<../spark-LauncherBackend.adoc#connect, connect>> first and then executes `Client.setupCredentials` to set up credentials for future calls.
...
[align="center"]
|===
| The name of the Spark application | link:../SparkConf.adoc#spark.app.name[spark.app.name] configuration setting or `Spark` if not set
...
If however the executor memory (as a sum of xref:core:Executor.adoc#spark.executor.memory[spark.executor.memory] and link:spark-yarn-settings.adoc#spark.yarn.executor.memoryOverhead[spark.yarn.executor.memoryOverhead] settings) is more than the maximum memory capability, `verifyClusterResources` throws an `IllegalArgumentException` with the following message:
...
Additionally, the archive contains a `__spark_conf__.properties` with the current link:../SparkConf.adoc[Spark configuration].
...
+
NOTE: `extraClassPath` corresponds to link:../spark-driver.adoc#spark_driver_extraClassPath[spark.driver.extraClassPath] for the driver and link:../configuration-properties.adoc#spark.executor.extraClassPath[spark.executor.extraClassPath] for executors.
...
It then instantiates link:../SparkConf.adoc[SparkConf], parses command-line arguments (using <<ClientArguments, ClientArguments>>) and passes the call on to <<run, Client.run>> method.
...
When the application state changes, `LauncherBackend` is <<../spark-LauncherBackend.adoc#setState, notified>>.
...
`reportLauncherState` merely requests the LauncherBackend` to <<../spark-LauncherBackend.adoc#setState, setState>> with the given state.
