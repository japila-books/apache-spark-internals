...
NOTE: `Client` was initially used as a <<main, standalone application>> to link:../spark-submit.md#submit[submit Spark applications] to a YARN cluster, but is currently considered obsolete.
...
NOTE: The additional files to distribute can be defined using link:spark-submit.md[spark-submit] using command-line options link:spark-submit.md#jars[--jars], link:spark-submit.md#files[--files], and link:spark-submit.md#archives[--archives].
...
NOTE: It is executed by link:spark-submit.md#submit[spark-submit] with `--master yarn` command-line argument.
