...
Computed using the optional link:spark-yarn-settings.adoc#spark.yarn.max.executor.failures[spark.yarn.max.executor.failures] if set. Otherwise, it is twice xref:core:Executor.adoc#spark.executor.instances[spark.executor.instances] or link:../spark-dynamic-allocation.adoc#spark.dynamicAllocation.maxExecutors[spark.dynamicAllocation.maxExecutors] (with dynamic allocation enabled) with the minimum of `3`.
...
`registerAM` link:spark-yarn-ExecutorRunnable.adoc#launchContextDebugInfo[prints YARN launch context diagnostic information (with command, environment and resources) for executors] (with xref:core:Executor.adoc#spark.executor.memory[spark.executor.memory], xref:core:Executor.adoc#spark.executor.cores[spark.executor.cores] and dummy `<executorId>` and `<hostname>`)
