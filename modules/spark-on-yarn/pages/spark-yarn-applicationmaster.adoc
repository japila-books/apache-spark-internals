...
| (uninitialized)
| xref:rpc:RpcEndpointRef.adoc[RpcEndpointRef] to the *YarnAM* RPC endpoint initialized when `ApplicationMaster` <<runAMEndpoint, runAMEndpoint>>.
...
| [[sparkConf]] `sparkConf`
| New link:../SparkConf.adoc[SparkConf]
| FIXME
...
| [[rpcEnv]] `rpcEnv`
| (uninitialized)
a| xref:rpc:index.adoc[RpcEnv] which is:
...
Computed using the optional link:spark-yarn-settings.adoc#spark.yarn.max.executor.failures[spark.yarn.max.executor.failures] if set. Otherwise, it is twice xref:executor:Executor.adoc#spark.executor.instances[spark.executor.instances] or xref:ROOT:spark-dynamic-allocation.adoc#spark.dynamicAllocation.maxExecutors[spark.dynamicAllocation.maxExecutors] (with dynamic allocation enabled) with the minimum of `3`.
...
`runExecutorLauncher` link:../index.adoc#create[creates `sparkYarnAM` RPC environment] (on link:spark-yarn-settings.adoc#spark.yarn.am.port[spark.yarn.am.port] port, the internal <<sparkConf, SparkConf>> and `clientMode` enabled).

[TIP]
====
Read the note in link:../index.adoc#create[Creating RpcEnv] to learn the meaning of `clientMode` input argument.
...
[[runDriver-rpcEnv]]
runDriver waits link:spark-yarn-settings.adoc#spark.yarn.am.waitTime[spark.yarn.am.waitTime] time till the Spark application's xref:ROOT:SparkContext.adoc[] is available and accesses the link:../index.adoc[current `RpcEnv`] (and saves it as the internal <<rpcEnv, rpcEnv>>).

NOTE: runDriver uses xref:core:SparkEnv.adoc#rpcEnv[`SparkEnv` to access the current `RpcEnv`] that the xref:ROOT:SparkContext.adoc#env[Spark application's `SparkContext` manages].
...
`registerAM` then creates a link:../index.adoc#RpcEndpointAddress[RpcEndpointAddress] for the driver's xref:scheduler:CoarseGrainedSchedulerBackend.adoc#CoarseGrainedScheduler[CoarseGrainedScheduler RPC endpoint] available at link:../spark-driver.adoc#spark.driver.host[spark.driver.host] and link:../spark-driver.adoc#spark.driver.port[spark.driver.port].

`registerAM` link:spark-yarn-ExecutorRunnable.adoc#launchContextDebugInfo[prints YARN launch context diagnostic information (with command, environment and resources) for executors] (with xref:executor:Executor.adoc#spark.executor.memory[spark.executor.memory], xref:executor:Executor.adoc#spark.executor.cores[spark.executor.cores] and dummy `<executorId>` and `<hostname>`)
...
Ultimately, it removes the cache-related settings from the link:../SparkConf.adoc[Spark configuration] and system properties.
...
* link:../configuration-properties.adoc#spark.master[spark.master] as `yarn`
...
`runAMEndpoint` sets up a xref:rpc:RpcEndpointRef.adoc[RpcEndpointRef] to the driver's `YarnScheduler` endpoint and registers *YarnAM* endpoint.
...
Internally, `runAMEndpoint` link:../index.adoc#setupEndpointRefByURI[gets a `RpcEndpointRef`] to the driver's `YarnScheduler` endpoint (available on the `host` and `port`).
...
`runAMEndpoint` then link:../index.adoc#setupEndpoint[registers the RPC endpoint] as *YarnAM* (and link:spark-yarn-AMEndpoint.adoc[AMEndpoint] implementation with ``ApplicationMaster``'s <<rpcEnv, RpcEnv>>, `YarnScheduler` endpoint reference, and `isClusterMode` flag).
