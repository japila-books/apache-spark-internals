...
It is a custom xref:scheduler:TaskSchedulerImpl.adoc[TaskSchedulerImpl] with ability to compute racks per hosts, i.e. it comes with a specialized <<getRackForHost, getRackForHost>>.
...
NOTE: `getRackForHost` overrides the xref:scheduler:TaskSchedulerImpl.adoc#getRackForHost[parent TaskSchedulerImpl's getRackForHost]
