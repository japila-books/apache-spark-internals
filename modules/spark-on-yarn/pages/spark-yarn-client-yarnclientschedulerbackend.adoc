...
`start` starts link:spark-yarn-yarnschedulerbackend.adoc#start[YarnSchedulerBackend] (that in turn starts the top-level xref:executor:CoarseGrainedExecutorBackend.adoc#start[CoarseGrainedSchedulerBackend]).
...
That leads to stopping the current `SparkContext` (using xref:ROOT:SparkContext.adoc#stop[SparkContext.stop]).
