...
`YarnClientSchedulerBackend` is the link:spark-yarn-yarnschedulerbackend.md[YarnSchedulerBackend] used when a Spark application is link:../spark-submit.md[submitted to a YARN cluster] in link:../spark-deploy-mode.md#client[`client` deploy mode].
