...
Internally, `resolveRelation` creates an instance of <<providingClass, providingClass>> (of a `DataSource`) and branches off per its type, i.e. `SchemaRelationProvider`, link:spark-sql-RelationProvider.adoc[RelationProvider] or <<FileFormat, FileFormat>>.
