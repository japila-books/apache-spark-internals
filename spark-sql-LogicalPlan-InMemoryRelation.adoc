...
`InMemoryRelation` is <<creating-instance, created>> when `CacheManager` is requested to link:spark-sql-CacheManager.adoc#cacheQuery[cache a Dataset].
...
[NOTE]
====
`InMemoryRelation` can be created using `apply` factory method that has no <<output, output attributes>> (and uses `child` physical plan's `output`).
