...
You can group data in a link:spark-sql-dataset.adoc[Dataset] to compute aggregates over a collection of (grouped) records.

You can use <<agg, agg>> method for computing aggregations per column on the entire dataset (without creating groups).
...
Internally, it first link:spark-sql-catalyst-LogicalPlan.adoc#resolveQuoted[resolves columns] and then builds a <<RelationalGroupedDataset, RelationalGroupedDataset>>.
