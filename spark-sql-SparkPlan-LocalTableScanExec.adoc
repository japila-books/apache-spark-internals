...
`LocalTableScanExec` is a result of applying link:spark-sql-SparkStrategy-BasicOperators.adoc[`BasicOperators` strategy] to link:spark-sql-LogicalPlan-LocalRelation.adoc[LocalRelation] and link:spark-sql-streaming-MemoryPlan.adoc[MemoryPlan] logical query plans.
