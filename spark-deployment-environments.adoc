...
A Spark application can run locally or on the cluster which is based on the cluster manager and the deploy mode (`--deploy-mode`). See link:spark-submit.adoc[spark-submit script].
