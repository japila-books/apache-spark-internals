...
In the end, `insertRecordIntoSorter` inserts the `record` to the internal `ShuffleExternalSorter` (as `Platform.BYTE_ARRAY_OFFSET` ).
...
`closeAndWriteOutput` clears the internal `ByteArrayOutputStream` and `SerializationStream`.

`closeAndWriteOutput` requests `ShuffleExternalSorter` to close itself and return `SpillInfo` metadata.

`closeAndWriteOutput` clears the internal `ShuffleExternalSorter`.
