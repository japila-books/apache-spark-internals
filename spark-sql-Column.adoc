...
[NOTE]
====
`Column` has a reference to Catalyst's link:spark-sql-catalyst-Expression.adoc[Expression] it was created for using `expr` method.
...
=== [[over]] Defining Windowing Column -- `over` Operator
...
`over` creates a *windowing column* that allows for window computations over a link:spark-sql-windows.adoc#WindowSpec[window] (i.e. a group of records that are in _some_ relation to the current record).

TIP: Read up on windowed aggregation in Spark SQL in link:spark-sql-windows.adoc[Windowed Aggregate Functions -- Windows].
