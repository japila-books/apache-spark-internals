...
`ExpressionEncoder` is an `Encoder` with additional properties, i.e. `flat`, one or many serializers and a deserializer expressions.

A `ExpressionEncoder` can be *flat* is which case there is only one Catalyst expression for the serializer.

*Serializer expressions* are used to encode an object to a row. It is assumed that all serializer expressions contain at least one and the same `BoundReference`.
