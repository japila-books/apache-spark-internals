...
=== [[load]] Loading Datasets (into DataFrames) -- `load` Method
...
`load` loads a dataset into an untyped link:spark-sql-dataframe.adoc[DataFrame].
...
=== [[jdbc]] Loading Datasets From JDBC Data Sources (into  DataFrames) -- `jdbc` Method
...
`jdbc` loads a dataset from an external table using JDBC into an untyped link:spark-sql-dataframe.adoc[DataFrame].
