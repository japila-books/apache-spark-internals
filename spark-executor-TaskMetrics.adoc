...
`TaskMetrics` is a <<metrics, collection of metrics>> tracked during execution of a <<spark-scheduler-Task.adoc#, Task>>.

NOTE: The <<metrics, metrics>> are simply <<spark-accumulators.adoc#, AccumulatorV2s>>.

`TaskMetrics` is <<creating-instance, created>> when:

* `TaskMetrics` factory object is requested to <<empty, empty>>, <<fromAccumulatorInfos, fromAccumulatorInfos>>, and <<fromAccumulators, fromAccumulators>>
...
NOTE: `registered` is used when...FIXME
...
NOTE: `fromAccumulatorInfos` is used when...FIXME
...
NOTE: `fromAccumulators` is used when...FIXME
