...
| Forwards launch task request from the driver to the managed single <<executor, executor>>.
...
NOTE: `CoarseGrainedExecutorBackend` acts as a proxy between the driver and the managed single executor and merely re-packages `LaunchTask` payload to pass it along for execution to the executor.
...
If <<executor, executor>> is not available, `LaunchTask` <<exitExecutor, exits `CoarseGrainedExecutorBackend`>> with the error code `1` and `ExecutorLossReason` with the following message:
