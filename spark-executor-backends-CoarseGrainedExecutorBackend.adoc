...
| [[executor]] `executor`
| (empty)
|  link:spark-executor.adoc#coarse-grained-executor[Executor] that is managed by the `CoarseGrainedExecutorBackend`.

Set when `CoarseGrainedExecutorBackend` <<RegisteredExecutor, registers with `CoarseGrainedSchedulerBackend`>>.

Used to <<LaunchTask, launch>> and <<KillTask, kill>> tasks.

Stopped when `CoarseGrainedExecutorBackend` gets notified to <<Shutdown, shut down>>.
