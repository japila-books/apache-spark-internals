...
NOTE: `LaunchTask` is sent when link:spark-CoarseGrainedSchedulerBackend.adoc#launchTasks[`CoarseGrainedSchedulerBackend` launches tasks] (one `LaunchTask` per task).
