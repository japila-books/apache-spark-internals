...
`PropagateEmptyRelation` is a link:spark-sql-LogicalPlan.adoc[LogicalPlan] optimization rule that collapses plans with empty link:spark-sql-logical-plan-LocalRelation.adoc[`LocalRelation` logical query plans], e.g. <<explode, explode>> or <<join, join>>.
