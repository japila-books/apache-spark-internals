...
*SQL Parser Framework* in Spark SQL uses ANTLR to parse a SQL text and then creates link:spark-sql-DataType.adoc[data types], Catalyst's link:spark-sql-catalyst-Expression.adoc[Expression], `TableIdentifier`, and link:spark-sql-catalyst-LogicalPlan.adoc[LogicalPlan].
...
`ParserInterface` is the parser contract for extracting link:spark-sql-catalyst-LogicalPlan.adoc[LogicalPlan], Catalyst `Expressions` (to create link:spark-sql-columns.adoc[Columns] from), and `TableIdentifiers` from a given SQL string.
...
`AbstractSqlParser` creates an layer of indirection and expects that subclasses provide custom <<AstBuilder, AstBuilder>> that in turn converts a ANTLR `ParseTree` into a link:spark-sql-DataType.adoc[data type], link:spark-sql-catalyst-Expression.adoc[Expression], `TableIdentifier`, or link:spark-sql-catalyst-LogicalPlan.adoc[LogicalPlan].
...
4. `visitSingleStatement` for a link:spark-sql-catalyst-LogicalPlan.adoc[LogicalPlan]
...
`SparkSqlParser` is used to parse sql strings into their corresponding link:spark-sql-catalyst-LogicalPlan.adoc[logical query plans] in the following:
