...
`WindowExpression` is an link:spark-sql-catalyst-Expression.adoc#Unevaluable[unevaluable expression] (i.e. with `eval` and `doGenCode` methods unsupported).

`WindowExpression` is created to represent `windowFunction` and `WindowSpecDefinition` link:spark-sql-catalyst-Expression.adoc[expressions] in a SQL query after `Analyzer` link:spark-sql-Analyzer.adoc#WindowsSubstitution[resolves `UnresolvedWindowExpressions`].
...
`UnresolvedWindowExpression` is an link:spark-sql-catalyst-Expression.adoc#Unevaluable[unevaluable expression] (i.e. with `eval` and `doGenCode` methods unsupported).
