...
When a `BlockManager` instance is created it first sets the internal `externalShuffleServiceEnabled` flag to the value of link:spark-shuffle-manager.adoc#spark.shuffle.service.enabled[spark.shuffle.service.enabled] setting.
