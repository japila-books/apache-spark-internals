...
=== [[reregister]] Re-registering Blocks with Driver -- `reregister` Method
...
Block Manager uses link:spark-MetricsSystem.adoc[Spark Metrics System] (via `BlockManagerSource`) to report metrics about internal status.
...
When a non-local executor starts, it initializes a `BlockManager` object using link:spark-configuration.adoc#spark.app.id[spark.app.id] Spark property for the id.
