...
* It registers itself to link:spark-BlockManagerMaster.adoc[BlockManagerMaster] (using <<registerBlockManager, BlockManagerMaster.registerBlockManager>>) passing the <<BlockManagerId, BlockManagerId>>, the maximum memory (as `maxMemory`), and the <<BlockManagerSlaveEndpoint, BlockManagerSlaveEndpoint>>.
