...
`reportBlockStatus` is an internal method for reporting a block status to the driver (calls `tryToReportBlockStatus`) and if told to reregister it prints out the following INFO message to the logs:
