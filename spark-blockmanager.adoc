...
It creates an instance of <<BlockInfoManager, BlockInfoManager>> (as `blockInfoManager`).
...
It registers <<BlockManagerSlaveEndpoint, BlockManagerSlaveEndpoint>> (as `slaveEndpoint`).
...
`MemoryStore` requires link:spark-configuration.adoc[SparkConf], <<BlockInfoManager, BlockInfoManager>>, `SerializerManager`, link:spark-sparkenv.adoc#MemoryManager[MemoryManager] and `BlockEvictionHandler` to be created.
...
It requests <<BlockInfoManager, BlockInfoManager>> for lock for writing for the `blockId` block. If it receives none, it prints out the following WARN message to the logs and quits.
...
The block is removed from <<BlockInfoManager, BlockInfoManager>>.
...
It then requests RDD blocks from <<BlockInfoManager, BlockInfoManager>> and <<removeBlock, removes them (from memory and disk)>> (without informing the driver).
...
It then requests all `BroadcastBlockId` objects that belong to the `broadcastId` broadcast from <<BlockInfoManager, BlockInfoManager>> and <<removeBlock, removes them (from memory and disk)>>.
...
For each block metadata (in <<BlockInfoManager, BlockInfoManager>>) it <<getCurrentBlockStatus, gets block current status>> and <<tryToReportBlockStatus, tries to send it to the BlockManagerMaster>>.
...
It then asserts that the `blockId` block is locked for writing (using <<BlockInfoManager, BlockInfoManager.assertBlockIsLockedForWriting>>).
