...
When a `BlockManager` instance is created it first sets the internal `externalShuffleServiceEnabled` flag to the value of link:spark-ExternalShuffleService.adoc#spark.shuffle.service.enabled[spark.shuffle.service.enabled] setting.
...
Ultimately, if the initialization happens on an executor and an link:spark-ExternalShuffleService.adoc#spark.shuffle.service.enabled[external shuffle service is used], it <<registerWithExternalShuffleServer, registers itself to the external shuffle service>>.
