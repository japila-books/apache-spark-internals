...
*Block Manager* is a key-value store for blocks. It acts as a local cache and runs on every node, i.e. a link:spark-driver.adoc[driver] and link:spark-executor.adoc[executors]. It provides interface for putting and retrieving blocks both locally and remotely into various stores, i.e. memory, disk, and off-heap.

A BlockManager manages the storage for most of the data in Spark which can be a block that represents a cached RDD partition, an intermediate shuffle data, a broadcast data, etc. See <<BlockId, BlockId>>.
