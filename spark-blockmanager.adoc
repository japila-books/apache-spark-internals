...
`BlockInfo` is a metadata of <<BlockId, BlockId>>.

It consists of link:spark-rdd-caching.adoc#StorageLevel[StorageLevel], `ClassTag` and `tellMaster` flag.

It has a size and the number of readers. It is assumed that the number of readers is non-negative, i.e. `0` or more.

It knows a so-called writer task.

It defaults to `0` for the size and the number of readers.
...
`doPut` executes the input `putBody` function with a <<BlockInfo, BlockInfo>> being a new `BlockInfo` object that link:spark-BlockInfoManager.adoc#lockNewBlockForWriting[BlockInfoManager managed to create a lock for writing].
