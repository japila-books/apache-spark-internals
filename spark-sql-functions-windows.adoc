...
*Window aggregate functions* (aka *window functions* or *windowed aggregates*) are functions that perform a calculation on a group of records (called a *window*) that are in _some_ relation to the current record.
...
Unlike the link:spark-sql-basic-aggregation.adoc[regular aggregate operators], windowed aggregates work across the rows that fall into the same partition as the current row.
