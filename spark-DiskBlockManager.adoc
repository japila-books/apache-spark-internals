...
NOTE: `subDirs` is a matrix with the columns being the number of local directories and the rows as collection of `subDirsPerLocalDir` size.
...
| [[spark_diskStore_subDirectories]] `spark.diskStore.subDirectories` | `64` |
|======================
