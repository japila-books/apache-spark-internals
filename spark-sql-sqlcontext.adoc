...
A `SQLContext` object requires a `SparkContext`, a `CacheManager`, and a <<SQLListener, SQLListener>>. They are all `transient` and do not participate in serializing a SQLContext.
...
`newSession` returns a new `SqlContext` that shares `SparkContext`, `CacheManager`, <<SQLListener, SQLListener>>, and `ExternalCatalog`.
...
==== [[SQLListener-getExecution]] Getting SQL Execution Data (getExecution method)
...
==== [[SQLListener-getExecutionMetrics]] Getting Execution Metrics (getExecutionMetrics method)
...
==== [[SQLListener-mergeAccumulatorUpdates]] mergeAccumulatorUpdates method
...
`spark.sql.ui.retainedExecutions` (default: `1000`) controls

=== [[SQLHistoryListener]] SQLHistoryListener -- SQL UI in History Server

`SQLHistoryListener` is a custom <<SQLListener, SQLListener>> for rendering the SQL UI in the link:spark-HistoryServer.adoc[History Server] with custom callback methods:
...
`SparkListenerSQLExecutionStart` and `SparkListenerSQLExecutionEnd` are posted from within <<SQLExecution-withNewExecutionId, SQLExecution.withNewExecutionId>>.
...
=== [[SQLExecution-withNewExecutionId]] SQLExecution.withNewExecutionId Helper Methods
...
<2> With auto-generated execution identifier

`SQLExecution.withNewExecutionId` is a family of two helper methods that execute an action (as the input `body`) with the *execution id* local property set (as `executionId` or auto-generated). The execution identifier is set as `spark.sql.execution.id` local property (using link:spark-sparkcontext.adoc#setLocalProperty[SparkContext.setLocalProperty]).

The use case is to track Spark jobs (even when running in separate threads) that belong to a single Spark action.

NOTE: It is used in a proxy-like `Dataset.withNewExecutionId` method.
...
In addition, the `QueryExecution` variant posts `SparkListenerSQLExecutionStart` and `SparkListenerSQLExecutionEnd` events (to link:spark-LiveListenerBus.adoc[LiveListenerBus] event bus) before and after executing the `body` action, respectively.
