...
The `implicits` object is a helper class with methods to convert  objects into link:spark-sql-dataset.adoc[Datasets], link:spark-sql-dataframe.adoc[DataFrames], and comes with many link:spark-sql-dataset.adoc#Encoder[Encoders] for "primitive" types as well as the collections thereof.
