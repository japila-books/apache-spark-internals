...
[CAUTION]
====
As of Spark *2.0.0* `SQLContext` is only for backward compatibility. Use link:spark-sql-sparksession.adoc[SparkSession] instead.
====

*SQLContext* is the entry point for Spark SQL in Spark *1.x*. Whatever you do in Spark SQL it has to start from <<creating-instance, creating an instance of SQLContext>>.
