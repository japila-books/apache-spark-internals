...
When executed, `spark-submit` first checks whether `SPARK_HOME` environment variable is set and sets it to the directory that contains `bin/spark-submit` shell script if not. It then executes link:spark-class.adoc[`spark-class` shell script] that in turn launches <<main, `org.apache.spark.deploy.SparkSubmit` class>>.
