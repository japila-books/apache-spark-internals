...
| `--deploy-mode` | `spark.submit.deployMode` | `DEPLOY_MODE` | | `deployMode`
| `--driver-class-path` | `spark.driver.extraClassPath` |  | | `driverExtraClassPath`
| `--driver-java-options` | `spark.driver.extraJavaOptions` |  | | `driverExtraJavaOptions`
| `--driver-library-path` | `spark.driver.extraLibraryPath` |  | | `driverExtraLibraryPath`
| `--driver-memory` | `spark.driver.memory` | `SPARK_DRIVER_MEMORY` | | `driverMemory`
| `--driver-cores` | `spark.driver.cores` | | | `driverCores`
| `--exclude-packages` | `spark.jars.excludes` | | | `packagesExclusions`
| `--executor-memory` | `spark.executor.memory` | `SPARK_EXECUTOR_MEMORY` | | `executorMemory`
| `--executor-cores` | `spark.executor.cores` | `SPARK_EXECUTOR_CORES` | | `executorCores`
