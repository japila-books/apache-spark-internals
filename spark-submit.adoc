...
With `--queue` you can choose the queue.

CAUTION: FIXME What is a `queue`?

NOTE: It corresponds to link:spark-yarn.adoc#spark.yarn.queue[spark.yarn.queue] system property.
...
`runMain` is an internal method to...FIXME

It prints out the input parameters when executed with `verbose` flag enabled.
...
It <<addJarToClasspath, adds the jars>> specified in `childClasspath` input parameter to the context classloader that loads the main class.

CAUTION: FIXME Where is `childClasspath` calculated?
...
CAUTION: FIXME Where are `sysProps` calculated?
...
===== [[addJarToClasspath]] addJarToClasspath method
