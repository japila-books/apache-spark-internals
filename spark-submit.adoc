...
`spark-submit` script allows you to manage your Spark applications. You can <<submit, submit your Spark application>> to a Spark deployment environment for execution, <<kill, kill>> or <<status, request status>> of Spark applications.
...
* `--driver-class-path` (see <<driver-class-path, driver-class-path>>)
...
`--driver-class-path` command-line option sets the extra `CLASSPATH` entries (i.e. jars and directories) for a driver.
...
NOTE: Command-line options (e.g. `--driver-class-path`) have higher precedence than their corresponding Spark settings in a Spark properties file (e.g. `spark.driver.extraClassPath`). You can override Spark settings on command line using the command-line options.

CAUTION: FIXME What about `--conf`?
...
| link:spark-driver.adoc#spark_driver_extraClassPath[spark.driver.extraClassPath] | `--driver-class-path` |
|======================
