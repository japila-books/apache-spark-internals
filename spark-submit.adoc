...
=== [[driver-cores]] Driver Cores in Cluster Deploy Mode (--driver-cores switch)

```
--driver-cores NUM
```

`--driver-cores` sets the number of cores for the link:spark-driver.adoc[driver] in the link:spark-deploy-mode.adoc#cluster[cluster deploy mode].
...
=== [[queue]] Specifying YARN Resource Queue (--queue switch)
