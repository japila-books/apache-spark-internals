...
A *DataFrame* is a data abstraction or a domain-specific language (DSL) for working with *structured* and *semi-structured data*. It is a collection of rows with a schema.
...
DataFrames use the link:spark-sql-catalyst.adoc[Catalyst query optimizer] to produce more efficient queries (and so they are supposed to be faster than corresponding RDD-based queries).

NOTE: Your DataFrames can also be type-safe and moreover further improve their performance through link:spark-sql-dataset.adoc#Encoder[specialized encoders that significantly cut serialization and deserialization times].

=== [[features]] Features of DataFrame

A `DataFrame` is a collection of "generic" link:spark-sql-dataframe-row.adoc[Row] instances (as `RDD[Row]`) and a schema (as `StructType`).
...
[source, scala]
----
scala> val df = Seq("hello", "world!").zip(0 to 1).toDF("text", "id")
df: org.apache.spark.sql.DataFrame = [text: string, id: int]

scala> val ds = df.as[(String, Int)]
ds: org.apache.spark.sql.Dataset[(String, Int)] = [text: string, id: int]
----
...
After you `import sqlContext.implicits._` (which is done for you by Spark shell) you may apply `toDF` method to convert objects to DataFrames.
