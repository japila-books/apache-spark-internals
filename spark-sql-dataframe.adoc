...
A *DataFrame* is a data abstraction or a domain-specific language (DSL) for working with *structured* and *semi-structured data*, i.e. datasets with a schema. A DataFrame is thus a collection of link:spark-sql-dataframe-row.adoc[rows] with a schema.

It uses the immutable, in-memory, resilient, distributed and parallel capabilities of link:spark-rdd.adoc[RDD], and puts a link:spark-sql-dataframe-schema.adoc[schema] on the data.
...
A `DataFrame` is a collection of "generic" link:spark-sql-dataframe-row.adoc[Row] instances (as `RDD[Row]`) and a link:spark-sql-dataframe-schema.adoc[schema].

NOTE: Regardless of how you create a `DataFrame`, it will always be a pair of `RDD[Row]` and link:spark-sql-dataframe-structtype.adoc[StructType].
