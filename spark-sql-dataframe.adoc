...
A *DataFrame* is a data abstraction or a domain-specific language (DSL) for working with *structured* and *semi-structured data*, i.e. datasets with a schema. A DataFrame is thus a collection of link:spark-sql-dataframe-row.adoc[rows] with a link:spark-sql-schema.adoc[schema] that is a result of a structured query it describes.
...
DataFrames use the link:spark-sql-catalyst-Optimizer.adoc[Catalyst query optimizer] to produce efficient queries (and so they are supposed to be faster than corresponding RDD-based queries).
