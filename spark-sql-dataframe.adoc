...
[NOTE]
====
In Spark *2.0.0* `DataFrame` is a type alias for `Dataset[Row]`.
...
A `DataFrame` is a collection of link:spark-sql-dataframe-row.adoc[Row] instances (as `RDD[Row]`) and a schema (as `StructType`).
