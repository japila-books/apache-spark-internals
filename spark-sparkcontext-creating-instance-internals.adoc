...
It expects that the class name represents a `SparkListenerInterface` listener with a constructor taking a `SparkConf` or no arguments and link:spark-LiveListenerBus.adoc#ListenerBus-addListener[register] it to link:spark-LiveListenerBus.adoc[LiveListenerBus]. You should see the following INFO message in the logs:

```
INFO Registered listener [className]
```

It link:spark-LiveListenerBus.adoc#LiveListenerBus-start[starts LiveListenerBus] and records it in the internal `_listenerBusStarted`.
