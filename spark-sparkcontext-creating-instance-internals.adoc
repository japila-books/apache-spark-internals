...
`SparkContext` sets link:spark-configuration.adoc#spark.app.id[spark.app.id] property to be the <<_applicationId, unique identifier of a Spark application>> and, if enabled, link:spark-webui-SparkUI.adoc#setAppId[passes it on to `SparkUI`].
...
link:spark-MetricsSystem.adoc#start[`MetricsSystem` is started] (after the application id is set using <<spark.app.id, spark.app.id>>).
...
link:spark-MetricsSystem.adoc#registerSource[`MetricsSystem` is requested to register] the following sources:
