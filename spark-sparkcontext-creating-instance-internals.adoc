...
[[_heartbeatReceiver]]
`SparkContext` registers link:spark-sparkcontext-HeartbeatReceiver.adoc[HeartbeatReceiver RPC endpoint].
...
[[taskScheduler-start]]
`TaskScheduler` link:spark-TaskScheduler.adoc#start[is started].
...
NOTE: The unique identifier of a Spark application is later used to initialize link:spark-webui-SparkUI.adoc#setAppId[SparkUI] and link:spark-blockmanager.adoc#initialize[BlockManager].

[[spark.app.id]]
link:spark-configuration.adoc#spark.app.id[spark.app.id] Spark property is set to the current application id and `SparkUI` link:spark-webui-SparkUI.adoc#setAppId[gets notified] if enabled.
...
link:spark-metrics.adoc#start[`MetricsSystem` is started] (after the application id is set using <<spark.app.id, spark.app.id>>).
