...
link:spark-webui.adoc#SparkUI-createLiveUI[SparkUI.createLiveUI] gets called to set `_ui` if the property link:spark-webui.adoc#spark_ui_enabled[spark.ui.enabled] is enabled (i.e. `true`).
