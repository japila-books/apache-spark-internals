...
This document describes the initialization sequence of link:spark-sparkcontext.adoc[SparkContext].
...
It all starts with checking link:spark-sparkcontext.adoc#allowMultipleContexts[whether SparkContexts can be shared or not using `spark.driver.allowMultipleContexts`].
...
link:spark-sparkcontext.adoc#listenerBus[listenerBus] is created.
...
`setupAndStartListenerBus` is an internal method that reads link:spark-scheduler-listeners.adoc#spark.extraListeners[spark.extraListeners] setting from the current link:spark-configuration.adoc[SparkConf] to create and register link:spark-sparkcontext.adoc#SparkListenerInterface[SparkListenerInterface] listeners.

It expects that the class name represents a `SparkListenerInterface` listener with a constructor taking a `SparkConf` or no arguments and link:spark-sparkcontext.adoc#ListenerBus-addListener[register] it to link:spark-sparkcontext.adoc#LiveListenerBus[LiveListenerBus]. You should see the following INFO message in the logs:

```
INFO Registered listener [className]
```

It link:spark-sparkcontext.adoc#LiveListenerBus-start[starts LiveListenerBus] and records it in the internal `_listenerBusStarted`.
