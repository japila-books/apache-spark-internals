...
[[_cleaner]]
If link:spark-service-contextcleaner.adoc#spark_cleaner_referenceTracking[spark.cleaner.referenceTracking] Spark property is enabled (i.e. `true`), link:spark-service-contextcleaner.adoc#creating-instance[`ContextCleaner` is created] (as `_cleaner`) and link:spark-service-contextcleaner.adoc#start[started] immediately. Otherwise, `_cleaner` is empty.
