...
NOTE: The current `AppStatusStore` is available as `statusStore` property of the `SparkContext`.
...
`SparkContext` link:spark-webui-SparkUI.adoc#create[creates a SparkUI] when link:spark-webui.adoc#spark.ui.enabled[spark.ui.enabled] Spark property is enabled (i.e. `true`).

NOTE: link:spark-webui.adoc#spark.ui.enabled[spark.ui.enabled] Spark property is assumed enabled when undefined.
