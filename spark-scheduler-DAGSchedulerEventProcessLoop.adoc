...
NOTE: The purpose of the `DAGSchedulerEventProcessLoop` is to have a separate thread to process events asynchronously and serially, i.e. one by one, and let link:spark-scheduler-DAGScheduler.adoc[DAGScheduler] do its work on the main thread.
...
| <<AllJobsCancelled, AllJobsCancelled>> | | `DAGScheduler` was requested to link:spark-scheduler-DAGScheduler.adoc#cancelAllJobs[cancel all running or waiting jobs].

| <<BeginEvent, BeginEvent>> | <<handleBeginEvent, handleBeginEvent>> | link:spark-scheduler-TaskSetManager.adoc[TaskSetManager] informs `DAGScheduler` that a task is starting (through link:spark-scheduler-DAGScheduler.adoc#taskStarted[taskStarted]).

| [[CompletionEvent]] `CompletionEvent`
| <<handleTaskCompletion, handleTaskCompletion>>
| Posted to inform link:spark-scheduler-DAGScheduler.adoc#taskEnded[`DAGScheduler` that a task has completed (successfully or not)].

`CompletionEvent` conveys the following information:

1. Completed link:spark-scheduler-Task.adoc[Task] instance (as `task`)
...
5. [[CompletionEvent-taskInfo]] link:spark-scheduler-TaskInfo.adoc[TaskInfo]

| <<ExecutorAdded, ExecutorAdded>> | <<handleExecutorAdded, handleExecutorAdded>> | `DAGScheduler` was informed (through link:spark-scheduler-DAGScheduler.adoc#executorAdded[executorAdded]) that an executor was spun up on a host.

| [[ExecutorLost]] `ExecutorLost`
| <<handleExecutorLost, handleExecutorLost>>
| Posted to notify link:spark-scheduler-DAGScheduler.adoc#executorLost[`DAGScheduler` that an executor was lost].
...
| <<GettingResultEvent, GettingResultEvent>> | |  link:spark-scheduler-TaskSetManager.adoc[TaskSetManager] informs `DAGScheduler` (through link:spark-scheduler-DAGScheduler.adoc#taskGettingResult[taskGettingResult]) that a task has completed and results are being fetched remotely.

| <<JobCancelled, JobCancelled>> | <<handleJobCancellation, handleJobCancellation>> | `DAGScheduler` was requested to link:spark-scheduler-DAGScheduler.adoc#cancelJob[cancel a job].

| <<JobGroupCancelled, JobGroupCancelled>> | <<handleJobGroupCancelled, handleJobGroupCancelled>> | `DAGScheduler` was requested to link:spark-scheduler-DAGScheduler.adoc#cancelJobGroup[cancel a job group].

| [[JobSubmitted]] `JobSubmitted`
| <<handleJobSubmitted, handleJobSubmitted>>
| Posted when `DAGScheduler` is requested to link:spark-scheduler-DAGScheduler.adoc#submitJob[submit a job] or link:spark-scheduler-DAGScheduler.adoc#runApproximateJob[run an approximate job].
...
6. The link:spark-scheduler-JobListener.adoc[JobListener] to inform about the status of the stage.

7. `Properties` of the execution

| [[MapStageSubmitted]] `MapStageSubmitted`
| <<handleMapStageSubmitted, handleMapStageSubmitted>>
| Posted to inform `DAGScheduler` that link:spark-SparkContext.adoc#submitMapStage[`SparkContext` submitted a `MapStage` for execution] (through link:spark-scheduler-DAGScheduler.adoc#submitMapStage[submitMapStage]).
...
| <<ResubmitFailedStages, ResubmitFailedStages>> | <<resubmitFailedStages, resubmitFailedStages>> | `DAGScheduler` was informed that a link:spark-scheduler-DAGScheduler.adoc#handleTaskCompletion-FetchFailed[task has failed due to `FetchFailed` exception].

| <<StageCancelled, StageCancelled>> | <<handleStageCancellation, handleStageCancellation>> | `DAGScheduler` was requested to link:spark-scheduler-DAGScheduler.adoc#cancelStage[cancel a stage].

| <<TaskSetFailed, TaskSetFailed>> | <<handleTaskSetFailed, handleTaskSetFailed>> | `DAGScheduler` was requested to link:spark-scheduler-DAGScheduler.adoc#taskSetFailed[cancel a `TaskSet`]

|===

When created, `DAGSchedulerEventProcessLoop` gets the reference to the owning link:spark-scheduler-DAGScheduler.adoc[DAGScheduler] that it uses to call event handler methods on.
...
NOTE: `GettingResultEvent` is posted to inform `DAGScheduler` (through link:spark-scheduler-DAGScheduler.adoc#taskGettingResult[taskGettingResult]) that a link:spark-scheduler-TaskSetManager.adoc#handleTaskGettingResult[task fetches results].
...
`handleGetTaskResult` merely posts link:spark-SparkListener.adoc#SparkListenerTaskGettingResult[SparkListenerTaskGettingResult] (to link:spark-scheduler-DAGScheduler.adoc#listenerBus[`LiveListenerBus` Event Bus]).
...
NOTE: `BeginEvent` is posted to inform `DAGScheduler` (through link:spark-scheduler-DAGScheduler.adoc#taskStarted[taskStarted]) that a link:spark-scheduler-TaskSetManager.adoc#resourceOffer[`TaskSetManager` starts a task].
...
`handleBeginEvent` looks the stage of `task` up in link:spark-scheduler-DAGScheduler.adoc#stageIdToStage[stageIdToStage] internal registry to compute the last attempt id (or `-1` if not available) and posts link:spark-SparkListener.adoc#SparkListenerTaskStart[SparkListenerTaskStart] (to link:spark-scheduler-DAGScheduler.adoc#listenerBus[listenerBus] event bus).
...
NOTE: `JobGroupCancelled` is posted when `DAGScheduler` is informed (through link:spark-scheduler-DAGScheduler.adoc#cancelJobGroup[cancelJobGroup]) that link:spark-SparkContext.adoc#cancelJobGroup[`SparkContext` was requested to cancel a job group].
...
Internally, `handleJobGroupCancelled` computes all the active jobs (registered in the internal link:spark-scheduler-DAGScheduler.adoc#activeJobs[collection of active jobs]) that have `spark.jobGroup.id` scheduling property set to `groupId`.
...
`handleMapStageSubmitted` link:spark-scheduler-DAGScheduler.adoc#getOrCreateShuffleMapStage[finds or creates a new `ShuffleMapStage`] for the input link:spark-rdd-ShuffleDependency.adoc[ShuffleDependency] and `jobId`.
...
`handleMapStageSubmitted` link:spark-scheduler-DAGScheduler.adoc#clearCacheLocs[clears the internal cache of RDD partition locations].
...
`handleMapStageSubmitted` registers the new job in link:spark-scheduler-DAGScheduler.adoc#jobIdToActiveJob[jobIdToActiveJob] and link:spark-scheduler-DAGScheduler.adoc#activeJobs[activeJobs] internal registries, and link:spark-scheduler-ShuffleMapStage.adoc#addActiveJob[with the final `ShuffleMapStage`].

NOTE: `ShuffleMapStage` can have multiple ``ActiveJob``s registered.

`handleMapStageSubmitted` link:spark-scheduler-DAGScheduler.adoc#jobIdToStageIds[finds all the registered stages for the input `jobId`] and collects link:spark-scheduler-Stage.adoc#latestInfo[their latest `StageInfo`].

Ultimately, `handleMapStageSubmitted` posts link:spark-SparkListener.adoc#SparkListenerJobStart[SparkListenerJobStart] message to link:spark-scheduler-LiveListenerBus.adoc[LiveListenerBus] and link:spark-scheduler-DAGScheduler.adoc#submitStage[submits the `ShuffleMapStage`].

In case the link:spark-scheduler-ShuffleMapStage.adoc#isAvailable[`ShuffleMapStage` could be available] already, `handleMapStageSubmitted` link:spark-scheduler-DAGScheduler.adoc#markMapStageJobAsFinished[marks the job finished].
...
NOTE: `MapOutputTrackerMaster` is passed in when link:spark-scheduler-DAGScheduler.adoc#creating-instance[`DAGScheduler` is created].
...
NOTE: `TaskSetFailed` is posted when link:spark-scheduler-DAGScheduler.adoc#taskSetFailed[`DAGScheduler` is requested to cancel a `TaskSet`].
...
`handleTaskSetFailed` looks the stage (of the input `taskSet`) up in the internal <<stageIdToStage, stageIdToStage>> registry and link:spark-scheduler-DAGScheduler.adoc#abortStage[aborts] it.
...
`resubmitFailedStages` iterates over the internal link:spark-scheduler-DAGScheduler.adoc#failedStages[collection of failed stages] and link:spark-scheduler-DAGScheduler.adoc#submitStage[submits] them.

NOTE: `resubmitFailedStages` does nothing when there are no link:spark-scheduler-DAGScheduler.adoc#failedStages[failed stages reported].
...
`resubmitFailedStages` link:spark-scheduler-DAGScheduler.adoc#clearCacheLocs[clears the internal cache of RDD partition locations] first. It then makes a copy of the link:spark-scheduler-DAGScheduler.adoc#failedStages[collection of failed stages] so `DAGScheduler` can track failed stages afresh.

NOTE: At this point `DAGScheduler` has no failed stages reported.

The previously-reported failed stages are sorted by the corresponding job ids in incremental order and link:spark-scheduler-DAGScheduler.adoc#submitStage[resubmitted].
...
NOTE: `MapOutputTrackerMaster` is passed in (as `mapOutputTracker`) when link:spark-scheduler-DAGScheduler.adoc#creating-instance[`DAGScheduler` is created].
...
Otherwise, when the executor `execId` is not in the link:spark-scheduler-DAGScheduler.adoc#failedEpoch[list of executor lost] or the executor failure's epoch is smaller than the input `maybeEpoch`, the executor's lost event is recorded in link:spark-scheduler-DAGScheduler.adoc#failedEpoch[`failedEpoch` internal registry].
...
`handleExecutorLost` walks over all link:spark-scheduler-ShuffleMapStage.adoc[ShuffleMapStage]s in link:spark-scheduler-DAGScheduler.adoc#shuffleToMapStage[DAGScheduler's `shuffleToMapStage` internal registry] and do the following (in order):
...
In case link:spark-scheduler-DAGScheduler.adoc#shuffleToMapStage[DAGScheduler's `shuffleToMapStage` internal registry] has no shuffles registered,  link:spark-service-MapOutputTrackerMaster.adoc#incrementEpoch[`MapOutputTrackerMaster` is requested to increment epoch].

Ultimatelly, `DAGScheduler` link:spark-scheduler-DAGScheduler.adoc#clearCacheLocs[clears the internal cache of RDD partition locations].
...
NOTE: `JobCancelled` is posted when link:spark-scheduler-DAGScheduler.adoc#cancelJob[`DAGScheduler` is requested to cancel a job].
...
`handleJobCancellation` first makes sure that the input `jobId` has been registered earlier (using link:spark-scheduler-DAGScheduler.adoc#jobIdToStageIds[jobIdToStageIds] internal registry).
...
Otherwise, `handleJobCancellation` link:spark-scheduler-DAGScheduler.adoc#failJobAndIndependentStages[fails the active job and all independent stages] (by looking up the active job using link:spark-scheduler-DAGScheduler.adoc#jobIdToActiveJob[jobIdToActiveJob]) with failure reason:
...
| `task`
| Completed link:spark-scheduler-Task.adoc[Task] instance for a stage, partition and stage attempt.
...
`handleTaskCompletion` announces task completion application-wide (by posting a link:spark-SparkListener.adoc#SparkListenerTaskEnd[SparkListenerTaskEnd] to link:spark-scheduler-LiveListenerBus.adoc[LiveListenerBus]).

`handleTaskCompletion` checks the stage of the task out in the link:spark-scheduler-DAGScheduler.adoc#stageIdToStage[`stageIdToStage` internal registry] and if not found, it simply exits.
...
| `ExceptionFailure`
| link:spark-scheduler-DAGScheduler.adoc#updateAccumulators[Updates accumulators] (with partial values from the task).
...
For link:spark-scheduler-ResultTask.adoc[ResultTask], the stage is assumed a link:spark-scheduler-ResultStage.adoc[ResultStage].
...
`handleTaskCompletion` link:spark-scheduler-DAGScheduler.adoc#updateAccumulators[updates accumulators].
...
1. link:spark-scheduler-DAGScheduler.adoc#markStageAsFinished[Marks `ResultStage` computed].
2. link:spark-scheduler-DAGScheduler.adoc#cleanupStateForJobAndIndependentStages[Cleans up after `ActiveJob` and independent stages].
3. Announces the job completion application-wide (by posting a link:spark-SparkListener.adoc#SparkListenerJobEnd[SparkListenerJobEnd] to link:spark-scheduler-LiveListenerBus.adoc[LiveListenerBus]).
...
For link:spark-scheduler-ShuffleMapTask.adoc[ShuffleMapTask], the stage is assumed a  link:spark-scheduler-ShuffleMapStage.adoc[ShuffleMapStage].
...
If the executor is registered in link:spark-scheduler-DAGScheduler.adoc#failedEpoch[`failedEpoch` internal registry] and the epoch of the completed task is not greater than that of the executor (as in `failedEpoch` registry), you should see the following INFO message in the logs:
...
`handleTaskCompletion` does more processing only if the `ShuffleMapStage` is registered as still running (in link:spark-scheduler-DAGScheduler.adoc#runningStages[`runningStages` internal registry]) and the link:spark-scheduler-Stage.adoc#pendingPartitions[`ShuffleMapStage` stage has no pending partitions to compute].
...
Eventually, `handleTaskCompletion` link:spark-scheduler-DAGScheduler.adoc#submitWaitingChildStages[submits waiting child stages (of the ready `ShuffleMapStage`)].
...
In the end, `handleTaskCompletion` link:spark-scheduler-DAGScheduler.adoc#submitStage[submits the `ShuffleMapStage` for execution].
...
If the link:spark-scheduler-Stage.adoc#failedOnFetchAndShouldAbort[number of fetch failed attempts for the stage exceeds the allowed number], the link:spark-scheduler-DAGScheduler.adoc#abortStage[failed stage is aborted] with the reason:

```
[failedStage] ([name]) has failed the maximum allowable number of times: 4. Most recent failure reason: [failureMessage]
```

If there are no failed stages reported (link:spark-scheduler-DAGScheduler.adoc#failedStages[DAGScheduler.failedStages] is empty), the following INFO shows in the logs:
...
For all the cases, the failed stage and map stages are both added to the internal link:spark-scheduler-DAGScheduler.adoc#failedStages[registry of failed stages].
...
If `BlockManagerId` (as `bmAddress` in the `FetchFailed` object) is defined, `handleTaskCompletion` <<handleExecutorLost, notifies `DAGScheduler` that an executor was lost>> (with `filesLost` enabled and `maybeEpoch` from the link:spark-scheduler-Task.adoc#epoch[Task] that completed).
...
`handleStageCancellation` checks if the input `stageId` was registered earlier (in the internal link:spark-scheduler-DAGScheduler.adoc#stageIdToStage[stageIdToStage] registry) and if it was attempts to <<handleJobCancellation, cancel the associated jobs>> (with "because Stage [stageId] was cancelled" cancellation reason).
...
`handleJobSubmitted` link:spark-scheduler-DAGScheduler.adoc#createResultStage[creates a new `ResultStage`] (as `finalStage` in the picture below) given the input `finalRDD`, `func`, `partitions`, `jobId` and `callSite`.
...
`handleJobSubmitted` link:spark-scheduler-DAGScheduler.adoc#clearCacheLocs[clears the internal cache of RDD partition locations].
...
`handleJobSubmitted` then registers the new job in link:spark-scheduler-DAGScheduler.adoc#jobIdToActiveJob[jobIdToActiveJob] and link:spark-scheduler-DAGScheduler.adoc#activeJobs[activeJobs] internal registries, and link:spark-scheduler-ResultStage.adoc#setActiveJob[with the final `ResultStage`].

NOTE: `ResultStage` can only have one `ActiveJob` registered.

`handleJobSubmitted` link:spark-scheduler-DAGScheduler.adoc#jobIdToStageIds[finds all the registered stages for the input `jobId`] and collects link:spark-scheduler-Stage.adoc#latestInfo[their latest `StageInfo`].

Ultimately, `handleJobSubmitted` posts  link:spark-SparkListener.adoc#SparkListenerJobStart[SparkListenerJobStart] message to link:spark-scheduler-LiveListenerBus.adoc[LiveListenerBus] and link:spark-scheduler-DAGScheduler.adoc#submitStage[submits the stage].
...
`handleExecutorAdded` checks if the input `execId` executor was registered in link:spark-scheduler-DAGScheduler.adoc#failedEpoch[failedEpoch] and, if it was, removes it from the `failedEpoch` registry.
