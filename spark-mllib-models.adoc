...
NOTE: An `Estimator` is optional and is available only after `fit` (of link:spark-mllib-pipelines.adoc#Pipeline[Pipeline]) has been executed.
...
* `ProbabilisticClassificationModel` (the `abstract` parent of the other classification models)
