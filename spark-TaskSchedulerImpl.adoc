...
NOTE: `submitTasks` is part of the <<spark-TaskScheduler.adoc#submitTasks, TaskScheduler Contract>> to submit the tasks (of the given <<spark-taskscheduler-tasksets.adoc#, TaskSet>>) for execution.

In essence, `submitTasks` registers a new <<spark-TaskSetManager.adoc#, TaskSetManager>> for the given <<spark-taskscheduler-tasksets.adoc#, TaskSet>> and requests the <<backend, SchedulerBackend>> to <<spark-SchedulerBackend.adoc#reviveOffers, handle resource allocation offers (from the scheduling system)>>.
...
`submitTasks` then <<createTaskSetManager, creates a TaskSetManager>> (for the given <<spark-taskscheduler-tasksets.adoc#, TaskSet>> and the <<maxTaskFailures, acceptable number of task failures>>).

`submitTasks` registers (_adds_) the `TaskSetManager` per <<spark-taskscheduler-tasksets.adoc#stageId, stage>> and <<spark-taskscheduler-tasksets.adoc#stageAttemptId, stage attempt>> IDs (of the <<spark-taskscheduler-tasksets.adoc#, TaskSet>>) in the <<taskSetsByStageIdAndAttempt, taskSetsByStageIdAndAttempt>> internal registry.

NOTE: <<taskSetsByStageIdAndAttempt, taskSetsByStageIdAndAttempt>> internal registry tracks the <<spark-TaskSetManager.adoc#, TaskSetManagers>> (that represent <<spark-taskscheduler-tasksets.adoc#, TaskSets>>) per stage and stage attempts. In other words, there could be many `TaskSetManagers` for a single stage, each representing a unique stage attempt.
...
`submitTasks` requests the <<schedulableBuilder, SchedulableBuilder>> to link:spark-taskscheduler-schedulablebuilders.adoc#addTaskSetManager[add the TaskSetManager to the schedulable pool].
