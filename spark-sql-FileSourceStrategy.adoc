...
`FileSourceStrategy` is a `Strategy` that uses a <<PhysicalOperation, PhysicalOperation>> to destructure and then optimize a link:spark-sql-catalyst-LogicalPlan.adoc[LogicalPlan].
...
`PhysicalOperation` is used to destructure a link:spark-sql-catalyst-LogicalPlan.adoc[LogicalPlan] into a tuple of `(Seq[NamedExpression], Seq[Expression], LogicalPlan)`.
...
`collectProjectsAndFilters` is a pattern used to destructure a link:spark-sql-catalyst-LogicalPlan.adoc[LogicalPlan] that can be `Project`, `Filter` or `BroadcastHint`. Any other `LogicalPlan` give an _all-empty_ response.
