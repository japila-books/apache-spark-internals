...
You can submit your Spark applications to YARN using `yarn` <<masterURL, master URL>> in <<client-mode, client>> (default) or <<cluster-mode, cluster>> deploy modes.
