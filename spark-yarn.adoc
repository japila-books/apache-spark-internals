...
`getInitialTargetExecutorNumber` calculates the initial number of executors for Spark on YARN. It varies by whether link:spark-dynamic-allocation.adoc[dynamic allocation] is enabled or not (by calling link:spark-dynamic-allocation.adoc#isDynamicAllocationEnabled[Utils.isDynamicAllocationEnabled]).
