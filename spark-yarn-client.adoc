...
=== [[populateClasspath]] populateClasspath method
...
`populateClasspath` is a `private[yarn]` helper method that...

CAUTION: FIXME

NOTE: It is used when link:spark-yarn-ExecutorRunnable.adoc#prepareEnvironment[`ExecutorRunnable` prepares environment] and <<setupLaunchEnv, Client sets up the environment to launch a `ApplicationMaster` container>>.
