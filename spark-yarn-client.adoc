...
`run` <<submitApplication, submits a Spark application>> to link:spark-yarn-introduction.adoc[YARN ResourceManager].

=== [[submitApplication]] submitApplication

`submitApplication` submits a Spark application to YARN ResourceManager and returns its unique https://hadoop.apache.org/docs/current/api/org/apache/hadoop/yarn/api/records/ApplicationId.html[ApplicationId].

NOTE: `submitApplication` is called from <<run, run>>.
