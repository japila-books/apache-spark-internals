...
NOTE: The example uses Spark in link:spark-local.adoc[local mode], i.e. `setMaster("local[*]")`, but the initialization with link:spark-cluster.adoc[the other cluster modes] is similar.
...
`_executorMemory` is set to `spark.executor.memory` setting, or `SPARK_EXECUTOR_MEMORY` environment variable, or  (now deprecated) `SPARK_MEM` environment variable, or eventually `1024`
