...
`requestTotalExecutors` is a `private[spark]` method that link:spark-scheduler-backends-coarse-grained.adoc#requestTotalExecutors[requests the exact number of executors from a cluster manager].

It works for link:spark-scheduler-backends-coarse-grained.adoc[coarse-grained scheduler backends] only.
...
* Posts a application end event `SparkListenerApplicationEnd` to link:spark-LiveListenerBus.adoc[LiveListenerBus]
