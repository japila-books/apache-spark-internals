...
`broadcast` method creates a link:spark-broadcast.adoc[broadcast variable] that is a shared memory with `value` (in broadcast blocks) on the driver and all Spark executors.
...
NOTE: The current `BroadcastManager` is available using link:spark-sparkenv.adoc#broadcastManager[`SparkEnv.broadcastManager`] attribute.
