...
*SparkContext* (aka *Spark context*) is where most things in Spark are born.
A SparkContext represents the connection to a Spark execution environment, called *execution mode*, and can only then be used to create RDDs, accumulators and broadcast variables, access Spark services and run jobs.
...
`SparkContext.createTaskScheduler` is executed as part of SparkContext's initialization to create a link:spark-taskscheduler.adoc[task scheduler] (based on a <<master-urls, given master URL>>) and link:spark-execution-model.adoc#scheduler-backends[scheduler backend].
