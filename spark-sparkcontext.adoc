...
NOTE: `spark.driver.host` and `spark.driver.port` are expected to be set on the driver. It is later asserted by link:spark-runtime-environment.adoc#createDriverEnv[SparkEnv.createDriverEnv].
