...
`SparkContext.createTaskScheduler` is executed as part of SparkContext's initialization. It is the moment when Spark instantiates a link:spark-execution-model.adoc#scheduler-backends[scheduler backend] and link:spark-taskscheduler.adoc[task scheduler].
