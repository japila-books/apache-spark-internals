...
`broadcast` method creates a link:spark-broadcast.adoc[broadcast variable] that is a shared memory with `value` (as broadcast blocks) on the driver and later on all Spark executors.
