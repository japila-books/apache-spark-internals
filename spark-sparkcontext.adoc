...
*Default level of parallelism* is used for the number of link:spark-rdd-partitions.adoc[partitions] when not specified explicitly by a user.

It is used for the methods `SparkContext.parallelize`, `SparkContext.range` and `SparkContext.makeRDD` (as well as Spark Streaming's `DStream.countByValue` and `DStream.countByValueAndWindow` and few other places). It is also used to instantiate HashPartitioner or for the minimum number of partitions in Hadoop RDDs.

SparkContext queries link:spark-taskscheduler.adoc[TaskScheduler] for the value (refer to link:spark-taskscheduler.adoc#defaultParallelism[Default Level of Parallelism] for TaskScheduler).
