...
A link:spark-runtime-environment.adoc#SparkEnv[Spark execution environment] is created (using `createSparkEnv` that in turn calls link:spark-runtime-environment.adoc#createDriverEnv[SparkEnv.createDriverEnv]).
