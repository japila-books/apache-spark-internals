...
`SparkContext` offers a developer API for link:spark-dynamic-allocation.adoc[dynamic allocation of executors]:

* <<requestExecutors, requestExecutors>>
* <<killExecutors, killExecutors>>
* (private!) <<requestTotalExecutors, requestTotalExecutors>>
...
==== [[requestExecutors]] requestExecutors method
...
==== [[requestTotalExecutors]] requestTotalExecutors method
...
NOTE: It is used to link:spark-taskscheduler-FairSchedulableBuilder.adoc#spark.scheduler.pool[group jobs into pools in FAIR job scheduler by spark.scheduler.pool per-thread property] and in link:spark-sql-sqlcontext.adoc#SQLExecution-withNewExecutionId[SQLExecution.withNewExecutionId Helper Methods]
...
=== [[creating-accumulators]][[accumulator]] Creating Accumulators (accumulator method)
...
`name` input parameter allows you to give a name to accumulators and have them displayed in link:spark-webui-StagePage.adoc#accumulators[Spark UI] (under Stages tab for a given stage).
...
* It clears link:spark-yarn-client.adoc#SPARK_YARN_MODE[SPARK_YARN_MODE flag].
...
NOTE: You can also register custom listeners using link:spark-LiveListenerBus.adoc#spark.extraListeners[spark.extraListeners] setting.
