...
* Accessing services, e.g. link:spark-taskscheduler.adoc[Task Scheduler], link:spark-scheduler-listeners.adoc[Listener Bus], link:spark-blockmanager.adoc[Block Manager], link:spark-scheduler-backends.adoc[Scheduler Backends], link:spark-shuffle-service.adoc[Shuffle Service].
...
It cleans the `processPartition` input function argument and returns an instance of <<FutureAction, SimpleFutureAction>> that holds the link:link:spark-dagscheduler.adoc#JobWaiter[JobWaiter] instance (it has received from `DAGScheduler.submitJob`).
...
* Posts a application end event `SparkListenerApplicationEnd` to link:spark-scheduler-listeners.adoc[listenerBus]
...
* Stops link:spark-scheduler-listeners.adoc[listenerBus]
...
The amount of memory to allocate to each executor in Mb (as `_executorMemory`) is calculated. It is the value of link:spark-executor.adoc#settings[spark.executor.memory] setting, or `SPARK_EXECUTOR_MEMORY` environment variable, or (now deprecated) `SPARK_MEM` environment variable, or eventually `1024`.
