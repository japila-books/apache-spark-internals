...
`listenerBus` is a <<LiveListenerBus, LiveListenerBus>> object that acts as a mechanism to announce events to other services on the link:spark-driver.adoc[driver].
...
It <<setupAndStartListenerBus, registers user-defined listeners and starts event delivery to the listeners>>.
...
`setupAndStartListenerBus` is an internal method that reads link:spark-scheduler-listeners.adoc#spark.extraListeners[spark.extraListeners] setting from the current link:spark-configuration.adoc[SparkConf] to create and register <<SparkListenerInterface, SparkListenerInterface>> listeners.

It expects that the class name represents a `SparkListenerInterface` listener with a constructor taking a `SparkConf` or no arguments and <<ListenerBus-addListener, register>> it to <<LiveListenerBus, LiveListenerBus>>. You should see the following INFO message in the logs:

```
INFO Registered listener [className]
```

It <<LiveListenerBus-start, starts LiveListenerBus>> and records it in the internal `_listenerBusStarted`.
...
Any exception while registering a `SparkListenerInterface` listener <<stop, stops the SparkContext>> and a `SparkException` is thrown and the source exception's message.
