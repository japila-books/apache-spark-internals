...
*SparkContext* (aka *Spark context*) represents the connection to a link:spark-deployment-modes.adoc[Spark execution environment (deployment mode)], and can only then be used to <<creating-rdds, create RDDs>>, <<creating-accumulators, accumulators>> and <<creating-broadcasts, broadcast variables>>, access Spark services and <<running-jobs, run jobs>>.
...
`ConsoleProgressBar` shows the progress of active stages in console (to `stderr`). It polls the status of stages from <<SparkStatusTracker, SparkStatusTracker>> periodically and prints out active stages with more than one task. It keeps overwriting itself to hold in one line. The number of stages displayed at a time is at most 3.
...
It is only instantiated if the value of the boolean property `spark.ui.showConsoleProgress` (default: `true`) is `true` and the `INFO` log level of `org.apache.spark.SparkContext` logger is disabled (refer to link:spark-logging.adoc[Logging]).
...
It reads the current user name, i.e. a value of `SPARK_USER` environment variable or the currently logged-in user. It is available as `sparkUser`.
...
`postEnvironmentUpdate` is called to post `SparkListenerEnvironmentUpdate` event over link:spark-scheduler-listeners.adoc[Listener Bus] with information about Task Scheduler's scheduling mode, added jar and file paths, and other environmental details. They are eventually visible in Web UI's Environment tab.

`postApplicationStart` is called to post `SparkListenerApplicationStart` event over link:spark-scheduler-listeners.adoc[Listener Bus].
...
A new instance of Spark context is created.
