...
Once a <<creating-instance, `SparkContext` instance is created>> you can use it to <<creating-rdds, create RDDs>>, <<creating-accumulators, accumulators>> and <<creating-broadcast-variables, broadcast variables>>, access Spark services and <<runJob, run jobs>> (until `SparkContext` is <<stop, stopped>>).
...
8. link:spark-scheduler-listeners-eventlogginglistener.adoc#stop[Requests `EventLoggingListener` to stop].
9. link:spark-dagscheduler.adoc#stop[Requests `DAGScheduler` to stop].
10. link:spark-rpc.adoc#stop[Requests RpcEnv to stop `HeartbeatReceiver` endpoint].
11. link:spark-sparkcontext-ConsoleProgressBar.adoc#stop[Requests `ConsoleProgressBar` to stop].
12. Clears the reference to `TaskScheduler`, i.e. `_taskScheduler` is `null`.
13. link:spark-sparkenv.adoc#stop[Requests `SparkEnv` to stop] and clears `SparkEnv`.
