...
* [[deps]] *Parent RDDs*, i.e. <<spark-rdd-dependencies.adoc#, Dependencies>> (`Seq[Dependency[_]]`) that have to be all computed successfully before this RDD
...
. There is at least one <<spark-rdd-dependencies.adoc#rdd, parent RDD>> that has the flag enabled
