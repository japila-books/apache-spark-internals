...
*Window Aggregate Operators* (aka *Windows*) are `Dataset` operators that perform a calculation on a group of records (called a *window*) that are in _some_ relation to the current record. They calculate values for every records in a window.
...
As shown in the example above, you first describe a window using the convenience factory methods in <<Window-object, `Window` object>> that create a <<WindowSpec, window specification>> that you can further refine with *partitioning*, *ordering*, and *frame boundaries*.

Only with a window specification you can use <<functions, window aggregate functions>> like *ranking* functions, *analytic* functions, and *aggregate* functions. For aggregate functions you can use the regular link:spark-sql-aggregation.adoc[aggregate functions], e.g. `sum`, `avg`, `max`.
...
```
scala> empsalary.select('*, rankByDepname as 'rank).explain(true)
== Parsed Logical Plan ==
'Project [*, rank() windowspecdefinition('depname, 'salary DESC, UnspecifiedFrame) AS rank#2935]
+- LocalRelation [depName#2083, empNo#2084L, salary#2085L]

== Analyzed Logical Plan ==
depName: string, empNo: bigint, salary: bigint, rank: int
Project [depName#2083, empNo#2084L, salary#2085L, rank#2935]
+- Project [depName#2083, empNo#2084L, salary#2085L, rank#2935, rank#2935]
   +- Window [rank(salary#2085L) windowspecdefinition(depname#2083, salary#2085L DESC, ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS rank#2935], [depname#2083], [salary#2085L DESC]
      +- Project [depName#2083, empNo#2084L, salary#2085L]
         +- LocalRelation [depName#2083, empNo#2084L, salary#2085L]
...
== Physical Plan ==
Window [rank(salary#2085L) windowspecdefinition(depname#2083, salary#2085L DESC, ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS rank#2935], [depname#2083], [salary#2085L DESC]
+- *Sort [depname#2083 ASC, salary#2085L DESC], false, 0
   +- Exchange hashpartitioning(depname#2083, 200)
      +- LocalTableScan [depName#2083, empNo#2084L, salary#2085L]
```
