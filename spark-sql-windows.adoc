...
*Window Aggregates* (aka *Windows*) are link:spark-sql-dataset.adoc[Dataset] operators that perform a calculation on a group of records (called a *window*) that are in relation to the current record. They calculate values for every records in a window.

NOTE: Window-based framework is available as an experimental feature since Spark *1.4.0*.

Unlike the link:spark-sql-aggregation.adoc[regular aggregation operators], window aggregates does not group records as a single record.

[source, scala]
----
// Borrowed from 3.5. Window Functions in PostgreSQL documentation
case class Salary(deptName: String, empNo: Long, salary: Long)
val empsalary = Seq(
  Salary("sales", 1, 5000),
...
val byDeptName = Window.partitionBy('deptName)

val salaryAvg = avg('salary).over(byDeptName)
...
As shown in the example, you start by describing a window using the convenience factory methods in <<Window-object, `Window` object>> that create a <<WindowSpec, window specification>> that you can further refine with *partitioning*, *ordering*, and *frame boundaries*.

Only when you have a window specification you can apply <<functions, window aggregate functions>> like *ranking* functions, *analytic* functions, and *aggregate* functions. For aggregate functions you can use the regular link:spark-sql-aggregation.adoc[aggregate functions], e.g. `sum`, `avg`, `max`.
