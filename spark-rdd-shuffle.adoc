...
`join` operation is one of the *cogroup operations* that uses `defaultPartitioner`, i.e. walks through link:spark-rdd.adoc#lineage[the RDD lineage graph] (sorted by the number of partitions decreasing) and picks the partitioner with positive number of output partitions. Otherwise, it checks `spark.default.parallelism` setting and if defined picks link:spark-rdd-partitions.adoc#HashPartitioner[HashPartitioner] with the default parallelism of the link:spark-runtime-environment.adoc#scheduler-backends[scheduler backend].
