...
`PushDownPredicate` is a link:spark-sql-logical-plan.adoc[logical plan] link:spark-sql-catalyst-analyzer.adoc#rule[optimization rule] in *Operator Optimizations* link:spark-sql-catalyst-analyzer.adoc#batch[batch] in the base link:spark-sql-catalyst-Optimizer.adoc[Optimizer].
