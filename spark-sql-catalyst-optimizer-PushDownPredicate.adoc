...
`PushDownPredicate` is a link:spark-sql-LogicalPlan.adoc[logical plan] link:spark-sql-Analyzer.adoc#rule[optimization rule] in *Operator Optimizations* link:spark-sql-Analyzer.adoc#batch[batch] in the base link:spark-sql-catalyst-Optimizer.adoc[Optimizer].
