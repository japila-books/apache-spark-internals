...
NOTE: `makeNewStageAttempt` is used exclusively when link:spark-scheduler-DAGScheduler.adoc#submitMissingTasks[`DAGScheduler` submits missing tasks for a stage].
