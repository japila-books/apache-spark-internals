...
Internally, it requests the link:spark-TaskScheduler.adoc#rootPool[TaskScheduler for the root pool] and link:spark-Schedulable-Pool.adoc#schedulableNameToSchedulable[looks up the `Schedulable` by the `pool` name].
...
`getAllPools` collects the link:spark-Schedulable-Pool.adoc[Pools] in link:spark-TaskScheduler.adoc#contract[TaskScheduler.rootPool].
