...
`InputAdapter` is a link:spark-sql-SparkPlan.adoc#UnaryExecNode[unary physical operator] (i.e. with one <<child, child>> physical operator) that is an adapter for the single child operator that does not support link:spark-sql-CodegenSupport.adoc[whole-stage Java code generation directly] (and has link:spark-sql-CodegenSupport.adoc#supportCodegen[supportCodegen] flag turned off) but participates in such code generation for a structured query.
