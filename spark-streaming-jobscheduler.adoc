...
When `JobScheduler` starts, it starts <<eventLoop, eventLoop - JobSchedulerEvent Handler>>, StreamingListenerBus, link:spark-streaming-receivertracker.adoc[ReceiverTracker], and link:spark-streaming-jobgenerator.adoc[JobGenerator]. It instantiates <<InputInfoTracker, InputInfoTracker>> to track...FIXME
