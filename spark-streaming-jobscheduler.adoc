...
NOTE: JobSets are submitted by link:spark-streaming.adoc#JobGenerator[JobGenerator].
...
If the entire JobSet is completed, it removes it from <<internal-registries, jobSets>>, and calls `jobGenerator.onBatchCompletion`.
