...
A `TaskSetManager` manages execution of the tasks in a single <<taskset, TaskSet>> (after having it been handed over by link:spark-taskscheduler.adoc[TaskScheduler]).
...
`executorLost(execId: String, host: String, reason: ExecutorLossReason)` is part of link:spark-taskscheduler-schedulable.adoc[Schedulable Contract] that `TaskSetManager` follows.
...
* the task gets added to the collection of pending tasks (using <<addPendingTask, addPendingTask>> method)
