...
`DataFrameReader` is an interface to create link:spark-sql-dataframe.adoc[DataFrames] from <<creating-dataframes-from-files, files>>, <<creating-dataframes-from-tables, Hive tables>> or <<jdbc, JDBC>>.
