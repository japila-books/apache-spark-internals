== [[SparkConf]] SparkConf -- Programmable Configuration for Spark Applications
...
Set when `SparkContext` link:spark-sparkcontext-creating-instance-internals.adoc#spark.app.id[is created] (right after `TaskScheduler` link:spark-sparkcontext-creating-instance-internals.adoc#taskScheduler-start[is started] that in turn gives the identifier).
