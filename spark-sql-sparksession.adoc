...
`SparkSession` requires a link:spark-sparkcontext.adoc[SparkContext]. It uses an optional <<SharedState, SharedState>> (that represents the shared state across `SparkSession` instances).
...
CAUTION: FIXME What is `SharedState`?
