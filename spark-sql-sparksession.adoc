...
`SparkSession` is the entry point to developing Spark applications using the type-safe link:spark-sql-dataset.adoc[Dataset] (and untyped link:spark-sql-dataframe.adoc[DataFrame]) data abstractions.
