...
NOTE: `experimental` is used in link:spark-sql-queryplanner.adoc#SparkPlanner[SparkPlanner] and link:spark-sql-catalyst.adoc#SparkOptimizer[SparkOptimizer]. Hive and link:spark-sql-structured-streaming.adoc[Structured Streaming] use it for their own extra strategies and optimization rules.
