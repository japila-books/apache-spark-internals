...
=== [[createDataset]] Creating Dataset from Collections and RDDs (createDataset methods)
...
`createDataset` is an experimental API to create a link:spark-sql-dataset.adoc[Dataset] from a local Scala collection, i.e. `Seq[T]` or Java's `List[T]`, or a distributed `RDD[T]`.
