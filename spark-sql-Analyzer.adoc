...
`Analyzer` is a link:spark-sql-LogicalPlan.adoc[logical query plan] analyzer. It resolves unresolved attributes and relations to typed objects using information in a link:spark-sql-SessionCatalog.adoc[SessionCatalog] and link:spark-sql-FunctionRegistry.adoc[FunctionRegistry].
