== [[Analyzer]] Analyzer -- Spark SQL's Logical Plan Analyzer
...
[[execute]]
`Analyzer` is a link:spark-sql-catalyst-RuleExecutor.adoc[RuleExecutor] (with link:spark-sql-Analyzer-CheckAnalysis.adoc[CheckAnalysis]) that defines the <<batches, batches of rules>> for analyzing a link:spark-sql-LogicalPlan.adoc[LogicalPlan] (i.e. resolving, removing, and in general modifying it).
