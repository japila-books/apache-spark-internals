...
a| Resolves (aka _replaces_) link:spark-sql-Expression-WindowExpression.adoc[WindowExpressions]:

1. `ResolveWindowFrame` makes sure that the window frame of a `WindowFunction` is unspecified or matches the `SpecifiedWindowFrame` of the link:spark-sql-Expression-WindowSpecDefinition.adoc[WindowSpecDefinition] expression.
+
