...
`StreamExecution` manages execution of a streaming query for a `SQLContext` and a link:spark-sql-sink.adoc[Sink]. It requires a link:spark-sql-logical-plan.adoc[LogicalPlan] to know the `Source` objects from which records are periodically pulled down.

`StreamExecution` is a link:spark-sql-continuousquery.adoc[ContinuousQuery] with additional attributes:
...
scala> val out = in.write
  ...
  .queryName("memStream")
  .startStream()
out: org.apache.spark.sql.ContinuousQuery = Continuous Query - memStream [state = ACTIVE]
