...
CAUTION: Does `zk://` master URL work? It is accepted in `MESOS_REGEX`.

`CoarseMesosSchedulerBackend` is the default scheduler backend for Spark on Mesos.
...
It is a specialized link:spark-runtime-environment.adoc#CoarseGrainedSchedulerBackend[CoarseGrainedSchedulerBackend] and implements Mesos's org.apache.mesos.Scheduler interface.
