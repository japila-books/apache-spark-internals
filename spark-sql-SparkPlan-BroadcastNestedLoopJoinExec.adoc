...
`BroadcastNestedLoopJoinExec` <<requiredChildDistribution, requires that partition requirements>> for the two child physical operators match `BroadcastDistribution` (with `IdentityBroadcastMode`) and `UnspecifiedDistribution` (for <<left, left>> and <<right, right>> sides of a join or vice versa).
