...
`BroadcastNestedLoopJoinExec` is a link:spark-sql-SparkPlan.adoc#BinaryExecNode[physical operator] (with two child <<left, left>> and <<right, right>> physical operators) that is <<creating-instance, created>> (and converted to) when link:spark-sql-JoinSelection.adoc[JoinSelection] physical plan strategy finds a link:spark-sql-LogicalPlan-Join.adoc[Join] logical operator that meets either case:

1. link:spark-sql-JoinSelection.adoc#canBuildRight[canBuildRight] join type and `right` physical operator link:spark-sql-JoinSelection.adoc#canBroadcast[broadcastable]
1. link:spark-sql-JoinSelection.adoc#canBuildLeft[canBuildLeft] join type and `left` link:spark-sql-JoinSelection.adoc#canBroadcast[broadcastable]
1. non-``InnerLike`` join type

[NOTE]
====
link:spark-sql-JoinSelection.adoc#canBuildRight[canBuildRight] join types are:

* CROSS, INNER, LEFT ANTI, LEFT OUTER, LEFT SEMI or Existence

link:spark-sql-JoinSelection.adoc#canBuildLeft[canBuildLeft] join types are:
