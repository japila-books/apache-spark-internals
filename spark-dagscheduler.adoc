...
=== [[submitMapStage]] `submitMapStage` Method
...
`taskEnded` simply posts a <<CompletionEvent, CompletionEvent>> event to the <<eventProcessLoop, DAGScheduler's internal event loop>>.
