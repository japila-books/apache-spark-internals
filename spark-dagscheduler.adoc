...
`getOrCreateParentStages` <<getShuffleDependencies, finds all `ShuffleDependency` immediate parents>> and <<getOrCreateShuffleMapStage, finds ``ShuffleMapStage``s (and creates them if not available)>> for each link:spark-rdd-ShuffleDependency.adoc[ShuffleDependency].
...
=== [[getMissingAncestorShuffleDependencies]] Finding...FIXME -- `getMissingAncestorShuffleDependencies` Internal Method
...
NOTE: `getMissingAncestorShuffleDependencies` is used when `DAGScheduler` <<getOrCreateShuffleMapStage, finds ``ShuffleMapStage``s for a `ShuffleDependency` (and creates them if not available)>>.
...
=== [[getOrCreateShuffleMapStage]] Finding or Creating New `ShuffleMapStage` for `ShuffleDependency` -- `getOrCreateShuffleMapStage` Internal Method
...
If no `ShuffleDependency` was available, `getOrCreateShuffleMapStage` <<getMissingAncestorShuffleDependencies, finds the ancestor shuffle dependencies (that are not registered in `shuffleToMapStage` registry yet)>> and <<createShuffleMapStage, creates them>> (including one for the input `shuffleDep`).

NOTE: All the newly-created ``ShuffleMapStage``s are accociated with the input `firstJobId`.
...
NOTE: `createShuffleMapStage` is executed only when `DAGScheduler` <<getOrCreateShuffleMapStage, finds  ``ShuffleMapStage``s for a `ShuffleDependency` (and creates them if not available)>>.
...
Internally, `getShuffleDependencies` takes the link:spark-rdd.adoc#dependencies[dependencies of the input RDD] and records all parent shuffle dependencies. If however the direct parent RDD is a link:spark-rdd-NarrowDependency.adoc[NarrowDependency] `getShuffleDependencies` follows along the dependency chain until it finds the closest parent `ShuffleDependency` and includes in the result.
...
For each `ShuffleDependency`, `getMissingParentStages` <<getOrCreateShuffleMapStage, finds ``ShuffleMapStage``s (and creates them if not available)>>. If the `ShuffleMapStage` is not _available_, it is added to the set of missing (map) stages.
