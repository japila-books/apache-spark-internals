...
`getOrCreateParentStages` <<getShuffleDependencies, computes `ShuffleDependency` immediate parents>> and <<getOrCreateShuffleMapStage, gets or creates a `ShuffleMapStage`>> for each link:spark-rdd-ShuffleDependency.adoc[ShuffleDependency].
...
=== [[getOrCreateShuffleMapStage]] Finding `ShuffleMapStage` for `ShuffleDependency` (Possibly Creating One If Not Available) -- `getOrCreateShuffleMapStage` Internal Method
...
NOTE: `createShuffleMapStage` is executed only when `DAGScheduler` <<getOrCreateShuffleMapStage, creates a missing `ShuffleMapStage` for a `ShuffleDependency`>>.
