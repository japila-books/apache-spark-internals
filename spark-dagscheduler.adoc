...
If the link:spark-ExternalShuffleService.adoc[external shuffle service] is not used or the `ExecutorLost` event was for a map output fetch operation, all ShuffleMapStages (using `shuffleToMapStage`) are called (in order):
