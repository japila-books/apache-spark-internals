...
`submitJob` creates a <<JobWaiter, JobWaiter>> and <<JobSubmitted, posts a `JobSubmitted` event>>.
...
4. Posts <<JobSubmitted, JobSubmitted>> event and returns a <<JobWaiter, JobWaiter>>.
...
NOTE: `submitJob` is called by `SparkContext.submitJob` and <<runJob, DAGScheduler.runJob>>.
...
* `shuffleToMapStage` for a mapping between ids to link:spark-dagscheduler-ShuffleMapStage.adoc[ShuffleMapStages]
...
`DAGScheduler.eventProcessLoop` (of type `DAGSchedulerEventProcessLoop`) - is the event process loop to which Spark (by <<submitJob, submitJob>>) posts jobs to schedule their execution. Later on, link:spark-tasksetmanager.adoc[TaskSetManager] talks back to DAGScheduler to inform about the status of the tasks using the same "communication channel".
...
* <<MapStageSubmitted, MapStageSubmitted>> - posted when a link:spark-dagscheduler-ShuffleMapStage.adoc[ShuffleMapStage] is submitted (via `submitMapStage`).
...
It creates a new link:spark-dagscheduler-ResultStage.adoc[ResultStage] (as `finalStage` on the picture) and instantiates `ActiveJob`.
...
==== [[CompletionEvent]][[handleTaskCompletion]] `CompletionEvent` and `handleTaskCompletion` Internal Method

`CompletionEvent` event informs `DAGScheduler` about task completions. It is handled by `handleTaskCompletion`.
...
=== [[submitMissingTasks]] submitMissingTasks for Stage and Job (submitMissingTasks method)
