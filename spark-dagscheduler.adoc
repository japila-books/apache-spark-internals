...
It starts with the target RDD of `stage` given as an input parameter and <<internal-registries, cacheLocs>> mapping is checked by the RDD's id.
...
If link:spark-rdd-caching.adoc[a storage level of an RDD is NONE], there is no caching and hence whenever asked DAGScheduler returns a collection with empty-location elements for each partition.

Otherwise, a collection of `RDDBlockId` instances for each partition is created and link:spark-blockmanager.adoc[BlockManager] is asked for locations (using `BlockManagerMaster.getLocations`). The result is then mapped to a collection of `TaskLocation` for host and executor id.
