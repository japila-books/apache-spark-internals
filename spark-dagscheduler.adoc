...
| `cacheLocs` | The internal cache of partition locations per RDD.

See <<cache-tracking, Cache Tracking>>.
| [[failedEpoch]] `failedEpoch` | The lookup table of failed executors and the epoch when the executor failure was caught.
|======================
