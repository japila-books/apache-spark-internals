...
==== [[ExecutorLost]] ExecutorLost and handleExecutorLost

`ExecutorLost(execId)` event triggers execution of `DAGScheduler.handleExecutorLost(execId: String, fetchFailed: Boolean, maybeEpoch: Option[Long] = None)`.

`handleExecutorLost` recognizes two cases for executor being lost - when it has happened during fetch (`fetchFailed` is `true`) or not (`fetchFailed` is `false`).

There can be epoch provided or it is calculated by querying link:spark-service-mapoutputtracker.adoc#MapOutputTrackerMaster[MapOutputTrackerMaster] for current epoch number.

CAUTION: FIXME image with DAGScheduler and MapOutputTrackerMaster, BlockManagerMaster and whoever causes ExecutorLost event.
...
If an external shuffle service is not used or the ExecutorLost event was for a map output fetch operation, all ShuffleMapStages (using `shuffleToMapStage`) are called (in order):
...
If `mapId` (in the `FetchFailed` object for the case) is provided, the map stage output is cleaned up (as it is broken) using `mapStage.removeOutputLoc(mapId, bmAddress)` and `mapOutputTracker.unregisterMapOutput(shuffleId, mapId, bmAddress)` methods.

CAUTION: FIXME What do `mapStage.removeOutputLoc` and `mapOutputTracker.unregisterMapOutput` methods do?

If `bmAddress` (in the `FetchFailed` object for the case) is provided, `handleExecutorLost(bmAddress.executorId, fetchFailed = true, Some(task.epoch))` is called.
