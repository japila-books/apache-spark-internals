...
=== [[cancelStage]] `cancelStage` Method
...
=== [[taskStarted]] `taskStarted` Method

CAUTION: FIXME

=== [[taskGettingResult]] `taskGettingResult` Method
...
NOTE: `taskEnded` method is called by a link:spark-tasksetmanager.adoc[TaskSetManager] to report task completions, failures including.

TIP: Read about `TaskMetrics` in link:spark-taskscheduler-taskmetrics.adoc[TaskMetrics].

=== [[submitMapStage]] Posting `TaskSetFailed` Event -- `submitMapStage` Method
...
`cancelJob` prints the following INFO message and posts a link:spark-dagscheduler-DAGSchedulerEventProcessLoop.adoc#JobCancelled[JobCancelled] to <<eventProcessLoop, eventProcessLoop>>.
...
`eventProcessLoop` is the link:spark-dagscheduler-DAGSchedulerEventProcessLoop.adoc[DAGScheduler event process loop] to which Spark (by <<submitJob, submitJob>>) posts jobs to schedule their execution. Later on, link:spark-tasksetmanager.adoc[TaskSetManager] talks back to `DAGScheduler` to inform about the status of the tasks using the same "communication channel".
...
=== [[submitWaitingStages]] Submitting Waiting Stages -- `submitWaitingStages` Method
