...
=== [[createShuffleMapStage]] Creating `ShuffleMapStage` (for `ShuffleDependency` and `ActiveJob`) -- `createShuffleMapStage` Method
...
Internally, `createShuffleMapStage` first <<getOrCreateParentStages, takes or creates missing parent `ShuffleMapStage` stages of the associated RDD>>.
...
| [[spark_test_noStageRetry]] `spark.test.noStageRetry` | `false` | When enabled (i.e. `true`), link:spark-dagscheduler-DAGSchedulerEventProcessLoop.adoc#handleTaskCompletion-FetchFailed[task failures with `FetchFailed` exceptions] will not cause stage retries, in order to surface the problem. Used for testing.
|======================
