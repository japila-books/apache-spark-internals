...
| [[cacheLocs]] `cacheLocs`
| The internal registry of partition locations per RDD.

Refer to <<cache-tracking, Cache Tracking>>.
...
`getMissingParentStages` starts with the ``stage``'s RDD (as `stage.rdd`) and searches the tree of all parent RDDs to check out for their <<getCacheLocs, uncached partitions>>.

NOTE: An *uncached partition* is a partition that has `Nil` in the <<cacheLocs, internal registry of partition locations per RDD>>, i.e. FIXME

`getMissingParentStages` traverses the parent dependencies of the RDD (as `RDD.dependencies`) and varies its processing based on their type, i.e. link:spark-rdd-dependencies.adoc#ShuffleDependency[ShuffleDependency] or link:spark-rdd-dependencies.adoc#NarrowDependency[NarrowDependency].
...
NOTE: `NarrowDependency` is a dependency that...FIXME

``ShuffleDependency``-ies are what `getMissingParentStages` looks for and processes in a custom way. For each `ShuffleDependency`, `getMissingParentStages` <<getOrCreateShuffleMapStage, takes a `ShuffleMapStage` or creates a new one>>. If the `ShuffleMapStage` is not _available_, it is added to the set of missing (map) stages.
...
=== [[getCacheLocs]][[cache-tracking]] Block Location Discovery -- `getCacheLocs` Internal Method
