...
==== [[CompletionEvent]] taskEnded, CompletionEvent and handleTaskCompletion

`DAGScheduler` comes with `taskEnded` method that posts a `CompletionEvent` event to `eventProcessLoop`.

NOTE: `DAGScheduler.taskEnded` method is called by a TaskSetManager only to report task completions, failures including.
