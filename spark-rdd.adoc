...
*Resilient Distributed Dataset (RDD)* is the primary data abstraction in Apache Spark (often called Spark Core leaving the name Spark to the project not a particular module).

RDD is a resilient and distributed collection of items. One could compare RDD to a Scala collection (that sits on a single JVM) to its distributed variant (that sits on many JVM, often on separate nodes in a cluster).
