...
*Resilient Distributed Dataset (RDD)* is the primary data abstraction in Apache Spark and the core of Spark (often called *Spark Core*).

A RDD is a resilient and distributed collection of records. One could compare RDD to a Scala collection (that sits on a single JVM) to its distributed variant (that sits on many JVM, often on separate nodes in a cluster).
...
The abstract `compute` method computes the link:spark-rdd-partitions.adoc[partition] to produce a collection of values (of type `T`) in the link:spark-taskscheduler-taskcontext.adoc[TaskContext].
