...
RDDs are a container of instructions on how to materialize big (arrays of) distributed data, and how to split it into partitions so Spark (using link:spark-runtime-environment.adoc#executor[executors]) can hold some of them.
...
Enable `spark.logLineage` (assumed: `false`) to see a RDD graph using `RDD.toDebugString` method every time an action is called.
