...
`deserialize` creates a <<DeserializeToObject, `DeserializeToObject` logical plan>> for the input `child` link:spark-sql-logical-plan.adoc[logical plan].
...
`DeserializeToObject` is a link:spark-sql-logical-plan.adoc#UnaryNode[unary logical plan] that takes the input row from the input `child` link:spark-sql-logical-plan.adoc[logical plan] and turns it into the input `outputObjAttr` attribute using the given `deserializer` expression.
