...
`serviceInit` is a part of YARN's `AuxiliaryService` contract and it called when...FIXME

CAUTION: FIXME

CAUTION: FIXME `TransportConf`?

When called, `serviceInit`, `TransportConf` is created for the `shuffle` module that is used to create `ExternalShuffleBlockHandler` (as `blockHandler`).
