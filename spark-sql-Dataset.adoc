...
| [[rdd]] `rdd`
a| Content of the `Dataset` as a (lazily-created) link:spark-rdd.adoc[RDD] with rows of the JVM type `T`.
...
Internally, `rdd` first link:spark-sql-catalyst-serde.adoc#deserialize[creates a new logical plan to deserialize] the Dataset's <<logicalPlan, logical plan>>.

`rdd` then requests `SessionState` to link:spark-sql-SessionState.adoc#executePlan[execute the logical plan] and gets link:spark-sql-QueryExecution.adoc#toRdd[the RDD of binary rows].
...
NOTE: `rdd` is at the "boundary" between the internal binary row format and the JVM type of the dataset.
