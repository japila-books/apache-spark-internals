...
`RandomForestRegressor` is a concrete <<Predictor, Predictor>> for http://en.wikipedia.org/wiki/Random_forest[Random Forest] learning algorithm. It trains `RandomForestRegressionModel` using `DataFrame` with `features` column of `Vector` type.
