...
NOTE: `Dataset[T]` type in Spark 2.0 is a Scala type constructor with the type parameter `T`. So is `Encoder[T]`. `DataFrame` type is a type alias for `Dataset[Row]` that says that there must be a `Encoder[Row]` available which is `RowEncoder` itself.
...
scala> val internalRow = encoder.toRow(Row(1L, "hello"))
internalRow: org.apache.spark.sql.catalyst.InternalRow = [0,1,1800000005,6f6c6c6568]

import org.apache.spark.sql.catalyst.dsl.expressions._

// in spark-shell there are competing implicits
// That's why DslSymbol is used explicitly
val attrs = Seq(DslSymbol('id).long, DslSymbol('name).string)

scala> encoder.resolveAndBind(attrs).fromRow(internalRow)
res2: org.apache.spark.sql.Row = [1,hello]
----
