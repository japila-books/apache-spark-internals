...
`LocalTableScanExec` is a link:spark-sql-catalyst-SparkPlan.adoc#LeafExecNode[leaf physical operator] with no `children` and `producedAttributes` being `outputSet`.
