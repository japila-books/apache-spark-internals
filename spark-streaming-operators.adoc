...
NOTE: `DStream` companion object offers a Scala implicit to convert `DStream[(K, V)]` to `PairDStreamFunctions` with methods on DStreams of key-value pairs, e.g. <<mapWithState, mapWithState>>.
...
It is part of the set of additional operators available on DStreams of key-value pairs, i.e. `DStream[(K, V)]`.

NOTE: `mapWithState` is added through an Scala implicit conversion `DStream.toPairDStreamFunctions`.

`StateSpec.function` creates a `StateSpec` for `mapWithState` operator.

Every `StateSpec.function` is `ClosureCleaner.clean` before being used (and passed along to `StateSpecImpl`).

It creates a link:spark-streaming-mapwithstatedstreams.adoc[MapWithStateDStream] stream.
