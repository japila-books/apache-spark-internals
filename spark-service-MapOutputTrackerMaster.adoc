...
| The queue with requests for map output statuses (as `GetMapOutputMessage`).
...
=== [[MessageLoop]][[run]] MessageLoop Thread
...
`MapOutputTrackerMaster` initializes the <<internal-registries, internal registries and counters>>.
