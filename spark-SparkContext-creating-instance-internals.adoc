...
[[_listenerBus]]
`SparkContext` creates a link:spark-LiveListenerBus.adoc#creating-instance[LiveListenerBus].

[[_statusStore]]
`SparkContext` link:spark-core-AppStatusStore.adoc#createLiveStore[creates a live store] (i.e. the `AppStatusStore` for an active Spark application) and requests <<listenerBus, LiveListenerBus>> to add the link:spark-core-AppStatusStore.adoc#listener[AppStatusListener] to the link:spark-LiveListenerBus.adoc#addToStatusQueue[status queue].
...
[[_env]]
`SparkContext` <<createSparkEnv, creates a SparkEnv>> and requests `SparkEnv` to link:spark-SparkEnv.adoc#set[use the instance as the default SparkEnv].
...
`SparkContext` link:spark-webui-SparkUI.adoc#create[creates a SparkUI] when link:spark-webui-properties.adoc#spark.ui.enabled[spark.ui.enabled] Spark property is enabled (i.e. `true`).
...
`SparkContext` requests the `MetricsSystem` to link:spark-MetricsSystem.adoc#start[start].
...
`SparkContext` requests the `MetricsSystem` to link:spark-MetricsSystem.adoc#getServletHandlers[getServletHandlers] and requests the <<_ui, SparkUI>> to link:spark-webui-WebUI.adoc#attachHandler[attaches them].
