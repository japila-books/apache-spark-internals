...
`org.apache.spark.sql.functions` object offers many built-in functions to process values in link:spark-sql-Column.adoc[Columns] in ``Dataset``s.
...
There are over 300 functions in the `functions` object. Some functions are transformations of link:spark-sql-Column.adoc[Column] objects (or column names) into other `Column` objects or transform `DataFrame` into `DataFrame`.
