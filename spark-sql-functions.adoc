...
There are over 300 functions in the `functions` object.
...
----
<1> Calls the former with `columnName` as a `Column`

`grouping` is an aggregate function that indicates whether a specified column in a GROUP BY list is aggregated or not, i.e. returns `1` for aggregated or `0` for not aggregated.
...
val workshopCities = tmpWorkshopCities union cityNull

scala> workshopCities
  .cube("city", "year")
