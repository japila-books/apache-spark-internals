...
`RelationalGroupedDataset` is a result of executing the untyped operators <<groupBy, groupBy>>, link:spark-sql-multi-dimensional-aggregation.adoc#rollup[rollup] and link:spark-sql-multi-dimensional-aggregation.adoc#cube[cube].
