...
The example shows how to develop a custom Spark Listener. You should read link:../spark-SparkListener.adoc[Spark Listeners -- Intercepting Events from Spark Scheduler] first to understand the motivation for the example.
...
[info] Done packaging.
[success] Total time: 3 s, completed Sep 15, 2016 3:15:21 PM
```
...
```
$ ./bin/spark-shell --conf spark.logConf=true --conf spark.extraListeners=pl.jaceklaskowski.spark.CustomSparkListener --driver-class-path /Users/jacek/dev/sandbox/custom-spark-listener/target/scala-2.11/custom-spark-listener_2.11-1.0.jar
```

Create a link:spark-sql-dataset.adoc#implicits[Dataset] and execute an action like `show` to start a job as follows:

```
scala> spark.read.text("build.sbt").count
