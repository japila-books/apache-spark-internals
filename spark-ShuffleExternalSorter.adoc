...
1. `memoryManager` -- link:spark-TaskMemoryManager.adoc[TaskMemoryManager]
...
`ShuffleExternalSorter` initializes itself as a link:spark-MemoryConsumer.adoc[MemoryConsumer] (with `pageSize` as the minimum of `PackedRecordPointer.MAXIMUM_PAGE_SIZE_BYTES` and link:spark-TaskMemoryManager.adoc#pageSizeBytes[pageSizeBytes], and Tungsten memory mode).
