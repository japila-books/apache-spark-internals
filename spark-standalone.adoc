...
=== [[spark-deply-scheduler-backend]] SparkDeploySchedulerBackend

It is the link:spark-execution-model.adoc#scheduler-backends[scheduler backend] for Spark Standalone.

CAUTION: FIXME `CoarseGrainedSchedulerBackend` and `AppClientListener`

CAUTION: What does `org.apache.spark.executor.CoarseGrainedExecutorBackend` do?
