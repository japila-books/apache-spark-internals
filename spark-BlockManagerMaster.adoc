...
NOTE: The method can only be executed on the driver where link:spark-sparkcontext.adoc#listenerBus[listenerBus] is available.
...
`SparkListenerBlockManagerRemoved(System.currentTimeMillis(), blockManagerId)` is posted to link:spark-sparkcontext.adoc#listenerBus[listenerBus].
