...
`registerBlockManager` then informs the driver about the new `BlockManager` by sending <<RegisterBlockManager, `RegisterBlockManager` to BlockManagerMaster RPC endpoint>> and waits for a confirmation.

If the response comes, you should see the following INFO message in the logs:
...
`updateBlockInfo` sends a `UpdateBlockInfo` message to link:spark-blockmanager-BlockManagerMasterEndpoint.adoc[BlockManagerMaster RPC endpoint] and waits for a response.
