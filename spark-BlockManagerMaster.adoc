...
=== [[getPeers]] Getting All BlockManagers in Cluster -- `getPeers` Method
...
`getPeers` posts `GetPeers(blockManagerId)` message to link:spark-blockmanager-BlockManagerMasterEndpoint.adoc[BlockManagerMaster RPC endpoint] and waits for a response which becomes the return value.
