...
`ShuffleDependency` is a link:spark-rdd-dependencies.adoc[Dependency] of key-value pairs that represents a dependency on the output of a link:spark-dagscheduler-ShuffleMapStage.adoc[shuffle map stage] (i.e. `ShuffleMapStage`).
