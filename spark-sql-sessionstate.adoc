...
`optimizer` is a Spark session's link:spark-sql-catalyst-Optimizer.adoc[Catalyst query optimizer] for link:spark-sql-catalyst-LogicalPlan.adoc[logical query plans].
...
`executePlan` executes the input link:spark-sql-catalyst-LogicalPlan.adoc[LogicalPlan] to produce a link:spark-sql-query-execution.adoc[QueryExecution] in the current link:spark-sql-sparksession.adoc[SparkSession].
