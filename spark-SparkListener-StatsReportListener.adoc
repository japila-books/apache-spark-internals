...
`org.apache.spark.scheduler.StatsReportListener` (see https://spark.apache.org/docs/latest/api/scala/index.html#org.apache.spark.scheduler.StatsReportListener[the listener's scaladoc]) is a link:spark-scheduler-SparkListener.adoc[SparkListener] that logs summary statistics when each stage completes.

`StatsReportListener` listens to link:spark-scheduler-SparkListener.adoc#SparkListenerTaskEnd[SparkListenerTaskEnd] and link:spark-scheduler-SparkListener.adoc#SparkListenerStageCompleted[SparkListenerStageCompleted] events and prints them out at `INFO` logging level.
