...
NOTE: The one and only `Catalog` is <<CatalogImpl, CatalogImpl>>.
...
`CatalogImpl` is the one and only <<contract, Catalog>> that uses a per-session link:spark-sql-SessionCatalog.adoc[SessionCatalog] (through link:spark-sql-SparkSession.adoc[SparkSession]).

.CatalogImpl uses SessionCatalog (through SparkSession)
image::images/spark-sql-CatalogImpl.png[align="center"]

`CatalogImpl` lives in `org.apache.spark.sql.internal` package.
...
NOTE: `cacheTable` is a part of <<contract, Catalog Contract>>.
