...
`TaskInfo` is information about a running task attempt inside a link:spark-scheduler-TaskSet.adoc[TaskSet].

`TaskInfo` is created when:

* link:spark-scheduler-TaskSetManager.adoc#resourceOffer[`TaskSetManager` dequeues a task for execution (given resource offer)] (and records the task as running)
...
* [[taskId]] Task ID
* [[index]] Index of the task within its link:spark-scheduler-TaskSet.adoc[TaskSet] that may not necessarily be the same as the ID of the RDD partition that the task is computing.
...
* [[taskLocality]] link:spark-scheduler-TaskSchedulerImpl.adoc#TaskLocality[TaskLocality], i.e. locality preference of the task
...
NOTE: `markFinished` is used when `TaskSetManager` is notified that a task has finished link:spark-scheduler-TaskSetManager.adoc#handleSuccessfulTask[successfully] or link:spark-scheduler-TaskSetManager.adoc#handleFailedTask[failed].
