...
CAUTION: I'm new to Machine Learning as a discipline and Spark MLlib in particular so mistakes are considered a norm (not an exception).
...
The amount of data that is measured in TB or PB is what makes Spark MLlib especially important since a human could not possibly extract much value from the dataset in a short time.

Spark handles data distribution and makes the huge data available by means of RDDs, DataFrames, and recently Datasets.

Use cases for Machine Learning and hence Spark MLlib:
