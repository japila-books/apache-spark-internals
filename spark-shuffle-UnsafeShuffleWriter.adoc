...
`UnsafeShuffleWriter` is a concrete link:spark-shuffle-ShuffleWriter.adoc[ShuffleWriter] that is used to <<write, write records>> (i.e. key-value pairs).

`UnsafeShuffleWriter` is chosen when link:spark-shuffle-SortShuffleManager.adoc#getWriter[`SortShuffleManager` is requested for a `ShuffleWriter`] for a link:spark-shuffle-SerializedShuffleHandle.adoc[SerializedShuffleHandle].
...
| `sorter`
| (uninitialized)
| link:spark-shuffle-ShuffleExternalSorter.adoc[ShuffleExternalSorter]
...
NOTE: `write` is part of link:spark-shuffle-ShuffleWriter.adoc#contract[`ShuffleWriter` contract].
...
NOTE: `stop` is part of link:spark-shuffle-ShuffleWriter.adoc#contract[`ShuffleWriter` contract].
...
4. link:spark-shuffle-SerializedShuffleHandle.adoc[SerializedShuffleHandle]
5. `mapId`
...
NOTE: `UnsafeShuffleWriter` is created exclusively when link:spark-shuffle-SortShuffleManager.adoc#getWriter[`SortShuffleManager` selects a `ShuffleWriter`] (for a link:spark-shuffle-SerializedShuffleHandle.adoc[SerializedShuffleHandle]).
