...
`spark.sql.parquet.filterPushdown` (default: `true`) is a flag to control the Parquet link:spark-sql-catalyst-optimizer-PushDownPredicate.adoc[filter predicate push-down optimization].
