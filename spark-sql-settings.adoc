...
`spark.sql.parquet.filterPushdown` (default: `true`) is a flag to control the Parquet link:spark-sql-predicate-pushdown.adoc[filter predicate push-down optimization].
