...
`isCheckpointPresent` internal method is to tell the other parts of a streaming application whether the `StreamingContext` instance was created from a <<initialCheckpoint, (initial) checkpoint>> or not.
