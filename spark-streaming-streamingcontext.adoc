...
* link:spark-streaming-checkpointing.adoc#recreating-streamingcontext[Recreating StreamingContext from a checkpoint directory]:
...
where `path` is the <<checkpoint-directory, checkpoint directory>>.
