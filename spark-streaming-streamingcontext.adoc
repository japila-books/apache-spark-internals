...
```
WARN StreamingContext: spark.master should be set as local[n], n > 1 in local mode if you have link:spark-streaming-receivers.adoc[receivers] to get data, otherwise Spark jobs will not get resources to process the received data.
```
====
