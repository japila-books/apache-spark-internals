...
It then register the <<stopOnShutdown, shutdown hook stopOnShutdown>> and <<streamingSource, registers streaming metrics source>>. If link:spark-webui.adoc#spark.ui.enabled[web UI is enabled], it attaches the link:spark-streaming-webui.adoc[Streaming tab].
