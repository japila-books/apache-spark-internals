...
*Initial checkpoint* is the checkpoint `StreamingContext` has been recreated from.

[NOTE]
====
You can create `StreamingContext` using either link:spark-sparkcontext.adoc[SparkContext] or link:spark-streaming-checkpointing.adoc#Checkpoint[checkpoint] (not both!)

Refer to <<creating-instance, Creating Instance>> section in this document.
====
