...
You use `checkpoint(directory: String)` method to set up a HDFS-compatible `directory` where the checkpoint data will be stored.
...
NOTE: Calling `checkpoint` with `null` as `directory` clears the checkpoint directory.
