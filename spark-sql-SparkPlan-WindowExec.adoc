...
// use rangeBetween first
val rangeWithTwoDevicesById = Window.partitionBy('device).orderBy('id).rangeBetween(start = -1, end = Window.currentRow)
...
Window [sum(cast(level#901 as bigint)) windowspecdefinition(device#900, id#899 ASC NULLS FIRST, RANGE BETWEEN 1 PRECEDING AND CURRENT ROW) AS sum#919L], [device#900], [id#899 ASC NULLS FIRST]
...
// use rowsBetween
val rowsWithTwoDevicesById = Window.partitionBy('device).orderBy('id).rowsBetween(start = -1, end = Window.currentRow)
...
Internally, `doExecute` link:spark-sql-SparkPlan.adoc#execute[executes] the single `child` physical operator and maps over partitions (using `RDD.mapPartitions` operator).
...
`doExecute` creates an `Iterator[InternalRow]` that first <<fetchNextRow, fetches the first row>> from the upstream RDD.
