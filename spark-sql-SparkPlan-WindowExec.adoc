...
`WindowExec` is a link:spark-sql-SparkPlan.adoc#UnaryExecNode[unary physical operator] with a collection of link:spark-sql-catalyst-Expression.adoc#NamedExpression[named expressions] (for windows), a collection of link:spark-sql-catalyst-Expression.adoc[expressions] (for partitions), a collection of `SortOrder` (for sorting) and a `child` link:spark-sql-SparkPlan.adoc[physical operator].
