...
// just a dataset of 1 row
val oneRow = spark.range(1)

// create windows of rows to use window aggregate function over every window
import org.apache.spark.sql.expressions.Window
val groupsOrderById = Window.partitionBy('id).orderBy('id).rangeBetween(start = -1, end = Window.currentRow)
val query = oneRow.select('*, sum('id) over groupsOrderById as "sum")
...
Window [sum(id#0L) windowspecdefinition(id#0L, id#0L ASC NULLS FIRST, RANGE BETWEEN 1 PRECEDING AND CURRENT ROW) AS sum#4L], [id#0L], [id#0L ASC NULLS FIRST]
...
== Physical Plan ==
Window [sum(id#0L) windowspecdefinition(id#0L, id#0L ASC NULLS FIRST, RANGE BETWEEN 1 PRECEDING AND CURRENT ROW) AS sum#4L], [id#0L], [id#0L ASC NULLS FIRST]
+- *Sort [id#0L ASC NULLS FIRST, id#0L ASC NULLS FIRST], false, 0
...
Internally, `createResultProjection` creates a translation table with a `BoundReference` per expression (in the input `expressions`).
