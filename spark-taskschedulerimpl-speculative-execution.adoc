...
It works as link:spark-scheduler-TaskSchedulerImpl.adoc#task-scheduler-speculation[`task-scheduler-speculation` daemon thread pool] using `j.u.c.ScheduledThreadPoolExecutor` with core pool size `1`.
...
| [[spark_speculation]] `spark.speculation` | `false` | Enables (`true`) or disables (`false`) speculative execution of tasks (by means of link:spark-scheduler-TaskSchedulerImpl.adoc#task-scheduler-speculation[`task-scheduler-speculation` Scheduled Executor Service]).
