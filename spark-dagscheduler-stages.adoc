...
`makeNewStageAttempt` link:spark-taskscheduler-taskmetrics.adoc[creates a new `TaskMetrics`] and link:spark-taskscheduler-taskmetrics.adoc#register[registers the RDD's `SparkContext` with it].
