...
It requires a link:spark-executor-backends.adoc[ExecutorBackend] (to send the task's status updates to), task and attempt ids, task name, and serialized version of the task (as `ByteBuffer`).
...
`kill` marks the task (handled by a `TaskRunner`) to be killed.
...
Internally, `kill` enables the internal flag `killed` and only if the task is available kills it (using link:spark-taskscheduler-tasks.adoc#kill[Task.kill]).

CAUTION: FIXME When could `task` not be set?
