...
Executors use a <<thread-pool, thread pool>> for <<metrics, sending metrics>> and <<launching-tasks, launching tasks>> (using link:spark-executor-taskrunner.adoc[TaskRunner]).
...
Executors maintain a mapping between task ids and running tasks (as instances of link:spark-executor-taskrunner.adoc[TaskRunner]) in `runningTasks` internal map. Consult <<launching-tasks, Launching Tasks>> section.
