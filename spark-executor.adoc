...
TaskRunner can run a single task only. When TaskRunner finishes (running a task), it is removed from `runningTasks`.
...
Executors keep sending <<metrics, active task metrics>> every <<settings, spark.executor.heartbeatInterval>>.
