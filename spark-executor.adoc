...
*Executors* are distributed agents to run tasks. They typically (?) run for the entire lifetime of a Spark application. Executors communicate with the driver to send computation results back.
...
Internally, executors use a daemon cached thread pool `Executor task launch worker-ID` for <<metrics, sending metrics>> and execute tasks.
