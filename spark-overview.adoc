...
Because it runs computations in memory (as often as possible), it's faster than existing MapReduce processing engines for iterative algorithms or interactive data mining (due to no partial result writes to disk). Spark aims at speed, ease of use, and interactive analytics.
...
Spark is written in http://scala-lang.org/[Scala], but it supports other languages beside Scala, i.e. Java, Python, and R.
