...
> One of the Spark project goals was to deliver a platform that supports a very wide array of *diverse workflows* - not only MapReduce batch jobs (there were available in Hadoop already), but also iterative computations like graph algorithms or Machine Learning.
>
> And also different scales of workloads from sub-second interactive jobs to jobs that run for many hours.

They are iterative by nature and no saves to disk or transfers over network means much better performance.
...
Not only can you use map and reduce (as in Hadoop MapReduce jobs) in Spark, but also use a wide array of other operators to ease your Spark queries and application development.
...
Apache Spark uses a link:spark-scheduler.adoc[directed acyclic graph (DAG) of computation stages] (aka *execution DAG*). It postpones any processing until really required for actions. Spark's *lazy evaluation* gives plenty of opportunities to induce low-level optimizations (so users have to know less to do more).
...
Spark supports diverse workloads, but excels at iterative ones.
