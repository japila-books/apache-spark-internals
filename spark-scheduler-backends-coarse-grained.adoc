...
It serializes a `TaskDescription` (using `SparkEnv.closureSerializer`) to a serialized task and the size is checked to fit <<spark.rpc.message.maxSize, spark.rpc.message.maxSize>> max message size for messages.
