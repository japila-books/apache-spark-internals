...
NOTE: It is customized by the coarse-grained scheduler backends for  link:spark-yarn-yarnschedulerbackend.adoc#doRequestTotalExecutors[YARN], link:spark-standalone-StandaloneSchedulerBackend.adoc[Spark Standalone], and link:spark-mesos-MesosCoarseGrainedSchedulerBackend.adoc[Mesos].
