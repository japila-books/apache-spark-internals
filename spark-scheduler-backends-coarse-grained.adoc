...
`RegisterExecutor(executorId, executorRef, hostPort, cores, logUrls)` is sent by link:spark-executor-backends.adoc#CoarseGrainedExecutorBackend[CoarseGrainedExecutorBackend] to register itself.
