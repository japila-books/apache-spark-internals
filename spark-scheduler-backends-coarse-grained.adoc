...
=== [[requestExecutors]] Requesting Executors (requestExecutors method)
...
`requestExecutors` is a "decorator" method that ultimately calls cluster-specific <<doRequestTotalExecutors, doRequestTotalExecutors>>.
...
NOTE: `requestExecutors` is a part of link:spark-service-ExecutorAllocationClient.adoc[ExecutorAllocationClient Contract] that...FIXME
