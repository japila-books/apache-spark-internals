...
Spark uses a *master/worker architecture*. There is a link:spark-driver.adoc[driver] that talks to a single coordinator link:spark-master.adoc[master] that communicates with one or more link:spark-workers.adoc[workers] in which link:spark-executor.adoc[executors] run.
