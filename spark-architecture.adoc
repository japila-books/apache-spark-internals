...
Spark uses a *master/worker architecture*. There is a *driver* that talks to a single coordinator *master* that communicates with one or more *workers* in which *executors* run.
