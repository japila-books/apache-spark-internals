...
Spark uses a *master/worker architecture*. There is a link:spark-driver.adoc[driver] that talks to a single coordinator *master* that communicates with one or more *workers* in which link:spark-executors.adoc[executors] run.
