...
`FsHistoryProvider` is the default <<ApplicationHistoryProvider, application history provider>> for link:spark-history-server.adoc[HistoryServer]. It uses link:spark-configuration.adoc[SparkConf] and `Clock` objects for its operation.
