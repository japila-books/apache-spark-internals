...
Since a Spark application requires different settings per deploy mode, `isClusterMode` flag "splits" `Client` per deploy mode.

CAUTION: FIXME Replace the internal fields used below with their true meanings.

CAUTION: FIXME Use a table to present the differences.

When the `isClusterMode` flag is enabled, i.e. the Spark application runs in cluster deploy mode:

1. `amMemory` is configured using link:../spark-driver.adoc#spark.driver.memory[spark.driver.memory].
2. `amMemoryOverhead` is configured using link:spark-yarn-settings.adoc#spark.yarn.driver.memoryOverhead[spark.yarn.driver.memoryOverhead].
3. `amCores` is configured using link:../spark-driver.adoc#spark.driver.cores[spark.driver.cores].
4. `javaOpts` is link:../spark-driver.adoc#spark.driver.extraJavaOptions[spark.driver.extraJavaOptions].
5. `libraryPaths` is link:../spark-driver.adoc#spark.driver.extraLibraryPath[spark.driver.extraLibraryPath] and link:../spark-driver.adoc##spark.driver.libraryPath[spark.driver.libraryPath].
6. link:spark-yarn-applicationmaster.adoc#command-line-parameters[`--class` command-line argument for `ApplicationMaster`] is set to `args.userClass`.
7. The application master class is link:spark-yarn-applicationmaster.adoc[org.apache.spark.deploy.yarn.ApplicationMaster].

When the `isClusterMode` flag is disabled, i.e. the Spark application runs in client deploy mode:

1. `amMemory` is configured using link:spark-yarn-settings.adoc#spark.yarn.am.memory[spark.yarn.am.memory].
2. `amMemoryOverhead` is configured using link:spark-yarn-settings.adoc#spark.yarn.am.memoryOverhead[spark.yarn.am.memoryOverhead].
3. `amCores` is configured using link:spark-yarn-settings.adoc#spark.yarn.am.cores[spark.yarn.am.cores].
4. `javaOpts` is link:spark-yarn-settings.adoc#spark.yarn.am.extraJavaOptions[spark.yarn.am.extraJavaOptions].
5. `libraryPaths` is link:spark-yarn-settings.adoc#spark.yarn.am.extraLibraryPath[spark.yarn.am.extraLibraryPath].
6. The application master class is link:spark-yarn-applicationmaster.adoc[org.apache.spark.deploy.yarn.ExecutorLauncher].
