...
<<creating-instance, `YarnAllocator` is created>> when link:spark-yarn-yarnrmclient.adoc#register[`YarnRMClient` registers `ApplicationMaster` for a Spark application] (with YARN ResourceManager).

.Creating YarnAllocator
image::../images/spark-yarn-YarnAllocator-amClient-ResourceManager.png[align="center"]

`YarnAllocator` communicates with YARN ResourceManager through the internal <<amClient, AMRMClient>> reference.
...
=== [[allocateResources]] Allocating YARN Resource Containers for Spark Executors (and Cancelling Outstanding Containers) -- `allocateResources` Method
