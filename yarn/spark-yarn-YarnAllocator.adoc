...
`YarnAllocator` <<allocateResources, allocates resource containers>> from the link:spark-yarn-introduction.adoc#ResourceManager[YARN ResourceManager] to run Spark executors on and <<internalReleaseContainer, releases them>> when the Spark application no longer needs them.
...
.YarnAllocator uses amClient Reference to YARN ResourceManager
