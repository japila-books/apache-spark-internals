...
After the external shuffle service is configured in YARN you enable it in a Spark application using link:spark-ExternalShuffleService.adoc#spark_shuffle_service_enabled[spark.shuffle.service.enabled] flag.
...
It creates a `TransportServer` as `shuffleServer` to listen to link:spark-ExternalShuffleService.adoc#spark_shuffle_service_port[spark.shuffle.service.port] (default: `7337`). It reads `spark.shuffle.service.port` key in the configuration.
...
If link:spark-ExternalShuffleService.adoc#spark_shuffle_service_enabled[external shuffle service is enabled], you need to add `spark_shuffle` to `yarn.nodemanager.aux-services` in the `yarn-site.xml` file on all nodes.
...
When you link:spark-ExternalShuffleService.adoc#spark_shuffle_service_enabled[enable an external shuffle service in a Spark application] when using link:README.adoc[Spark on YARN] but do not <<installation, install YARN Shuffle Service>> you will see the following exception in the logs:
