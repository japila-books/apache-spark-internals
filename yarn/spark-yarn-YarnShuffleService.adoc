...
`YarnShuffleService` is an external shuffle service for link:yarn/README.adoc[Spark on YARN]. It is YARN NodeManager's auxiliary service that implements `org.apache.hadoop.yarn.server.api.AuxiliaryService`.
...
When you link:spark-ExternalShuffleService.adoc#spark.shuffle.service.enabled[enable an external shuffle service in a Spark application] when using link:yarn/README.adoc[Spark on YARN] but do not <<installation, install YARN Shuffle Service>> you will see the following exception in the logs:
