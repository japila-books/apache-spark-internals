...
=== [[launchContextDebugInfo]] Creating INFO Template with Launch Context Debug -- `launchContextDebugInfo` Method
...
`launchContextDebugInfo` <<prepareCommand, prepares the command to launch `CoarseGrainedExecutorBackend`>> (as `commands` value) and <<prepareEnvironment, prepareEnvironment>> (as `env` value).
