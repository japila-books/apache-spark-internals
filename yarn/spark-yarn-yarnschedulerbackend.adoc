...
`YarnSchedulerBackend` registers in the RPC Environment as <<yarnSchedulerEndpointRef, `YarnScheduler` RPC endpoint>>.

`YarnSchedulerBackend` takes link:spark-taskschedulerimpl.adoc[TaskSchedulerImpl] and link:spark-sparkcontext.adoc[SparkContext] to initialize itself.
...
| link:spark-yarn-cluster-YarnSchedulerEndpoint.adoc[YarnScheduler] object
|

| [[yarnSchedulerEndpointRef]] `yarnSchedulerEndpointRef`
| link:../spark-RpcEndpointRef.adoc[RPC endpoint reference] to *YarnScheduler* RPC endpoint
