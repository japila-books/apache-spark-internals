...
`registerAM` link:spark-yarn-ExecutorRunnable.adoc#launchContextDebugInfo[prints INFO messages about the YARN launch context for executors] (with link:../spark-executor.adoc#spark.executor.memory[spark.executor.memory], link:../spark-executor.adoc#spark.executor.cores[spark.executor.cores] and dummy `<executorId>` and `<hostname>`)
