...
`ApplicationMaster` class acts as the link:yarn/spark-yarn-introduction.adoc#ApplicationMaster[YARN ApplicationMaster] for a Spark application deployed to a YARN cluster (which is commonly called link:README.adoc[Spark on YARN]).
...
`isClusterMode` is an internal flag that says whether the main class of the Spark application (through <<command-line-parameters, `--class` command-line argument>>) was specified or not. It is a mechanism to inform `ApplicationMaster` that it runs in link:../spark-deploy-mode.adoc#cluster[cluster mode] when link:spark-yarn-client.adoc#createContainerLaunchContext[`Client` creates YARN's `ContainerLaunchContext` (for launching `ApplicationMaster`)].
