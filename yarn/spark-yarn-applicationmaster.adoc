...
`ApplicationMaster` class acts as the link:spark-yarn-introduction.adoc#ApplicationMaster[YARN ApplicationMaster] for a Spark application running on a YARN cluster (which is commonly called link:README.adoc[Spark on YARN]).

`ApplicationMaster` uses <<allocator, YarnAllocator>> to manage YARN containers with executors.

`ApplicationMaster` is a <<main, standalone application>> that link:spark-yarn-introduction.adoc#NodeManager[YARN NodeManager] runs in a YARN container and manages a Spark application on YARN.

When <<creating-instance, created>> `ApplicationMaster` takes a <<client, YarnRMClient>> (which registers and unregisters a Spark application).
...
=== [[runAMEndpoint]] `runAMEndpoint` Internal Method
...
Besides, `client` helps obtaining <<getAttemptId, an application attempt id>> and link:spark-yarn-yarnrmclient.adoc#getMaxRegAttempts[the allowed number of attempts to register `ApplicationMaster`]. `client` also <<addAmIpFilter, gets filter parameters to secure ApplicationMaster's UI>>.
...
`runExecutorLauncher` reads link:spark-yarn-settings.adoc#spark.yarn.am.port[spark.yarn.am.port] Spark property (or defaults to `0`) and link:../spark-rpc.adoc#create[creates the `sparkYarnAM` RPC environment] (with the port, the internal <<sparkConf, SparkConf>> and `clientMode` enabled).
...
`runExecutorLauncher` then <<waitForSparkDriver, waits until the driver is network-accessible and creates `RpcEndpointRef` to communicate>>.
...
NOTE: `runExecutorLauncher` is used exclusively when <<run, `ApplicationMaster` runs>>.
...
`waitForSparkDriver` tries to connect to the driver's host and port until the driver listens to it but no longer than link:spark-yarn-settings.adoc#spark.yarn.am.waitTime[spark.yarn.am.waitTime] setting or <<finished, finished>> internal flag is enabled.
