...
When `ApplicationMaster` is started as a <<main, standalone command-line application>>, ultimately `run` is executed.
...
NOTE: `ApplicationMaster` is executed in a YARN container on a node in a YARN cluster.

`run` sets cluster mode settings, registers a cleanup shutdown hook, schedules `AMDelegationTokenRenewer` and finally registers `ApplicationMaster` for the Spark application (either calling <<runDriver, runDriver>> for cluster mode or <<runExecutorLauncher, runExecutorLauncher>> for client mode).

When in cluster mode, it sets the following system properties:
...
After the settings, `run` prints the following INFO message out to the logs:
...
The cleanup shutdown hook is registered with shutdown priority lower than that of link:spark-sparkcontext.adoc[SparkContext] (so it is executed after `SparkContext`).
