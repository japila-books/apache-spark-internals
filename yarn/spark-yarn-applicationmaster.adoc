...
`ApplicationMaster` class acts as the YARN ApplicationMaster for a Spark application.

It is a <<main, standalone application>> that YARN NodeManager runs inside a YARN resource container to manage a Spark application's execution on YARN (that accepts <<command-line-parameters, command-line parameters>>, e.g. `--jar`, `--class`, `--arg`, `--properties-file`).
...
`ApplicationMaster` (and `ExecutorLauncher`) is created when link:spark-yarn-client.adoc#createContainerLaunchContext[`Client` creates a `ContainerLaunchContext`] to launch Spark on YARN.
...
It then passes the call to `unregister` on to link:spark-yarn-yarnrmclient.adoc#unregister[YarnRMClient].
