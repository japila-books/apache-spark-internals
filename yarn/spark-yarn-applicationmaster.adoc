...
=== [[waitForSparkDriver]] `waitForSparkDriver` Internal Method
...
CAUTION: FIXME Why is `clientMode` enabled?

`runExecutorLauncher` then <<waitForSparkDriver, waits for the driver to be available>>.
