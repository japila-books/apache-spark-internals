...
`spark.yarn.maxAppAttempts` is the maximum number of attempts to register *ApplicationMaster* before deploying the Spark application to YARN is considered unsuccessful.
...
The value is used to set YARN's link:++https://hadoop.apache.org/docs/current/api/org/apache/hadoop/yarn/api/records/ApplicationSubmissionContext.html#setQueue(java.lang.String)++https://hadoop.apache.org/docs/current/api/org/apache/hadoop/yarn/api/records/ApplicationSubmissionContext.html#setQueue(java.lang.String)[ApplicationSubmissionContext.setQueue].
