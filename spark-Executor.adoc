...
Internally, `launchTask` creates a <<spark-Executor-TaskRunner.adoc#, TaskRunner>> (with the given <<spark-ExecutorBackend.adoc#, ExecutorBackend>> and the <<spark-TaskDescription.adoc#, TaskDescription>>) and registers it in the <<runningTasks, runningTasks>> internal registry (by the <<spark-TaskDescription.adoc#taskId, task ID>>).
...
* `LocalEndpoint` RPC endpoint (of <<spark-LocalSchedulerBackend.adoc#, LocalSchedulerBackend>>) is requested to <<spark-LocalEndpoint.adoc#reviveOffers, reviveOffers>>
