...
NOTE: Use *Details for Query* page in link:spark-webui-sql.adoc#ExecutionPage[SQL tab] in web UI to see the SQL execution metrics of a structured query.
...
This is why executing some physical operators (e.g. LocalTableScanExec) may not have SQL metrics in web UI's link:spark-webui-sql.adoc#ExecutionPage[Details for Query] in SQL tab.
...
`postDriverMetricUpdates` posts a link:spark-webui-SQLListener.adoc#SparkListenerDriverAccumUpdates[SparkListenerDriverAccumUpdates] event to link:spark-LiveListenerBus.adoc[LiveListenerBus] when `executionId` is specified.
