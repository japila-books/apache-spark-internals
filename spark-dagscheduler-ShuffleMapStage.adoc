...
`ShuffleMapStage` (aka *shuffle map stage* or simply *map stage*) is an link:spark-dagscheduler-stages.adoc[intermediate stage] in the *physical execution DAG* that corresponds to a link:spark-rdd-ShuffleDependency.adoc[ShuffleDependency].
...
Output locations can be missing, i.e. partitions have not been cached or are lost.
