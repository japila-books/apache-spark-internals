...
NOTE: `fetchBlocks` is called when link:spark-blocktransferservice.adoc#fetchBlockSync[`BlockTransferService` fetches one block synchronously] and `ShuffleBlockFetcherIterator` sends a request for blocks (using `sendRequest`).
