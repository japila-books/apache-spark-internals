...
* `spark.streaming.backpressure.enabled` (default: `false`) - enables (`true`) or disables (`false`) link:spark-streaming-backpressure.adoc[back pressure] in link:spark-streaming-receiverinputdstreams.adoc#back-pressure[input streams with receivers] or link:spark-streaming-kafka.adoc#back-pressure[DirectKafkaInputDStream].
