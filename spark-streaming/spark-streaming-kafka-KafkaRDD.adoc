...
`KafkaRDD` class represents a link:../spark-rdd.adoc[RDD dataset] built from records in Apache Kafka. It uses `KafkaRDDPartition` for partitions that know their preferred locations as the host of the topic (not port however!). It then nicely maps a RDD partition to a Kafka partition.
