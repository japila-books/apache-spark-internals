...
With that, `compute` builds a `StreamInputInfo` that it then passes on to link:spark-streaming-jobscheduler.adoc#InputInfoTracker[InputInfoTracker.reportInfo] (removing empty offset ranges). The metadata for the `StreamInputInfo` holds information about the offsets, the number thereof and a human-friendly description.
...
=== [[latestOffsets]] `latestOffsets` Method

CAUTION: FIXME

NOTE: Every call to `latestOffsets` calls Kafka brokers for the offsets.
