...
`getPreferredHosts` calculates preferred hosts per topic partition (that are later used to map link:spark-streaming-kafka-kafkardd.adoc[KafkaRDD] partitions to host leaders of topic partitions that Spark executors read from).
