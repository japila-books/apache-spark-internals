...
`TaskSchedulerImpl` is the default implementation of link:spark-taskscheduler.adoc#contract[TaskScheduler Contract] and extends it <<getRackForHost, to track racks per host and port>>. It can schedule tasks for multiple types of cluster managers by means of link:spark-SchedulerBackend.adoc[Scheduler Backends].
...
The private `waitBackendReady` method waits until a link:spark-SchedulerBackend.adoc#contract[SchedulerBackend is ready].
...
`resourceOffers` method is called by link:spark-SchedulerBackend.adoc[SchedulerBackend] (for clustered environments) or link:spark-LocalSchedulerBackend.adoc[LocalSchedulerBackend] (for local run mode) with `WorkerOffer` resource offers that represent cores (CPUs) available on all the active executors with one `WorkerOffer` per active executor.
...
NOTE: `resourceOffers` is called when link:spark-CoarseGrainedSchedulerBackend.adoc#makeOffers[`CoarseGrainedSchedulerBackend` makes resource offers].
...
NOTE: `statusUpdate` is called when link:spark-CoarseGrainedSchedulerBackend.adoc#StatusUpdate[CoarseGrainedSchedulerBackend], `LocalSchedulerBackend` and `MesosFineGrainedSchedulerBackend` inform about task state changes.
