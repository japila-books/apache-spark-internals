...
For a `FINISHED` task, link:spark-taskscheduler-tasksets.adoc[TaskSet.removeRunningTask] is called and then link:spark-taskscheduler.adoc#TaskResultGetter[TaskResultGetter.enqueueSuccessfulTask].

For a task in `FAILED`, `KILLED`, or `LOST` state, link:spark-taskscheduler-tasksets.adoc[TaskSet.removeRunningTask] is called (as for the `FINISHED` state) and then link:spark-taskscheduler.adoc#TaskResultGetter[TaskResultGetter.enqueueFailedTask].
