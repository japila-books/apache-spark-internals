...
`submitTasks` creates a link:spark-tasksetmanager.adoc[TaskSetManager] for the input link:spark-taskscheduler-tasksets.adoc[TaskSet] and adds it to the link:spark-taskscheduler-schedulablebuilders.adoc#addTaskSetManager[root `Schedulable` pool]. It then makes sure that the requested resources, i.e. CPU and memory, are assigned to the Spark application and ultimately requests the link:spark-scheduler-backends.adoc#reviveOffers[`SchedulerBackend` to revive offers].
...
Ultimately, `submitTasks` requests link:spark-scheduler-backends.adoc#reviveOffers[`SchedulerBackend` to revive offers].
...
Check whether the number of cores in an offer is more than the number of cores needed for a task (using <<spark.task.cpus, spark.task.cpus>>).
