...
=== [[scheduler-backends]] SchedulerBackends

A *Scheduler Backend* is a Spark service that...FIXME

As a Spark service it can be started and stopped. It can  reviveOffers, calculate defaultParallelism, kill tasks, return application attempt id (supported only by `YarnClusterSchedulerBackend`) and URLs for the driver logs.
