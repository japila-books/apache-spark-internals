...
NOTE: `getWriter` expects that the input `handle` is of type link:spark-BaseShuffleHandle.adoc[BaseShuffleHandle] (despite the signature that says that it can work with any `ShuffleHandle`).
...
NOTE: `getReader` assumes that the input `ShuffleHandle` is really a link:spark-BaseShuffleHandle.adoc[BaseShuffleHandle].
