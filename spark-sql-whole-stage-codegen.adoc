== [[WholeStageCodegen]][[WholeStageCodegenExec]] Whole Stage Codegen
...
*Whole stage codegen* (aka *WholeStageCodegen* or *WholeStageCodegenExec*) compiles a subtree of <<CodegenSupport, plans that support codegen>> together into a single Java function.
...
[source, scala]
----
scala> sqlContext.range(10).sample(false, 0.4).explain
== Physical Plan ==
WholeStageCodegen
...
[source, scala]
----
scala> sqlContext.range(10).explain
== Physical Plan ==
WholeStageCodegen
...
`CollapseCodegenStages` uses the internal setting `spark.sql.codegen.maxFields` (default: `200`) to control the number of fields in input and output schemas before deactivating whole-stage codegen. It counts the fields included in complex types, i.e. `StructType`, `MapType`, `ArrayType`, `UserDefinedType`, and their combinations, recursively. See https://issues.apache.org/jira/browse/SPARK-14554[SPARK-14554].
