...
*Executors* are distributed agents that run tasks. They typically (?) run for the entire lifetime of a Spark application. Executors communicate with the driver to send computation results back.
...
Executors use a daemon cached thread pool `Executor task launch worker-ID` for sending metrics and execute tasks.

(only for non-local mode) Executors register metrics source *executor* with the following metrics:
...
`driver-heartbeater` is a daemon single-thread scheduled pool executor, i.e. `ScheduledThreadPoolExecutor`, used by executor to send heartbeats and partial task metrics to a driver. They are sent to the driver every <<executor-settings, spark.executor.heartbeatInterval>>.
