...
NOTE: `TaskRunner` manages a single individual link:spark-taskscheduler-Task.adoc[task] and is managed by an link:spark-Executor.adoc#launchTask[`Executor` to launch a task].
...
2. link:local/spark-LocalSchedulerBackend.adoc[LocalSchedulerBackend] (for link:spark-local.adoc[local run mode])
