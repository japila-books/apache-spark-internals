...
=== [[read]] Reading Combined Key-Value Records For Reduce Task -- `read` Method
...
`read` creates a new `SerializerInstance` (using link:spark-rdd-ShuffleDependency.adoc#serializer[`Serializer` from ShuffleDependency]).
