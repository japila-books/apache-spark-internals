...
At this point in SparkContext's lifecycle, the internal `_taskScheduler` points at the `TaskScheduler` and it is "announced" by sending a blocking `TaskSchedulerIsSet` message to link:spark-sparkcontext-HeartbeatReceiver.adoc[HeartbeatReceiver RPC endpoint].
