...
It requires a `SparkEnv` and `TaskSchedulerImpl`.
...
It uses <<settings, spark.resultGetter.threads>> for the number of threads to use for the internal daemon thread thread pool prefixed *task-result-getter* (as `getTaskResultExecutor`). The thread pool is used for executing threads
