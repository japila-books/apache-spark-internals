...
*Task Scheduler* schedules <<tasks, tasks>> for a single link:spark-sparkcontext.adoc[SparkContext].
...
When a task has finished, it triggers TaskSetManager to send link:spark-scheduler.adoc#completionevent[a CompletionEvent message to DAGScheduler].
