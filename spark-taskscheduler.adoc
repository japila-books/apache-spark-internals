...
*TaskSetManager* manages execution of the tasks in a single <<taskset, TaskSet>>.

.TaskSetManager and its Dependencies
image::images/TaskSetManager-TaskSchedulerImpl-TaskSet.png[align="center"]

It schedules the tasks from the taskset and keeps track of their execution, i.e. retries them upon failure (up to <<settings, spark.task.maxFailures>> attempts), and handles locality-aware scheduling for the taskset via delay scheduling.

It also keeps track of the tasks pending execution per executor, host, rack or with no locality preferences.
...
TaskSetManager requests the current epoch from the map output tracker and sets it on all tasks in the taskset.
...
The check procedure is in `TaskSetManager.checkSpeculatableTasks` method.
