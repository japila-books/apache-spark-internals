...
NOTE: A `SchedulableBuilder` is created when <<initialize, TaskSchedulerImpl is being initialized>>. You can select the `SchedulableBuilder` to use by link:spark-taskschedulerimpl.adoc#spark.scheduler.mode[spark.scheduler.mode] setting.
