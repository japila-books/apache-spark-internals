...
a| link:spark-sql-Expression-WindowExpression.adoc#WithWindowDefinition[WithWindowDefinition] for link:spark-sql-windows.adoc[windowed aggregates].

Used for <<withQueryResultClauses, withQueryResultClauses>> and <<withQuerySpecification, withQuerySpecification>> with `windows` definitions.

```
WINDOW namedWindow (',' namedWindow)*

namedWindow: identifier AS windowSpec
```
