...
`ExternalClusterManager` is a <<contract, contract for pluggable cluster managers>>. It returns a link:spark-scheduler-TaskScheduler.adoc[task scheduler] and a link:spark-SchedulerBackend.adoc[backend scheduler] that will be used by link:spark-SparkContext.adoc[SparkContext] to schedule tasks.
...
`createTaskScheduler` creates a link:spark-scheduler-TaskScheduler.adoc[TaskScheduler] given a link:spark-SparkContext.adoc[SparkContext] and the input `masterURL`.
...
`createSchedulerBackend` creates a link:spark-SchedulerBackend.adoc[SchedulerBackend] given a link:spark-SparkContext.adoc[SparkContext], the input `masterURL`, and link:spark-scheduler-TaskScheduler.adoc[TaskScheduler].
...
`initialize` is called after the link:spark-scheduler-TaskScheduler.adoc[task scheduler] and the link:spark-SchedulerBackend.adoc[backend scheduler] were created and initialized separately.
...
NOTE: link:spark-scheduler-TaskScheduler.adoc[TaskScheduler] and link:spark-SchedulerBackend.adoc[SchedulerBackend] (with link:spark-scheduler-DAGScheduler.adoc[DAGScheduler]) are commonly referred to as *scheduling components*.
