...
When it is started (using `start(time: Time)` method), it saves `time` as `startTime`, and calls `initialize` and `remember` on every output stream (one by one). It then starts the input streams (in parallel).
