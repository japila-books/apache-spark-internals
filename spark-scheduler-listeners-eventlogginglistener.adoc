...
When <<spark.eventLog.enabled, enabled>> it writes events to a log file under <<spark.eventLog.dir, spark.eventLog.dir>> directory. All link:spark-SparkListener.adoc[Spark events] are logged.
...
If <<spark.eventLog.compress, event compression is enabled>>, `CompressionCodec.createCodec(sparkConf)` is called to create a compression codec using a short codec name or the fully-qualified class name of a codec.
