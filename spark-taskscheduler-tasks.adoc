...
In Spark, a *task* (aka _command_) is the smallest individual unit of execution that can compute a link:spark-rdd-Partition.adoc[RDD partition]. Tasks are link:spark-executor.adoc#launchTask[launched on executors] and <<run, ran when `TaskRunner` starts>>.
...
The <<contract, Task contract>> expects that custom tasks define <<runTask, runTask>> method.
...
`run` link:spark-blockmanager.adoc#registerTask[registers the task (identified as `taskAttemptId`) with the executor's `BlockManager`].
