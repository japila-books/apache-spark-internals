...
`SparkEnv.createDriverEnv` creates the driver. It uses SparkConf, whether or not it runs in local mode, a listener bus, number of cores, and the link:spark-service-outputcommitcoordinator.adoc[OutputCommitCoordinator].
