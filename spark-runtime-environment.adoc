...
A Spark *driver* is the process that launches the `main` method of your Spark application and manages the execution of jobs and tasks (using link:spark-dagscheduler.adoc[DAGScheduler] and link:spark-taskscheduler.adoc[Task Scheduler]). It hosts link:spark-webui.adoc[Web UI] for the environment.
...
It uses RPC Environment to talk to the driver (`driverEndpoint`).
