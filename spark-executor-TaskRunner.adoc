...
`TaskRunner` is a *thread of execution* of a <<taskDescription, single task>>.

`TaskRunner` is <<creating-instance, created>> exclusively when `Executor` is requested to link:spark-Executor.adoc#launchTask[launch a task].
...
* [[execBackend]] link:spark-ExecutorBackend.adoc[ExecutorBackend]
* [[taskDescription]] link:spark-TaskDescription.adoc[TaskDescription]
