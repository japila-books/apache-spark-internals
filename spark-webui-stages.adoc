...
The page uses the parent's link:spark-webui-SparkUI.adoc[SparkUI] to access required services, i.e. link:spark-sparkcontext.adoc[SparkContext], link:spark-sql-SQLConf.adoc[SparkConf], link:spark-webui-JobProgressListener.adoc[JobProgressListener], `RDDOperationGraphListener`, and <<killEnabled, whether kill is enabled or not>>.
