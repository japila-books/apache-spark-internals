...
(that is assumed to be a link:spark-ShuffleClient-ExternalShuffleClient.adoc[ExternalShuffleClient])
...
=== [[replicate]] Replicating Block Across Node(s) -- `replicate` Internal Method
...
NOTE: `getOrElseUpdate` is used when `RDD` is requested to link:spark-rdd.adoc#getOrCompute[getOrCompute].
