...
`doPutIterator` simply <<doPut, doPut>> with the `putBody: BlockInfo => Option[T]` function that does the following:

. `putBody` branches off per whether the `StorageLevel` indicates to use link:spark-rdd-StorageLevel.adoc#useMemory[memory] or link:spark-rdd-StorageLevel.adoc#useDisk[disk] only, i.e.

* When the input `StorageLevel` indicates to link:spark-rdd-StorageLevel.adoc#useMemory[use memory] for storage in link:spark-rdd-StorageLevel.adoc#deserialized[deserialized] format, `putBody`...FIXME
