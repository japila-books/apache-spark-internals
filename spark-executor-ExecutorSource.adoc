...
NOTE: Every executor has its own separate `ExecutorSource` that is registered when link:spark-executor-backends-coarse-grained.adoc#RegisteredExecutor[`CoarseGrainedExecutorBackend` receives a `RegisteredExecutor`].
