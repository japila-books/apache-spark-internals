...
It transforms the link:spark-sql-catalyst-LogicalPlan.adoc[LogicalPlan] of the input DataFrame `df` so all link:spark-sql-streaming-streamingrelation.adoc[StreamingRelation] "nodes" become link:spark-sql-streaming-streamingrelation.adoc#StreamingExecutionRelation[StreamingExecutionRelation]. It uses link:spark-sql-datasource.adoc#createSource[DataSource.createSource(metadataPath)] where `metadataPath` is `$checkpointLocation/sources/$nextSourceId`. Otherwise, it returns the `LogicalPlan` untouched.
