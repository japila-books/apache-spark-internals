...
`handleMapStageSubmitted` link:spark-dagscheduler.adoc#getOrCreateShuffleMapStage[finds or creates a new `ShuffleMapStage`] for the input `ShuffleDependency` and `jobId`.

`handleMapStageSubmitted` creates a link:spark-dagscheduler-jobs.adoc[ActiveJob] for the input `jobId`, `callSite`, `listener` and `properties`, and the `ShuffleMapStage`.
...
`handleMapStageSubmitted` registers the job in link:spark-dagscheduler.adoc#jobIdToActiveJob[jobIdToActiveJob] and link:spark-dagscheduler.adoc#activeJobs[activeJobs] internal registries, and then link:spark-dagscheduler-ShuffleMapStage.adoc#addActiveJob[assigns the job to the final `ShuffleMapStage`].
...
`handleJobSubmitted` link:spark-dagscheduler.adoc#createResultStage[creates a new `ResultStage`] (as `finalStage` in the picture below) and an link:spark-dagscheduler-jobs.adoc[ActiveJob].
...
`handleJobSubmitted` then registers the job in link:spark-dagscheduler.adoc#jobIdToActiveJob[jobIdToActiveJob] and link:spark-dagscheduler.adoc#activeJobs[activeJobs] internal registries, and then link:spark-dagscheduler-ResultStage.adoc#setActiveJob[assigns the job to the final `ResultStage`]
