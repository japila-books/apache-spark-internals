...
`handleTaskCompletion` informs link:spark-service-outputcommitcoordinator.adoc#taskCompleted[OutputCommitCoordinator] that a task completed.
...
For link:spark-taskscheduler-ShuffleMapTask.adoc[ShuffleMapTask], the stage is link:spark-dagscheduler-ShuffleMapStage.adoc[ShuffleMapStage].

link:spark-dagscheduler.adoc#updateAccumulators[DAGScheduler.updateAccumulators(event)] is called.

`event.result` is link:spark-taskscheduler-ShuffleMapTask.adoc#MapStatus[MapStatus] that knows the executor id where the task has finished (using `status.location.executorId`).
