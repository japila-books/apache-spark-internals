...
`def handleJobCancellation(jobId: Int, reason: String = "")` checks whether the job exists in link:spark-dagscheduler.adoc#jobIdToStageIds[jobIdToStageIds] and if not, prints the following DEBUG to the logs:
