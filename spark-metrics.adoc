...
*Spark Metrics* gives you execution metrics of a link:spark-metrics-MetricsSystem.adoc#subsystems[Spark subsystem] (aka _metrics instance_), e.g. the driver of a Spark application or the master of a Spark Standalone cluster.

The main part of Spark Metrics is link:spark-metrics-MetricsSystem.adoc[MetricsSystem] that is a registry of metrics link:spark-metrics-Source.adoc[sources] and link:spark-metrics-Sink.adoc[sinks] of a Spark subsystem. A Spark subsystem accesses the `MetricsSystem` through the link:spark-SparkEnv.adoc#metricsSystem[SparkEnv.metricsSystem] property.
