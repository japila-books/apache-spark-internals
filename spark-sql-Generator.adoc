...
If you want to have more than one generator in a structured query you may want to consider `LATERAL VIEW` that is however supported only in SQL, e.g.

[source, scala]
----
val ncs = Seq((Array(1,2,3), Array("a","b","c"))).toDF("ns", "cs")
