...
Used exclusively when `LauncherBackend` is requested to <<connect, connect>> (to access <<spark-configuration-properties.adoc#spark.launcher.port, spark.launcher.port>> and <<spark-configuration-properties.adoc#spark.launcher.secret, spark.launcher.secret>> configuration properties)
