...
Used in `ExternalSorter`, `BypassMergeSortShuffleWriter` and `ExternalAppendOnlyMap` (for `fileBufferSize`) and in link:spark-ShuffleExternalSorter.adoc[ShuffleExternalSorter] (for link:spark-ShuffleExternalSorter.adoc#fileBufferSizeBytes[fileBufferSizeBytes]).
