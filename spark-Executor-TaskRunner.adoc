...
* [[taskDescription]] <<spark-TaskDescription.adoc#, Metadata of the task to be executed>>
...
| task
a| [[task]] Deserialized <<spark-taskscheduler-Task.adoc#, task>> to execute
...
| taskId
a| [[taskId]] The <<spark-TaskDescription.adoc#taskId, task ID>> (of the <<taskDescription, TaskDescription>>)

Used when:

* `TaskRunner` is requested to <<run, run>> (to create a <<spark-TaskMemoryManager.adoc#, TaskMemoryManager>> and serialize a `IndirectTaskResult` for a large task result) and <<kill, kill>> the task and for the <<threadName, threadName>>

* `Executor` is requested to <<spark-Executor.adoc#reportHeartBeat, reportHeartBeat>>

| taskName
a| [[taskName]] The <<spark-TaskDescription.adoc#name, name of the task>> (of the <<taskDescription, TaskDescription>>) that is used exclusively for <<logging, logging>> purposes when `TaskRunner` is requested to <<run, run>> and <<kill, kill>> the task
...
`run` link:spark-TaskMemoryManager.adoc#creating-instance[creates a `TaskMemoryManager`] (using the current link:spark-MemoryManager.adoc[MemoryManager] and <<taskId, taskId>>).
...
`run` then link:spark-TaskMemoryManager.adoc#cleanUpAllAllocatedMemory[requests `TaskMemoryManager` to clean up allocated memory] (that helps finding memory leaks).

If `run` detects memory leak of the managed memory (i.e. the memory freed is greater than `0`) and <<spark-TaskSetManager-configuration-properties.adoc#spark.unsafe.exceptionOnMemoryLeak, spark.unsafe.exceptionOnMemoryLeak>> Spark property is enabled (it is not by default) and no exception was reported while the task ran, `run` reports a `SparkException`:

```
Managed memory leak detected; size = [freedMemory] bytes, TID = [taskId]
```

Otherwise, if <<spark-TaskSetManager-configuration-properties.adoc#spark.unsafe.exceptionOnMemoryLeak, spark.unsafe.exceptionOnMemoryLeak>> is disabled, you should see the following ERROR message in the logs instead:
...
If `run` detects lock leaking (i.e. the number of locks released) and <<spark-TaskSetManager-configuration-properties.adoc#spark.storage.exceptionOnPinLeak, spark.storage.exceptionOnPinLeak>> Spark property is enabled (it is not by default) and no exception was reported while the task ran, `run` reports a `SparkException`:
...
Otherwise, if <<spark-TaskSetManager-configuration-properties.adoc#spark.storage.exceptionOnPinLeak, spark.storage.exceptionOnPinLeak>> is disabled or the task reported an exception, you should see the following INFO message in the logs instead:
