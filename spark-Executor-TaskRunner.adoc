...
`TaskRunner` is a *thread of execution* (a https://docs.oracle.com/javase/8/docs/api/java/lang/Runnable.html[java.lang.Runnable]) of a <<taskDescription, single task>>.
...
Once <<creating-instance, created>>, `TaskRunner` can be <<run, run>> and optionally <<kill, killed>>.

Put simply, a `TaskRunner` manages execution of a <<taskDescription, single task>>.
...
[[internal-registries]]
.TaskRunner's Internal Properties (e.g. Registries, Counters and Flags)
...
| finished
a| [[finished]][[isFinished]] Flag that says whether the <<taskDescription, task>> has finished or not
...
NOTE: `setTaskFinishedAndClearInterruptStatus` is used when `TaskRunner` is requested to <<collectAccumulatorsAndResetStatusOnFailure, collectAccumulatorsAndResetStatusOnFailure>> and <<run, run>>.
...
`run` link:spark-memory-TaskMemoryManager.adoc#creating-instance[creates a `TaskMemoryManager`] (using the current link:spark-MemoryManager.adoc[MemoryManager] and <<taskId, taskId>>).
...
NOTE: `run` uses link:spark-scheduler-TaskDescription.adoc[TaskDescription] that is specified when `TaskRunner` <<creating-instance, is created>>.
...
NOTE: `run` uses `TaskDescription` link:spark-scheduler-TaskDescription.adoc#serializedTask[to access serialized task].
...
`run` link:spark-scheduler-Task.adoc#run[runs the task] (with `taskAttemptId` as <<taskId, taskId>>, `attemptNumber` from `TaskDescription`, and `metricsSystem` as the current link:spark-metrics-MetricsSystem.adoc[MetricsSystem]).
...
`run` then link:spark-memory-TaskMemoryManager.adoc#cleanUpAllAllocatedMemory[requests `TaskMemoryManager` to clean up allocated memory] (that helps finding memory leaks).

If `run` detects memory leak of the managed memory (i.e. the memory freed is greater than `0`) and <<spark-configuration-properties.adoc#spark.unsafe.exceptionOnMemoryLeak, spark.unsafe.exceptionOnMemoryLeak>> Spark property is enabled (it is not by default) and no exception was reported while the task ran, `run` reports a `SparkException`:

```
Managed memory leak detected; size = [freedMemory] bytes, TID = [taskId]
```

Otherwise, if <<spark-configuration-properties.adoc#spark.unsafe.exceptionOnMemoryLeak, spark.unsafe.exceptionOnMemoryLeak>> is disabled, you should see the following ERROR message in the logs instead:
...
If the link:spark-scheduler-Task.adoc#kill[task was killed] (while it was running), `run` reports a `TaskKilledException` (and the `TaskRunner` exits).
...
`run` records the link:spark-scheduler-Task.adoc#metrics[task metrics]:

* link:spark-executor-TaskMetrics.adoc#setExecutorDeserializeTime[executorDeserializeTime]
* link:spark-executor-TaskMetrics.adoc#setExecutorDeserializeCpuTime[executorDeserializeCpuTime]
* link:spark-executor-TaskMetrics.adoc#setExecutorRunTime[executorRunTime]
* link:spark-executor-TaskMetrics.adoc#setExecutorCpuTime[executorCpuTime]
* link:spark-executor-TaskMetrics.adoc#setJvmGCTime[jvmGCTime]
* link:spark-executor-TaskMetrics.adoc#setResultSerializationTime[resultSerializationTime]

`run` link:spark-scheduler-Task.adoc#collectAccumulatorUpdates[collects the latest values of internal and external accumulators used in the task].

`run` creates a link:spark-scheduler-TaskResult.adoc#DirectTaskResult[DirectTaskResult] (with the serialized result and the latest values of accumulators).
...
TIP: Read about link:spark-configuration-properties.adoc#spark.driver.maxResultSize[spark.driver.maxResultSize].
...
In this case, `run` creates a link:spark-scheduler-TaskResult.adoc#IndirectTaskResult[IndirectTaskResult] (with a `TaskResultBlockId` for the task's <<taskId, taskId>> and `resultSize`) and link:spark-Serializer.adoc#serialize[serializes it].
...
NOTE: The final `serializedResult` is either a link:spark-scheduler-TaskResult.adoc#IndirectTaskResult[IndirectTaskResult] (possibly with the block stored in `BlockManager`) or a link:spark-scheduler-TaskResult.adoc#DirectTaskResult[DirectTaskResult].
...
`run` then link:spark-scheduler-Task.adoc#collectAccumulatorUpdates[collects the latest values of internal and external accumulators] (with `taskFailed` flag enabled to inform that the collection is for a failed task).
...
`kill` marks the `TaskRunner` as <<killed, killed>> and link:spark-scheduler-Task.adoc#kill[kills the task] (if available and not <<finished, finished>> already).
