...
It then calls the link:spark-yarn-yarnschedulerbackend.adoc#start[parent's start] and sets the parent's link:spark-yarn-yarnschedulerbackend.adoc#totalExpectedExecutors[totalExpectedExecutors] to link:spark-yarn-YarnSparkHadoopUtil.adoc#getInitialTargetExecutorNumber[YarnSparkHadoopUtil.getInitialTargetExecutorNumber].
...
Internally, it retrieves the link:spark-yarn.adoc#getContainerId[container id] and through environment variables computes the base URL.
