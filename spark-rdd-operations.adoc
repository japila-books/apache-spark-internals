...
*Transformations* are lazy operations on a RDD that return another RDD or an array of RDDs, e.g. `map`,  `filter`, `reduceByKey`, `join`, `cogroup`, `randomSplit`, etc.
...
*Actions* are operations that return a non-RDD value, i.e. any RDD operation that returns any value but `RDD[T]` is an action.
