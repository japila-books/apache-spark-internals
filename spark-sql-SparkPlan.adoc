...
`SparkPlan` is the base Catalyst link:spark-sql-catalyst-QueryPlan.adoc[query plan] for *physical operators* to build an *physical query plan* (aka *query execution plan*) that, when <<execute, executed>>, can produce an RDD of link:spark-sql-InternalRow.adoc[InternalRows] (i.e. `RDD[InternalRow]`).
