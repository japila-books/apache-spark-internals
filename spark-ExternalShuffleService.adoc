...
| [[spark_shuffle_service_enabled]] `spark.shuffle.service.enabled` | `false` |
...
It is explicitly disabled for `LocalSparkCluster` (and _any_ attempts to set are ignored).
