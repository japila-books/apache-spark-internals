...
NOTE: A link:spark-taskscheduler-TaskSet.adoc[TaskSet] represents a set of link:spark-taskscheduler-Task.adoc[tasks] that correspond to missing link:spark-rdd-partitions.adoc[partitions] of a link:spark-DAGScheduler-Stage.adoc[stage].

`TaskSetManager` is <<creating-instance, created>> exclusively when `TaskSchedulerImpl` is requested to <<spark-TaskSchedulerImpl.adoc#createTaskSetManager, create one>> (when submitting tasks for a given `TaskSet`).
...
NOTE: `executorAdded` is used exclusively when `TaskSchedulerImpl` is requested to <<spark-TaskSchedulerImpl.adoc#resourceOffers, resourceOffers>>.
...
NOTE: `abortIfCompletelyBlacklisted` is used exclusively when `TaskSchedulerImpl` is requested to <<spark-TaskSchedulerImpl.adoc#resourceOffers, resourceOffers>>.
...
+
It means that it can only be a leaf in the tree of Schedulables (with link:spark-Schedulable-Pool.adoc[Pools] being the nodes).
...
* `priority` is the priority of the owned link:spark-taskscheduler-TaskSet.adoc[TaskSet] (using `taskSet.priority`).
* `stageId` is the stage id of the owned link:spark-taskscheduler-TaskSet.adoc[TaskSet] (using `taskSet.stageId`).
...
`addRunningTask` adds `tid` to <<runningTasksSet, runningTasksSet>> internal registry and link:spark-Schedulable-Pool.adoc#increaseRunningTasks[requests the `parent` pool to increase the number of running tasks] (if defined).
...
`removeRunningTask` removes `tid` from <<runningTasksSet, runningTasksSet>> internal registry and link:spark-Schedulable-Pool.adoc#decreaseRunningTasks[requests the `parent` pool to decrease the number of running task] (if defined).
...
NOTE: `resourceOffer` is used exclusively when `TaskSchedulerImpl` is requested to <<spark-TaskSchedulerImpl.adoc#resourceOfferSingleTaskSet, resourceOfferSingleTaskSet>>.
...
* `TaskResultGetter` is requested to <<spark-TaskResultGetter.adoc#enqueueSuccessfulTask, enqueueSuccessfulTask>> (that has failed)

* `TaskSchedulerImpl` is requested to <<spark-TaskSchedulerImpl.adoc#cancelTasks, cancelTasks>> and <<spark-TaskSchedulerImpl.adoc#error, error>>
...
* [[sched]] <<spark-TaskSchedulerImpl.adoc#, TaskSchedulerImpl>>
* [[taskSet]] <<spark-taskscheduler-TaskSet.adoc#, TaskSet>>
...
* `TaskSchedulerImpl` is requested to <<spark-TaskSchedulerImpl.adoc#handleFailedTask, handle a failed task>>

* `TaskSetManager` is requested to <<executorLost, handle a lost executor>>
====
...
NOTE: Since `TaskSetManager` manages execution of the tasks in a single link:spark-taskscheduler-TaskSet.adoc[TaskSet], when an executor gets lost, the affected tasks that have been running on the failed executor need to be re-enqueued. `executorLost` is the mechanism to "announce" the event to all `TaskSetManagers`.
