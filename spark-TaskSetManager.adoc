...
Updated with the task (id) and the corresponding `TaskInfo` when `TaskSetManager` <<resourceOffer, dequeues a task for execution (given resource offer)>>.
...
=== [[resourceOffer]] Dequeuing Task For Execution (Given Resource Offer) -- `resourceOffer` Method
...
`handleFailedTask` <<removeRunningTask, removes `tid` task from the internal registry of running tasks>> and marks the corresponding link:spark-TaskInfo.adoc[TaskInfo] as finished. It decreases the number of the `tid` task's copies running (in <<copiesRunning, copiesRunning>> internal registry).
