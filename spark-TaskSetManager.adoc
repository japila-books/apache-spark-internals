...
* link:spark-TaskSchedulerImpl.adoc#spark.task.maxFailures[spark.task.maxFailures] property for link:local/spark-local.adoc[Spark local-cluster] and link:spark-cluster.adoc[Spark clustered] (using Spark Standalone, Mesos and YARN)

[[maxResultSize]]
`TaskSetManager` uses <<spark-TaskSetManager-configuration-properties.adoc#MAX_RESULT_SIZE, spark.driver.maxResultSize>> configuration property (default: `1g`) to <<canFetchMoreResults, check available memory for task results>>.
...
TIP: In Spark shell with local master, `spark.task.maxFailures` is fixed to `1` and you need to use link:local/spark-local.adoc[local-with-retries master] to change it to some other value.
...
* [[taskSet]] <<spark-taskscheduler-tasksets.adoc#, TaskSet>>
...
If the current internal <<totalResultSize, totalResultSize>> is bigger than the <<spark-TaskSetManager-configuration-properties.adoc#maxResultSize, maximum result size>>, `canFetchMoreResults` prints out the following ERROR message to the logs:
...
NOTE: `canFetchMoreResults` uses <<spark-TaskSetManager-configuration-properties.adoc#spark.driver.maxResultSize, spark.driver.maxResultSize>> configuration property to control the maximum result size. The default value is `1g`.
