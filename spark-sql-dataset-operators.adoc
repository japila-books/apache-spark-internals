...
Internally, `coalesce` creates a `Repartition` logical operator with `shuffle` disabled.
...
Internally, `repartition` creates a `Repartition` or `RepartitionByExpression` logical operators with `shuffle` enabled, respectively.
