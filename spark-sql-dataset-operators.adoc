...
=== [[repartition]] Repartitioning Dataset -- `repartition` Methods
...
`repartition` repartition the `Dataset` to exactly `numPartitions` partitions or using `partitionExprs` expressions.

Internally, `repartition` creates a `Repartition` or `RepartitionByExpression` logical operator.

NOTE: `repartition` methods correspond to SQL's `DISTRIBUTE BY` or `CLUSTER BY`.

=== [[select]] Projecting Columns -- `select` Methods
...
=== [[randomSplit]] Randomly Split Dataset -- `randomSplit` Methods
...
=== [[explain]] Explaining Logical and Physical Plans -- `explain` Method
...
`explain` prints the link:spark-sql-LogicalPlan.adoc[logical] and (with `extended` enabled) link:spark-sql-catalyst-SparkPlan.adoc[physical] plans to the console. Use it to review the structured queries and optimizations applied.
...
Internally, `explain` link:spark-sql-catalyst-SparkPlan.adoc#executeCollect[executes] a link:spark-sql-ExplainCommand.adoc[ExplainCommand] logical command.
