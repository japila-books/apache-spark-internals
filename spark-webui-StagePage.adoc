...
.Details for Stage 2 and Attempt 0
...
It uses the parent's link:spark-webui-JobProgressListener.adoc[JobProgressListener] and `RDDOperationGraphListener` to calculate the <<metrics, metrics>>. More specifically, `StagePage` uses link:spark-webui-JobProgressListener.adoc#stageIdToData[JobProgressListener.stageIdToData] registry to access the stage for given stage `id` and `attempt`.
