...
Internally, `join` link:spark-sql-Dataset.adoc#ofRows[creates a `DataFrame`] with link:spark-sql-LogicalPlan-Join.adoc[`Join` logical operator] (in the current link:spark-sql-SparkSession.adoc[SparkSession]).
