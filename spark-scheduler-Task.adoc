...
* link:spark-scheduler-ShuffleMapTask.adoc[ShuffleMapTask] that executes a task and divides the task's output to multiple buckets (based on the task's partitioner).
* link:spark-scheduler-ResultTask.adoc[ResultTask] that executes a task and sends the task's output back to the driver application.
...
* [[stageId]] link:spark-scheduler-Stage.adoc[Stage] ID
...
* [[serializedTaskMetrics]] Serialized link:spark-executor-TaskMetrics.adoc[TaskMetrics] (that link:spark-scheduler-Stage.adoc#latestInfo[were part of the owning Stage])
* (optional) [[jobId]] link:spark-scheduler-ActiveJob.adoc[Job] ID
...
NOTE: `run` is used exclusively when link:spark-Executor-TaskRunner.adoc#run[`TaskRunner` starts]. The `Task` instance has just been deserialized from `taskBytes` that were sent over the wire to an executor. `localProperties` and link:spark-memory-TaskMemoryManager.adoc[TaskMemoryManager] are already assigned.
