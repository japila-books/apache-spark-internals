...
`spark.driver.extraClassPath` can act as a Spark setting that you can set on command line (e.g. using ``spark-submit``'s link:spark-submit.adoc#driver-class-path[`--driver-class-path` command-line option]), a link:spark-configuration.adoc#properties-file[Spark properties file], or in link:spark-configuration.adoc[`SparkConf` instance].
...
NOTE: Use ``spark-submit``'s link:spark-submit.adoc#driver-class-path[`--driver-class-path` command-line option] on command line to override `spark.driver.extraClassPath` from a Spark properties file.
