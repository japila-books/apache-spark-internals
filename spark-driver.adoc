...
NOTE: Use ``spark-submit``'s link:spark-submit.adoc#driver-class-path[`--driver-class-path` command-line option] to assign a value on command line and possibly override `spark.driver.extraClassPath` from a properties file.
