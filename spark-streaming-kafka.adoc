...
NOTE: Use explicitly-typed version of `KafkaUtils.createDirectStream`, e.g. `KafkaUtils.createDirectStream[String, String, StringDecoder, StringDecoder]`, so proper decoders are used to decode messages from Kafka.
...
`metadata.broker.list` is a comma-separated list of Kafka's (seed) brokers in the format of `<host>:<port>`.
...
// You may or may not want to enable some additional logging
...
NOTE: It is link:spark-streaming-dstreams.adoc#DStreamGraph-generateJobs[DStreamGraph to request generating streaming jobs for batches].
