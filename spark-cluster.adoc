...
The cluster can be on-premise or in cloud.

Running Spark in cluster requires workload and resource management on distributed systems.

link:spark-driver.adoc[Spark driver] requests resources from a cluster manager, i.e. CPU and memory. It is a cluster manager's responsibility to spawn Spark executors in the cluster (on workers).
