...
Spark driver communicates with cluster manager for resources, e.g. CPU, memory, disk. The cluster manager spawns Spark executors in the cluster. After the executors are started, the Spark driver communicates with the Spark executors directly to launch jobs (as tasks). Executors remain alive after jobs are finished for future ones. This allows for better data utilization as intermediate data is cached in memory.
