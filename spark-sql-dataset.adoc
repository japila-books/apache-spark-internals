...
NOTE: `SQLContext` and `QueryExecution` are transient and hence do not participate in Dataset serialization. The only _firmly-tied_ feature of a Dataset is the Encoder.
...
Internally, it uses `SQLContext.executePlan(logicalPlan)`.
