...
=== [[apply]] Creating Column -- apply method
...
`explain` prints the link:spark-sql-logical-plan.adoc[logical] and physical plans to the console. Use it for debugging structured queries.
...
Internally, `explain` link:spark-sql-spark-plan.adoc#executeCollect[executes] a link:spark-sql-logical-plan.adoc#ExplainCommand[ExplainCommand] logical command.
...
=== [[ofRows]] Creating DataFrame (ofRows method)
...
NOTE: `ofRows` is a `private[sql]` method that can only be accessed from code in `org.apache.spark.sql` package. It is not a part of ``Dataset``'s public API.
