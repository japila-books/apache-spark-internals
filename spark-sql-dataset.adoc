...
NOTE: `foreachPartition` is used in `org.apache.spark.sql.execution.datasources.jdbc.JdbcUtils#saveTable`.

CAUTION: FIXME Where is `saveTable` method used?
...
=== [[withNewExecutionId]] Tracking Multi-Job Actions (withNewExecutionId method)
...
`withNewExecutionId` is a `private[sql]` method that executes the input `body` action (using link:spark-sql-sqlcontext.adoc#SQLExecution-withNewExecutionId[SQLExecution.withNewExecutionId]).
