...
*Dataset* is the API for working with *structured data*, i.e. records with a known schema. It is the result of executing a SQL query against data storage like files or databases.

.Dataset's Internals
image::images/spark-sql-Dataset.png[align="center"]

Dataset API comes with declarative and type-safe operators (that improves on the experience in data processing using link:spark-sql-dataframe.adoc[DataFrames]).
