...
NOTE: Current batch may not be eligible for checkpointing when the time interval between current batch `time` and link:spark-streaming-dstreamgraph.adoc#zero-time[zero time] is not a multiple of link:spark-streaming-streamingcontext.adoc#checkpoint-interval[checkpoint interval].

CAUTION: FIXME Can a streaming context have different batch interval across checkpoints? Can _ever_ the note above be met?
