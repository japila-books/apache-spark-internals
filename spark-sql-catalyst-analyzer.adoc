...
NOTE: `TreeType` is the type of the plan tree that a rule works with, e.g. link:spark-sql-logical-plan.adoc[LogicalPlan], link:spark-sql-spark-plan.adoc[SparkPlan] or link:spark-sql-Expression.adoc[Expression].
