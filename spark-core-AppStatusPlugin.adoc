...
1. `SparkContext` is link:spark-sparkcontext-creating-instance-internals.adoc#ui[created] (and creates a web UI)

1. `FsHistoryProvider` link:spark-history-server-FsHistoryProvider.adoc#getAppUI[creates] a web UI

1. `AppStatusStore` creates an in-memory store for a live Spark application
====
