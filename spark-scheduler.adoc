...
When a job is submitted, a new <<ResultStage, ResultStage>> stage is created with the parent ShuffleMapStages linked (they can thus be created or linked to if other stages use them already).
...
A *ShuffleMapStage* (represented by  https://github.com/apache/spark/blob/master/core/src/main/scala/org/apache/spark/scheduler/ShuffleMapStage.scala[org.apache.spark.scheduler.ShuffleMapStage]) is an intermediate stage (in the execution DAG) that produces data for link:spark-rdd-shuffle.adoc[a shuffle] and is an input for the other stages in the DAG of stages.
