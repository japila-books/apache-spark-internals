...
Internally, a running job is represented by the `private[spark]` class `ActiveJob`.

[CAUTION]
====
FIXME

* When can ActiveJob (and hence a Spark job) have `finalStage` `ResultStage` vs `ShuffleMapStage`?
====
