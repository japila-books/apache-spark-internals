...
*DAGScheduler* is the present and only scheduling layer in Apache Spark that implements stage-oriented scheduling, i.e. RDD actions trigger jobs that become stages with tasks (see link:spark-execution-model.adoc[Execution Model]).

DAGScheduler does three things in Spark (explanations follow):
...
DAGScheduler uses an event queue architecture in which a thread can post an `DAGSchedulerEvent` event, e.g. a new job being submitted (`JobSubmitted`), that the scheduler reads and executes, sequentially. See the section <<event-loop, Internal Event Loop - dag-scheduler-event-loop>>.
...
When a job is submitted, a new <<ResultStage, ResultStage>> stage is created with the parent ShuffleMapStages linked (they can be created from scratch or linked to, i.e. shared, if other jobs use them already).
...
A *ShuffleMapStage* (aka *shuffle map stage*, or simply *map stage*) is an intermediate stage in the execution DAG that produces data for link:spark-rdd-shuffle.adoc[shuffle operation]. It is an input for the other following stages in the DAG of stages.

ShuffleMapStage is also known as a *shuffle dependency's map side* (see link:spark-rdd-dependencies.adoc#shuffle-dependency[ShuffleDependency])

ShuffleMapStages might contain multiple pipelined operations, e.g. `map` and `filter`, before shuffle operation.

CAUTION: FIXME: Show the example and the logs + figures

A single ShuffleMapStage can be part of many jobs.

CAUTION: FIXME: Show the example and the logs + figures

A ShuffleMapStage is a stage with additional link:spark-rdd-dependencies.adoc#shuffle-dependency[ShuffleDependency] - the shuffle that it is part of and `outputLocs` and `numAvailableOutputs` track how many map outputs are ready.
...
* newShuffleMapStage <- newShuffleMapStage
...
DAGScheduler tracks the mapping between shuffle id (FIXME tracked by SparkContext) and ShuffleMapStage in `shuffleToMapStage`. When there is no ShuffleMapStage for the shuffle id, one is created with the ancestor shuffle dependencies of the stage being registered to link:spark-service-mapoutputtracker.adoc[MapOutputTrackerMaster].

[CAUTION]
====
FIXME What does this mean "getShuffleMapStage = Get or create a shuffle map stage for the given shuffle dependency's map side."?
====
...
`DAGScheduler.submitMissingTasks` is called when the current stage's parents, i.e. <<ShuffleMapStage, ShuffleMapStages>>, are available and it is now possible to run task(s) for the current <<ResultStage, ResultStage>> stage.
...
DAGScheduler finds missing partitions of the target RDD to compute. The *missing partitions* are partitions for which the ActiveJob doesn't know are finished.
