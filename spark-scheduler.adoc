...
A job computes a stage as either <<ResultStage, ResultStage>> that is <<spark-rdd.adoc#actions,a result of executing an action on an RDD>> in a Spark user program or (_still work in progress_) <<ShuffleMapStage, ShuffleMapStage>> for <<adaptive-query-planning, adaptive query planning>>.

When a job is submitted, a new ResultStage stage is created with the parent ShuffleMapStages linked (they can be created from scratch or linked to, i.e. shared, if other jobs use them already).
...
DAGScheduler tracks the mapping of shuffles (by their ids from SparkContext) to corresponding ShuffleMapStages that compute them in `shuffleToMapStage`.

FIXME: Where's shuffleToMapStage used?

When there is no ShuffleMapStage for a shuffle id, one is created with the ancestor shuffle dependencies of the stage being registered to link:spark-service-mapoutputtracker.adoc[MapOutputTrackerMaster].
...
* Code review `getShuffleMapStage` - what it takes to create a shuffle map stage?
