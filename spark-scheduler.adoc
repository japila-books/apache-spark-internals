...
A *ShuffleMapStage* (represented by  https://github.com/apache/spark/blob/master/core/src/main/scala/org/apache/spark/scheduler/ShuffleMapStage.scala[org.apache.spark.scheduler.ShuffleMapStage]) is an intermediate stage (in the execution DAG) that produces data for link:spark-rdd-shuffle.adoc[shuffle operation] and is an input for the other following stages in the DAG of stages.

ShuffleMapStages might contain multiple pipelined operations before shuffle operation, e.g. `map` and `filter`.

CAUTION: FIXME: Show the example and the logs + figures

There can be multiple jobs trying to compute the same ShuffleMapStage.
