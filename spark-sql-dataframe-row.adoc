...
NOTE: It is link:spark-sql-catalyst.adoc#RowEncoder[RowEncoder] to take care of assigning a schema to a `Row` when `toDF` on a link:spark-sql-dataset.adoc[Dataset] or when instantiating link:spark-sql-dataframe.adoc[DataFrame] through link:spark-sql-dataframereader.adoc[DataFrameReader].
