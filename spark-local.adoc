...
You can run Spark in *local mode*. In this non-distributed single-JVM deployment mode, Spark spawns all the execution components - link:spark-runtime-environment.adoc#driver[driver], link:spark-runtime-environment.adoc#executor[executor], <<LocalBackend, backend>>, and link:spark-runtime-environment.adoc#master[master] - in the same JVM.
