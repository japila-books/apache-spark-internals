...
When `ReviveOffers` or `StatusUpdate` messages are received, `LocalEndpoint` launches a task (using `reviveOffers()` method).

The number of tasks to be launched is controlled by the number of threads as specified in <<masterURL, master URL>>.
