...
If there is at least one link:spark-streaming-dstreams.adoc#ReceiverInputDStream[ReceiverInputDStreams] the RPC endpoint `ReceiverTracker` (in <<ReceiverTrackerEndpoint, ReceiverTrackerEndpoint>>) is set up.
...
It then starts the <<ReceiverSupervisor, ReceiverSupervisor>> for `receiver` and keeps awaiting termination, i.e. once the task is run it does so until _a termination message_ comes from _some_ other external source). The task is a long-running task for `receiver`.
