...
=== [[allocateBlocksToBatch]] allocateBlocksToBatch

`allocateBlocksToBatch(batchTime: Time)` simply passes all the calls on to <<ReceivedBlockTracker, ReceivedBlockTracker>>, but only when there _are_ link:spark-streaming-receiverinputdstreams.adoc[receiver input streams] registered (in `receiverInputStreams` internal registry).
